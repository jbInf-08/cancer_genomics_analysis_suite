...
This module provides data collection capabilities for https://api.nih.gov.
"""
...
class NihCollector(DataCollectorBase):
    ...
    NIH provides:
    - clinical_trials, grants, publications
    """
    
    def __init__(self, output_dir: str = "data/external_sources/nih", **kwargs):
        """Initialize nih collector."""
        super().__init__(output_dir, **kwargs)
        self.base_url = self.config.get("base_url", "https://api.nih.gov")
        self.sample_limit = self.config.get("sample_limit", 30)
        self.cancer_types = self.config.get("cancer_types", [])
        self.data_types = self.config.get("data_types", ['clinical_trials', 'grants', 'publications'])
    ...
    def collect_data(self, 
        ...
        """
        self.logger.info(f"Collecting {data_type} data from NIH")
        ...
        try:
            ...
            mock_data = [
                {
                    "sample_id": f"nih_sample_{i}",
                    "data_type": data_type,
                    "value": i * 1.5,
                    "metadata": f"Sample {i} from nih"
                }
    ...
    def get_available_datasets(self) -> List[Dict[str, Any]]:
        """Get list of available datasets from NIH."""
        datasets = []
        ...
        for data_type in self.data_types:
            for cancer_type in self.cancer_types:
                datasets.append({
                    ...
                    "description": f"NIH {data_type} data for {cancer_type}",
