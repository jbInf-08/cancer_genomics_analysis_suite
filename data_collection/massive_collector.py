...
This module provides data collection capabilities for https://massive.ucsd.edu.
"""
...
class MassiveCollector(DataCollectorBase):
    """
    Data collector for MASSIVE.
    
    MASSIVE provides:
    - proteomics, metabolomics, spectra
    """
    
    def __init__(self, output_dir: str = "data/external_sources/massive", **kwargs):
        """Initialize massive collector."""
        super().__init__(output_dir, **kwargs)
        self.base_url = self.config.get("base_url", "https://massive.ucsd.edu")
        self.sample_limit = self.config.get("sample_limit", 25)
        self.cancer_types = self.config.get("cancer_types", [])
        self.data_types = self.config.get("data_types", ['proteomics', 'metabolomics', 'spectra'])
    
    def collect_data(self, 
                    data_type: str = "proteomics",
                    **kwargs) -> Dict[str, Any]:
        """
        Collect data from MASSIVE.
        ...
        """
        self.logger.info(f"Collecting {data_type} data from MASSIVE")
        ...
        try:
            ...
            mock_data = [
                {
                    "sample_id": f"massive_sample_{i}",
                    "data_type": data_type,
                    "value": i * 1.5,
                    "metadata": f"Sample {i} from massive"
                }
    ...
    def get_available_datasets(self) -> List[Dict[str, Any]]:
        """Get list of available datasets from MASSIVE."""
        datasets = []
        ...
        for data_type in self.data_types:
            for cancer_type in self.cancer_types:
                datasets.append({
                    ...
                    "description": f"MASSIVE {data_type} data for {cancer_type}",
