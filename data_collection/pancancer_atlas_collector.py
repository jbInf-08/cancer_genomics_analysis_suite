...
This module provides data collection capabilities for https://pancanatlas.xenahubs.net.
"""
...
class PancancerAtlasCollector(DataCollectorBase):
    ...
    PANCANCER_ATLAS provides:
    - genomics, proteomics, pathology
    """
    
    def __init__(self, output_dir: str = "data/external_sources/pancancer_atlas", **kwargs):
        """Initialize pancancer_atlas collector."""
        super().__init__(output_dir, **kwargs)
        self.base_url = self.config.get("base_url", "https://pancanatlas.xenahubs.net")
        self.sample_limit = self.config.get("sample_limit", 50)
        self.cancer_types = self.config.get("cancer_types", ['all'])
        self.data_types = self.config.get("data_types", ['genomics', 'proteomics', 'pathology'])
    
    def collect_data(self, 
                    data_type: str = "genomics",
                    **kwargs) -> Dict[str, Any]:
        """
        Collect data from PANCANCER_ATLAS.
        ...
        """
        self.logger.info(f"Collecting {data_type} data from PANCANCER_ATLAS")
        ...
        try:
            ...
            mock_data = [
                {
                    "sample_id": f"pancancer_atlas_sample_{i}",
                    "data_type": data_type,
                    "value": i * 1.5,
                    "metadata": f"Sample {i} from pancancer_atlas"
                }
    ...
    def get_available_datasets(self) -> List[Dict[str, Any]]:
        """Get list of available datasets from PANCANCER_ATLAS."""
        datasets = []
        ...
        for data_type in self.data_types:
            for cancer_type in self.cancer_types:
                datasets.append({
                    ...
                    "description": f"PANCANCER_ATLAS {data_type} data for {cancer_type}",
                    "estimated_samples": self.sample_limit,
                    "source": "PANCANCER_ATLAS"
                })
