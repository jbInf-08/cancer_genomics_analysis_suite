...
This module provides data collection capabilities for https://miccai.org.
"""
...
class MiccaiCollector(DataCollectorBase):
    ...
    MICCAI provides:
    - imaging, segmentation, classification
    """
    
    def __init__(self, output_dir: str = "data/external_sources/miccai", **kwargs):
        """Initialize miccai collector."""
        super().__init__(output_dir, **kwargs)
        self.base_url = self.config.get("base_url", "https://miccai.org")
        self.sample_limit = self.config.get("sample_limit", 15)
        self.cancer_types = self.config.get("cancer_types", [])
        self.data_types = self.config.get("data_types", ['imaging', 'segmentation', 'classification'])
    ...
    def collect_data(self, 
        ...
        """
        self.logger.info(f"Collecting {data_type} data from MICCAI")
        ...
        try:
            ...
            mock_data = [
                {
                    "sample_id": f"miccai_sample_{i}",
                    "data_type": data_type,
                    "value": i * 1.5,
                    "metadata": f"Sample {i} from miccai"
                }
    ...
    def get_available_datasets(self) -> List[Dict[str, Any]]:
        """Get list of available datasets from MICCAI."""
        datasets = []
        ...
        for data_type in self.data_types:
            for cancer_type in self.cancer_types:
                datasets.append({
                    ...
                    "description": f"MICCAI {data_type} data for {cancer_type}",
