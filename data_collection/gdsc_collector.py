...
This module provides data collection capabilities for https://www.cancerrxgene.org.
"""
...
class GdscCollector(DataCollectorBase):
    ...
    GDSC provides:
    - drug_response, genomics, cell_lines
    """
    
    def __init__(self, output_dir: str = "data/external_sources/gdsc", **kwargs):
        """Initialize gdsc collector."""
        super().__init__(output_dir, **kwargs)
        self.base_url = self.config.get("base_url", "https://www.cancerrxgene.org")
        self.sample_limit = self.config.get("sample_limit", 50)
        self.cancer_types = self.config.get("cancer_types", [])
        self.data_types = self.config.get("data_types", ['drug_response', 'genomics', 'cell_lines'])
    
    def collect_data(self, 
                    data_type: str = "drug_response",
                    **kwargs) -> Dict[str, Any]:
        """
        Collect data from GDSC.
        ...
        """
        self.logger.info(f"Collecting {data_type} data from GDSC")
        ...
        try:
            ...
            mock_data = [
                {
                    "sample_id": f"gdsc_sample_{i}",
                    "data_type": data_type,
                    "value": i * 1.5,
                    "metadata": f"Sample {i} from gdsc"
                }
    ...
    def get_available_datasets(self) -> List[Dict[str, Any]]:
        """Get list of available datasets from GDSC."""
        datasets = []
        ...
        for data_type in self.data_types:
            for cancer_type in self.cancer_types:
                datasets.append({
                    ...
                    "description": f"GDSC {data_type} data for {cancer_type}",
