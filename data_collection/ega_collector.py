...
This module provides data collection capabilities for https://ega-archive.org.
"""
...
class EgaCollector(DataCollectorBase):
    ...
    def __init__(self, output_dir: str = "data/external_sources/ega", **kwargs):
        """Initialize ega collector."""
        super().__init__(output_dir, **kwargs)
        self.base_url = self.config.get("base_url", "https://ega-archive.org")
        self.sample_limit = self.config.get("sample_limit", 25)
        self.cancer_types = self.config.get("cancer_types", [])
        self.data_types = self.config.get("data_types", ['genomics', 'phenomics'])
    ...
    def collect_data(self, 
        ...
        """
        self.logger.info(f"Collecting {data_type} data from EGA")
        ...
        try:
            ...
            mock_data = [
                {
                    "sample_id": f"ega_sample_{i}",
                    "data_type": data_type,
                    "value": i * 1.5,
                    "metadata": f"Sample {i} from ega"
                }
    ...
    def get_available_datasets(self) -> List[Dict[str, Any]]:
        """Get list of available datasets from EGA."""
        datasets = []
        ...
        for data_type in self.data_types:
            for cancer_type in self.cancer_types:
                datasets.append({
                    ...
                    "description": f"EGA {data_type} data for {cancer_type}",
