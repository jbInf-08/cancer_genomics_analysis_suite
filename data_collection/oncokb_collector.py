...
This module provides data collection capabilities for https://www.oncokb.org/api/v1.
"""
...
class OncokbCollector(DataCollectorBase):
    ...
    ONCOKB provides:
    - mutations, drugs, evidence
    """
    
    def __init__(self, output_dir: str = "data/external_sources/oncokb", **kwargs):
        """Initialize oncokb collector."""
        super().__init__(output_dir, **kwargs)
        self.base_url = self.config.get("base_url", "https://www.oncokb.org/api/v1")
        self.sample_limit = self.config.get("sample_limit", 50)
        self.cancer_types = self.config.get("cancer_types", [])
        self.data_types = self.config.get("data_types", ['mutations', 'drugs', 'evidence'])
    
    def collect_data(self, 
                    data_type: str = "mutations",
                    **kwargs) -> Dict[str, Any]:
        """
        Collect data from ONCOKB.
        ...
        """
        self.logger.info(f"Collecting {data_type} data from ONCOKB")
        ...
        try:
            ...
            mock_data = [
                {
                    "sample_id": f"oncokb_sample_{i}",
                    "data_type": data_type,
                    "value": i * 1.5,
                    "metadata": f"Sample {i} from oncokb"
                }
    ...
    def get_available_datasets(self) -> List[Dict[str, Any]]:
        """Get list of available datasets from ONCOKB."""
        datasets = []
        ...
        for data_type in self.data_types:
            for cancer_type in self.cancer_types:
                datasets.append({
                    ...
                    "description": f"ONCOKB {data_type} data for {cancer_type}",
