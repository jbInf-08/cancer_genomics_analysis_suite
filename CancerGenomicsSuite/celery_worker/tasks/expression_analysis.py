...
@celery.task(
    bind=True, name="celery_worker.tasks.expression_analysis.normalize_expression_data"
)
def normalize_expression_data(
    self, data_path: str, method: str = "quantile"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 25, "total": 100, "status": "Data loaded"},
        )
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 75, "total": 100, "status": "Normalization complete"},
        )
...
@celery.task(
    bind=True, name="celery_worker.tasks.expression_analysis.differential_expression"
)
def differential_expression(
    ...
    try:
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 25, "total": 100, "status": "Calculating statistics"},
        )
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 75, "total": 100, "status": "Filtering results"},
        )

        # Filter significant results
        significant_genes = de_results[
            (de_results["p_value"] < 0.05) & (abs(de_results["log2_fold_change"]) > 1.0)
        ]
        ...
        stats = {
            ...
            "upregulated": len(
                significant_genes[significant_genes["log2_fold_change"] > 0]
            ),
            "downregulated": len(
                significant_genes[significant_genes["log2_fold_change"] < 0]
            ),
...
@celery.task(
    bind=True, name="celery_worker.tasks.expression_analysis.pathway_enrichment"
)
def pathway_enrichment(
    self, gene_list: List[str], pathway_db: str = "kegg"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 0, "total": 100, "status": "Loading pathway database"},
        )
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 25, "total": 100, "status": "Calculating enrichment"},
        )
...
def _calculate_enrichment(
    genes: List[str], pathways: Dict[str, List[str]]
) -> pd.DataFrame:
