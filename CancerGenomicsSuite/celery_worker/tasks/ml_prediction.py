...
def train_survival_model(
    ...
    try:
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 25, "total": 100, "status": "Splitting data"},
        )
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 50, "total": 100, "status": "Training model"},
        )
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 75, "total": 100, "status": "Evaluating model"},
        )
...
@celery.task(
    bind=True, name="celery_worker.tasks.ml_prediction.predict_treatment_response"
)
def predict_treatment_response(
    self, patient_features: Dict, treatment: str, model_path: str = None
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 25, "total": 100, "status": "Preparing features"},
        )
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 50, "total": 100, "status": "Scaling features"},
        )
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 75, "total": 100, "status": "Making prediction"},
        )
...
def cluster_patients(
    self, multi_omics_data: Dict, n_clusters: int = 3, method: str = "kmeans"
) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        self.update_state(
            state="PROGRESS", meta={"current": 50, "total": 100, "status": "Clustering"}
        )
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 75, "total": 100, "status": "Analyzing clusters"},
        )
        ...
        patient_assignments = pd.DataFrame(
            {"patient_id": X.index, "cluster": cluster_labels}
        )
...
def feature_selection(
    ...
    try:
        ...
        self.update_state(
            state="PROGRESS",
            meta={"current": 25, "total": 100, "status": "Running selection"},
        )

        # Perform feature selection
        selected_features, importance_scores = _perform_feature_selection(
            X, y, method, n_features
        )

        self.update_state(
            state="PROGRESS",
            meta={"current": 75, "total": 100, "status": "Analyzing results"},
        )

        # Analyze selected features
        feature_analysis = _analyze_selected_features(
...
# Helper functions
def _prepare_survival_features(
    clinical_data: Dict, expression_data: Dict, mutation_data: Dict
...
def _prepare_patient_features(
    patient_features: Dict, feature_names: List[str]
) -> pd.DataFrame:
...
def _perform_feature_selection(
    X: pd.DataFrame, y: np.ndarray, method: str, n_features: int
    ...
    if method == "mutual_info":
        ...
        importance_scores = dict(
            zip(selected_features, feature_importance[selected_indices])
        )
...
def _analyze_selected_features(
    selected_features: List[str], importance_scores: Dict[str, float]
) -> Dict[str, Any]:
    """Analyze selected features."""
    return {
        "top_features": sorted(
        ...
        "feature_categories": {
            "expression": len(
                [f for f in selected_features if "expression" in f.lower()]
            ),
