    ...
    async def demonstrate_pipeline_orchestration(self):
        ...
        try:
            ...
            nextflow_result = await self.nextflow_manager.execute_pipeline(
                ...
                config={"reference_genome": "hg38", "threads": 4, "memory": "8GB"},
            )
    ...
    async def demonstrate_real_time_processing(self):
        ...
        try:
            ...
            sample_data = {
                "sample_id": "S001",
                "patient_id": "P001",
                "gene_expression": {"BRCA1": 2.5, "BRCA2": 1.8, "TP53": 3.2},
                "mutations": [
                    {"gene": "BRCA1", "variant": "c.5266dupC", "type": "frameshift"}
                ],
                "timestamp": datetime.now().isoformat(),
                "metadata": {"tissue_type": "breast", "stage": "IIA", "grade": "2"},
            }

            await self.kafka_manager.send_data(
                topic=self.settings.kafka.kafka_data_topic, data=sample_data
            )
        ...
        try:
            ...
            if self.settings.oauth2.keycloak_enabled:
                ...
                user_info = await self.keycloak_client.authenticate_user(
                    username="demo@example.com", password="demo_password"
                )
        ...
        processed_data = {
            ...
            "analysis_results": {
                "expression_summary": {
                    ...
                    "high_expression": sum(
                        1 for v in data.get("gene_expression", {}).values() if v > 2.0
                    ),
                },
                "mutation_summary": {
                    "total_mutations": len(data.get("mutations", [])),
                    "pathogenic_mutations": len(
                        [
                            m
                            for m in data.get("mutations", [])
                            if m.get("type") == "frameshift"
                        ]
                    ),
                },
            },
    ...
    async def run_integrated_workflow(self):
        ...
        try:
            ...
            if self.settings.features.enable_oauth2_auth:
                ...
                if self.settings.oauth2.keycloak_enabled:
                    user_info = await self.keycloak_client.authenticate_user(
                        username="researcher@example.com", password="research_password"
                    )
                    logger.info(
                        f"User authenticated: {user_info['preferred_username']}"
                    )
            ...
            if self.settings.features.enable_kafka_streaming:
                ...
                await self.kafka_manager.send_data(
                    topic=self.settings.kafka.kafka_results_topic, data=results_data
                )
