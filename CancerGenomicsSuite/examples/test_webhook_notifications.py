    ...
    try:
        ...
        test_alerts = [
            create_test_alert(
                AlertSeverity.INFO,
                "Test Info Alert",
                "This is a test info alert from the Cancer Genomics Analysis Suite.",
            ),
            create_test_alert(
                AlertSeverity.WARNING,
                "Test Warning Alert",
                "This is a test warning alert indicating a potential issue.",
            ),
            create_test_alert(
                AlertSeverity.ERROR,
                "Test Error Alert",
                "This is a test error alert indicating a problem that needs attention.",
            ),
            create_test_alert(
                AlertSeverity.CRITICAL,
                "Test Critical Alert",
                "This is a test critical alert requiring immediate action.",
            ),
        ]
        ...
        for alert in test_alerts:
            ...
            if success:
                logger.info(
                    f"✅ {alert.severity.value} alert sent successfully to Slack"
                )
            else:
...
def test_teams_webhook(webhook_url: str):
    ...
    try:
        ...
        for alert in test_alerts:
            ...
            if success:
                logger.info(
                    f"✅ {alert.severity.value} alert sent successfully to Teams"
                )
            else:
...
def main():
    ...
    try:
        ...
        test_email_notification(
            SMTP_SERVER, SMTP_PORT, SMTP_USERNAME, SMTP_PASSWORD, FROM_EMAIL, TO_EMAILS
        )
