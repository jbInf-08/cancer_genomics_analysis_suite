...
async def main():
    ...
    try:
        ...
        execution_id = await dispatcher.execute_pipeline(
            pipeline_name=pipeline.name, inputs=inputs, parameters=parameters
        )
        ...
        for i in range(5):
            ...
            try:
                ...
                if execution.status in [
                    ...
                    logger.info(
                        f"Pipeline execution finished with status: {execution.status.value}"
                    )
        ...
        for pipeline in pipelines:
            logger.info(
                f"  - {pipeline.name} v{pipeline.version} ({pipeline.pipeline_type.value})"
            )
