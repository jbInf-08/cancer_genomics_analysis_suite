...
class BioinformaticsCLI:
    ...
    def list_tools(self):
        ...
        for tool_name, tool_client in self.tools.items():
            status = (
                "✅ Available"
                if self._is_tool_available(tool_client)
                else "❌ Not Available"
            )
    ...
    def matlab_cli(self, args):
        ...
        parser.add_argument(
            "--version", action="store_true", help="Show MATLAB version"
        )
    ...
    def editors_cli(self, args):
        ...
        parser.add_argument(
            "--list-editors", action="store_true", help="List available editors"
        )
        ...
        if parsed_args.open:
            result = client.open_file(
                parsed_args.open, parsed_args.editor, parsed_args.line
            )
            if result["success"]:
    ...
    def ape_cli(self, args):
        ...
        parser.add_argument(
            "--design-primers", help="Design primers (format: start,end)"
        )
        parser.add_argument(
            "--export", help="Export plasmid (format: genbank,fasta,ape)"
        )
    ...
    def gromacs_cli(self, args):
        ...
        parser.add_argument(
            "--version", action="store_true", help="Show GROMACS version"
        )
        parser.add_argument(
            "--run-simulation", help="Run simulation (input files JSON)"
        )
    ...
    def wgsim_cli(self, args):
        ...
        parser.add_argument(
            "--num-reads", type=int, default=1000000, help="Number of reads"
        )
        parser.add_argument("--read-length", type=int, default=100, help="Read length")
        parser.add_argument(
            "--tool", choices=["wgsim", "dwgsim"], default="wgsim", help="Tool to use"
        )
        ...
        if parsed_args.reference and parsed_args.output:
            ...
            if result["success"]:
                ...
                print(
                    f"Generated {parsed_args.num_reads} reads of length {parsed_args.read_length}"
                )
            else:
    ...
    def neurosnap_cli(self, args):
        ...
        parser.add_argument(
            "--analysis-type", default="spike_detection", help="Analysis type"
        )
        parser.add_argument(
            "--version", action="store_true", help="Show Neurosnap version"
        )
        ...
        if parsed_args.analyze:
            result = client.analyze_neural_data(
                parsed_args.analyze, parsed_args.analysis_type
            )
            if result["success"]:
    ...
    def tamarind_cli(self, args):
        ...
        parser.add_argument(
            "--version", action="store_true", help="Show Tamarind version"
        )
