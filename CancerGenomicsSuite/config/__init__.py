    ...
    try:
        ...
        if hasattr(settings, "database"):
            if not hasattr(settings.database, "url") or not settings.database.url:
                ...
                results["info"].append(
                    f"Database configured: {settings.database.url[:20]}..."
                )
            ...
            for attr in dir(settings.features):
                if attr.startswith("enable_") and getattr(settings.features, attr):
                    enabled_features.append(
                        attr.replace("enable_", "").replace("_", " ").title()
                    )
            ...
            if not os.path.exists(upload_folder):
                try:
                    ...
                    results["warnings"].append(
                        f"Could not create upload directory: {e}"
                    )
...
logger.info(
    f"Configuration package loaded for {settings.app_name} v{settings.app_version}"
)
