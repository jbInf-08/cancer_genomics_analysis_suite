...
def test_api_tokens():
    ...
    try:
        ...
        print(
            f"✓ COSMIC API Token: {'Set' if settings.external_apis.cosmic_api_token else 'Not set'}"
        )
        print(
            f"✓ COSMIC API Key: {'Set' if settings.external_apis.cosmic_api_key else 'Not set'}"
        )
        print(
            f"✓ Scopus API Key: {'Set' if settings.external_apis.scopus_api_key else 'Not set'}"
        )
        print(f"✓ ENCODE API Base: {settings.external_apis.encode_api_base}")
        print(
            f"✓ NCBI API Key: {'Set' if settings.external_apis.ncbi_api_key else 'Not set'}"
        )
...
def test_simple_config_compatibility():
    ...
    try:
        ...
        if hasattr(settings, "SQLALCHEMY_DATABASE_URI"):
            print(
                f"✓ SQLALCHEMY_DATABASE_URI: {settings.SQLALCHEMY_DATABASE_URI[:30]}..."
            )
        ...
        if hasattr(settings, "COSMIC_API_TOKEN"):
            print(
                f"✓ COSMIC_API_TOKEN: {'Set' if settings.COSMIC_API_TOKEN else 'Not set'}"
            )
        if hasattr(settings, "SCOPUS_API_KEY"):
            print(
                f"✓ SCOPUS_API_KEY: {'Set' if settings.SCOPUS_API_KEY else 'Not set'}"
            )
...
def main():
    ...
    tests = [test_api_tokens, test_simple_config_compatibility]
