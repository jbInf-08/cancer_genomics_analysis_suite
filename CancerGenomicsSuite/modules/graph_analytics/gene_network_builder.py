...
class GeneNetworkBuilder:
    ...
    def build_coexpression_network(
        ...
        logger.info(
            f"Built co-expression network with {network.number_of_nodes()} nodes and {edges_added} edges"
        )
        return network
    ...
    def build_protein_interaction_network(
        ...
        logger.info(
            f"Built protein interaction network with {network.number_of_nodes()} nodes and {network.number_of_edges()} edges"
        )
        return network
    ...
    def build_pathway_network(
        ...
        logger.info(
            f"Built pathway network with {network.number_of_nodes()} nodes and {network.number_of_edges()} edges"
        )
        return network
    ...
    def build_mutation_network(
        ...
        for (gene1, gene2), count in co_mutation_counts.items():
            if count >= co_mutation_threshold:
                network.add_edge(
                    gene1, gene2, co_mutation_count=count, edge_type="co_mutation"
                )

        self.networks["mutation"] = network
        logger.info(
            f"Built mutation network with {network.number_of_nodes()} nodes and {network.number_of_edges()} edges"
        )
        return network
    ...
    def integrate_networks(
            ...
            if integration_method == "union":
                ...
                for edge in network.edges(data=True):
                    ...
                    if integrated.has_edge(node1, node2):
                        # Update weight
                        current_weight = integrated[node1][node2].get(
                            edge_weight_attr, 1.0
                        )
                        new_weight = attrs.get(edge_weight_attr, 1.0)
                        integrated[node1][node2][edge_weight_attr] = (
        ...
        logger.info(
            f"Integrated {len(network_names)} networks into a single network with {integrated.number_of_nodes()} nodes and {integrated.number_of_edges()} edges"
        )
        return integrated
    ...
    def filter_network(
        ...
        logger.info(
            f"Filtered network: {network.number_of_nodes()} -> {filtered.number_of_nodes()} nodes, {network.number_of_edges()} -> {filtered.number_of_edges()} edges"
        )
        return filtered
    ...
    def get_network_statistics(
        ...
        stats = {
            ...
            "degree_distribution": {},
            "clustering": {"avg_clustering": nx.average_clustering(network)},
        }
    ...
    def export_network(
        self, network: nx.Graph, output_file: str, format: str = "graphml"
    ):
        """
    ...
    def get_network_summary(self) -> Dict[str, Any]:
        ...
        """
        summary = {"networks": {}, "integrated_network": {}}
