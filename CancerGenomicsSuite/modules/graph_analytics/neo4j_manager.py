...
class Neo4jManager:
    ...
    def __init__(
        ...
        if not NEO4J_AVAILABLE:
            raise ImportError(
                "Neo4j drivers not available. Install neo4j and py2neo packages."
            )
    ...
    def connect(self):
        ...
        try:
            ...
            self.graph = Graph(
                self.uri, auth=(self.username, self.password), database=self.database
            )
    ...
    def create_relationship(
        ...
        relationship = Relationship(
            start_node, relationship_type, end_node, **rel_properties
        )
        self.graph.create(relationship)

        logger.debug(
            f"Created relationship: {start_node} -[{relationship_type}]-> {end_node}"
        )
        return relationship
    ...
    def calculate_gene_centrality(self, gene_name: str) -> Dict[str, float]:
        ...
        degree_result = self.execute_cypher_query(
            degree_query, {"gene_name": gene_name}
        )
        degree_centrality = (
            degree_result[0]["degree_centrality"] if degree_result else 0
        )
        ...
        betweenness_result = self.execute_cypher_query(
            betweenness_query, {"gene_name": gene_name}
        )
        betweenness_centrality = (
            betweenness_result[0]["betweenness_centrality"] if betweenness_result else 0
        )
    ...
    def find_shortest_path(self, start_gene: str, end_gene: str) -> List[Dict]:
        ...
        results = self.execute_cypher_query(
            query, {"start_gene": start_gene, "end_gene": end_gene}
        )
        return results
    ...
    def import_protein_interactions(self, interaction_data: pd.DataFrame):
        ...
        for _, row in interaction_data.iterrows():
            ...
            protein1 = self.node_matcher.match(
                "Protein", protein_name=row["protein1"]
            ).first()
            if not protein1:
                protein1 = self.create_protein_node(
                    protein_id=row["protein1"], protein_name=row["protein1"]
                )

            protein2 = self.node_matcher.match(
                "Protein", protein_name=row["protein2"]
            ).first()
            if not protein2:
                protein2 = self.create_protein_node(
                    protein_id=row["protein2"], protein_name=row["protein2"]
                )
    ...
    def get_graph_statistics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            stats["node_counts"] = {
                item["label"]: item["count"] for item in node_counts
            }
        except Exception as e:
        ...
        try:
            ...
            stats["relationship_counts"] = {
                item["relationshipType"]: item["count"] for item in rel_counts
            }
        except Exception as e:
        ...
        try:
            ...
            total_rels = self.execute_cypher_query(total_rels_query)[0][
                "total_relationships"
            ]
