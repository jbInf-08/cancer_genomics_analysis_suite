...
try:
    ...
    logging.warning(
        "Visualization libraries not available. Install matplotlib and plotly packages."
    )
...
class GraphVisualizer:
    ...
    def visualize_gene_network(
        ...
        if node_size_attr and node_size_attr in next(iter(graph.nodes(data=True)))[1]:
            node_sizes = [
                graph.nodes[node].get(node_size_attr, 100) for node in graph.nodes()
            ]
            # Normalize sizes
        ...
        if node_color_attr and node_color_attr in next(iter(graph.nodes(data=True)))[1]:
            node_colors = [
                graph.nodes[node].get(node_color_attr, 0) for node in graph.nodes()
            ]
        else:
        ...
        if edge_width_attr and edge_width_attr in next(iter(graph.edges(data=True)))[2]:
            edge_widths = [
                graph.edges[edge].get(edge_width_attr, 1) for edge in graph.edges()
            ]
            # Normalize widths
            edge_widths = np.array(edge_widths)
            if edge_widths.max() > edge_widths.min():
                edge_widths = (edge_widths - edge_widths.min()) / (
                    edge_widths.max() - edge_widths.min()
                ) * 3 + 0.5
        else:
            edge_widths = 1
        ...
        nx.draw_networkx_nodes(
            graph, pos, node_size=node_sizes, node_color=node_colors, alpha=0.7, ax=ax
        )

        nx.draw_networkx_edges(
            graph, pos, width=edge_widths, alpha=0.5, edge_color="gray", ax=ax
        )
    ...
    def plot_centrality_analysis(
        ...
        for i, centrality_type in enumerate(centrality_types):
            ...
            gene_scores = [
                (gene, centrality_data[gene][centrality_type]) for gene in genes
            ]
            ...
            axes[i].set_title(
                f'Top {top_k} Genes by {centrality_type.replace("_", " ").title()}'
            )
    ...
    def plot_community_structure(
        ...
        for community_id in range(num_communities):
            community_nodes = [
                node for node, comm in communities.items() if comm == community_id
            ]
        ...
        # Draw edges
        nx.draw_networkx_edges(graph, pos, alpha=0.5, edge_color="gray", ax=ax)
    ...
    def create_dashboard(
        ...
        fig.add_trace(
            go.Scatter(x=[0, 1], y=[0, 1], mode="markers", name="Network"), row=1, col=1
        )
        ...
        if centrality_data:
            ...
            degree_centrality = [
                centrality_data[gene]["degree_centrality"] for gene in genes
            ]
        ...
        fig.add_trace(
            ...
            row=2 if enrichment_data else 1,
            col=3,
        )
        ...
        if enrichment_data is not None:
            ...
            fig.add_trace(
                go.Bar(
                    x=top_pathways["pathway_name"],
                    y=top_pathways["fold_enrichment"],
    ...
    def export_visualization(
        self, fig: Union[plt.Figure, go.Figure], output_path: str, format: str = "png"
    ):
        """
