...
class NetworkXAnalyzer:
    ...
    def build_gene_network(
        ...
        for _, gene in gene_data.iterrows():
            ...
            if (
                ...
                and gene["gene_name"] in expression_data.columns
            ):
        ...
        logger.info(
            f"Built gene network with {graph.number_of_nodes()} nodes and {graph.number_of_edges()} edges"
        )
        return graph
    ...
    def build_protein_interaction_network(
        ...
        for _, interaction in interaction_data.iterrows():
            ...
            if protein1 in graph.nodes and protein2 in graph.nodes:
                edge_attrs = {
                    "interaction_type": interaction.get("interaction_type", "unknown"),
                    "confidence": interaction.get("confidence", 0.0),
                    "experimental_method": interaction.get(
        ...
        logger.info(
            f"Built protein network with {graph.number_of_nodes()} nodes and {graph.number_of_edges()} edges"
        )
        return graph
    ...
    def build_pathway_network(
        ...
        self.graph = graph
        logger.info(
            f"Built pathway network with {graph.number_of_nodes()} nodes and {graph.number_of_edges()} edges"
        )
        return graph
    ...
    def detect_communities(
        self, graph: Optional[nx.Graph] = None, method: str = "greedy"
    ) -> Dict[str, int]:
        """
    ...
    def find_shortest_paths(
        self, source: str, target: str, graph: Optional[nx.Graph] = None
    ) -> List[List[str]]:
        """
    ...
    def calculate_pathway_enrichment(
        ...
        for _, pathway in pathway_data.iterrows():
            ...
            pathway_genes = set(
                gene_pathway_data[gene_pathway_data["pathway_name"] == pathway_name][
                    "gene_name"
                ]
            )
    ...
    def find_hub_genes(
        self, graph: Optional[nx.Graph] = None, top_k: int = 10
        ...
        degree_cent = degree_centrality(graph)
        hub_genes = sorted(degree_cent.items(), key=lambda x: x[1], reverse=True)[
            :top_k
        ]
    ...
    def get_network_statistics(
        ...
        stats = {
            ...
            "clustering": {"avg_clustering": nx.average_clustering(graph)},
        }
    ...
    def export_network_data(
        self, graph: Optional[nx.Graph] = None, output_file: str = "network_data.json"
    ):
        """
        ...
        network_data = {
            "nodes": [
                {"id": node, "attributes": dict(graph.nodes[node])}
                for node in graph.nodes()
            ],
            "edges": [
                {
