...
class PathwayAnalyzer:
    ...
    def load_pathway_data(
        self, pathway_data: pd.DataFrame, gene_pathway_data: pd.DataFrame
    ):
        """
        ...
        logger.info(
            f"Loaded {len(self.pathway_data)} pathways with {len(self.gene_pathway_mapping)} genes"
        )
    ...
    def _build_pathway_graph(self):
        ...
        logger.info(
            f"Built pathway graph with {self.pathway_graph.number_of_nodes()} nodes and {self.pathway_graph.number_of_edges()} edges"
        )
    ...
    def calculate_pathway_enrichment(
        ...
        for pathway_name, pathway_info in self.pathway_data.items():
            ...
            if (
                len(pathway_genes) < min_pathway_size
                or len(pathway_genes) > max_pathway_size
            ):
                continue
            ...
            enrichment_ratio = (overlap_count / len(gene_list)) / (
                len(pathway_genes) / len(background_genes)
            )
    ...
    def _hypergeometric_test(
        self, overlap: int, test_size: int, pathway_size: int, background_size: int
    ) -> float:
    ...
    def find_pathway_modules(
        self, method: str = "greedy", min_module_size: int = 3
    ) -> Dict[str, List[str]]:
        """
    ...
    def calculate_pathway_centrality(self) -> Dict[str, Dict[str, float]]:
        ...
        try:
            eigenvector_cent = nx.eigenvector_centrality(
                self.pathway_graph, max_iter=1000
            )
    ...
    def find_pathway_interactions(self, pathway1: str, pathway2: str) -> Dict[str, Any]:
        """
        ...
        overlap_coefficient = (
            len(overlap_genes) / min(len(genes1), len(genes2))
        ...
        graph_similarity = 0
        if graph_connected:
            graph_similarity = self.pathway_graph[pathway1][pathway2].get(
    ...
    def get_pathway_statistics(self) -> Dict[str, Any]:
        ...
        for pathway_info in self.pathway_data.values():
            ...
            stats["pathway_types"][pathway_type] = (
                stats["pathway_types"].get(pathway_type, 0) + 1
            )
    ...
    def find_similar_pathways(
        self, pathway_name: str, top_k: int = 10, min_similarity: float = 0.1
        ...
        for other_pathway in self.pathway_data.keys():
            ...
            if self.pathway_graph.has_edge(pathway_name, other_pathway):
                similarity = self.pathway_graph[pathway_name][other_pathway].get(
                    "jaccard_similarity", 0
                )
    ...
    def analyze_pathway_dysregulation(
        ...
        available_genes = [
            gene for gene in pathway_genes if gene in gene_expression_data.columns
        ]
        ...
        for group_name, samples in sample_groups.items():
            ...
            available_samples = [
                sample for sample in samples if sample in gene_expression_data.index
            ]
            ...
            pathway_expression = gene_expression_data.loc[
                available_samples, available_genes
            ]
