...
class MutationEffectPredictor:
    ...
    def predict_mutation_effect(
        self, mutation: Mutation, predictors: Optional[List[str]] = None
        ...
        for predictor_name in predictors:
            ...
            try:
                logger.info(
                    f"Running {predictor_name} prediction for {mutation.gene_symbol}"
                )
    ...
    def _predict_clinvar(self, mutation: Mutation) -> PredictionResult:
        ...
        clinvar_significance = (
            mutation.clinical_significance or "uncertain_significance"
        )
        ...
        prediction_class = significance_mapping.get(
    ...
    def batch_predict(
        self, mutations: List[Mutation], predictors: Optional[List[str]] = None
    ...
    def export_predictions(
        self, results: List[PredictionResult], format: str = "json"
    ) -> str:
        """
...
if __name__ == "__main__":
    ...
    for mutation in sample_mutations:
        ...
        for result in results:
            print(
                f"  {result.predictor_name}: {result.prediction_class} (score: {result.prediction_score:.3f}, confidence: {result.confidence:.3f})"
            )

        consensus = predictor.get_consensus_prediction(results)
        print(
            f"  Consensus: {consensus['consensus_class']} (ratio: {consensus['consensus_ratio']:.2f})"
        )
