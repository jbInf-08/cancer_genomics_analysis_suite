...
class DeliveryQueue:
    ...
    def _start_workers(self):
        ...
        for i in range(self.max_workers):
            worker = threading.Thread(
                target=self._worker_loop, name=f"DeliveryWorker-{i}", daemon=True
            )
            worker.start()
    ...
    def _handle_delivery_failure(self, message: NotificationMessage, error: str = None):
        ...
        if message.retry_count >= message.max_retries:
            ...
            logger.info(
                f"Message {message.id} scheduled for retry {message.retry_count} at {retry_time}"
            )
    ...
    def _update_message_status(self, message_id: str, status: MessageStatus, **updates):
    ...
    def get_messages_by_status(
        self, status: MessageStatus, limit: int = 100
