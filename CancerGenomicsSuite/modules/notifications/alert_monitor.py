    ...
    def _format_payload(self, alert: Alert) -> Dict[str, Any]:
        ...
        if alert.metadata:
            ...
            for k, v in alert.metadata.items():
                metadata_facts.append({"name": k, "value": str(v)})
...
class AlertMonitor:
    ...
    def _setup_default_rules(self):
        ...
        self.alert_rules = {
            ...
            "high_failure_rate": {
                "condition": lambda metrics: metrics.failed_analyses > 10
                and metrics.completed_analyses > 0
                and (
                    metrics.failed_analyses
                    / (metrics.completed_analyses + metrics.failed_analyses)
                )
                > 0.2,
    ...
    def start_monitoring(self):
        ...
        if not self.is_running:
            ...
            self.monitor_thread = threading.Thread(
                target=self._monitoring_loop, name="AlertMonitor", daemon=True
            )
    ...
    def _send_with_retry(self, notifier, alert: Alert, channel_name: str):
        ...
        for attempt in range(self.notification_retry_count):
            try:
                ...
                if success:
                    logger.info(
                        f"{channel_name} notification sent successfully for alert {alert.id}"
                    )
                    return
                else:
                    logger.warning(
                        f"{channel_name} notification failed for alert {alert.id}, attempt {attempt + 1}"
                    )
            except Exception as e:
                logger.error(
                    f"{channel_name} notification error for alert {alert.id}, attempt {attempt + 1}: {e}"
                )
        ...
        logger.error(
            f"Failed to send {channel_name} notification for alert {alert.id} after {self.notification_retry_count} attempts"
        )
