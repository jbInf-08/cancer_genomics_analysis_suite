...
class DashboardBuilder:
    ...
    def _generate_grid_layout(self, layout: DashboardLayout) -> html.Div:
        ...
        # Dashboard header
        if layout.title:
            children.append(html.H1(layout.title, className="dashboard-title"))
    ...
    def _generate_tab_layout(self, layout: DashboardLayout) -> html.Div:
        ...
        for tab_name, widgets in tab_groups.items():
            ...
            for widget in widgets:
                widget_component = self._create_widget_component(widget)
                tab_content.append(html.Div(widget_component, className="tab-widget"))

            tabs.append(
                dcc.Tab(label=tab_name, children=tab_content, className="tab-content")
            )
    ...
    def _create_widget_component(self, widget: DashboardWidget) -> html.Div:
        ...
        # Widget title
        if widget.title:
            widget_content.append(html.H3(widget.title, className="widget-title"))
            ...
            if filter_type == "dropdown":
                ...
                widget_content.append(
                    dcc.RangeSlider(
                        ...
                        marks={
                            i: str(i) for i in widget.data[:: len(widget.data) // 10]
                        },
                    )
                )
        elif widget.widget_type == WidgetType.TEXT:
            widget_content.append(html.Div(widget.data, className="text-widget"))
    ...
    def _create_plot_figure(self, widget: DashboardWidget) -> go.Figure:
        ...
        fig.update_layout(title=widget.title, showlegend=True, template="plotly_white")
