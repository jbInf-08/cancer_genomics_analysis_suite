...
class ManagerDashboard:
        ...
        return html.Div(
            [
                ...
                html.Div(
                    [
                        html.H1(
                            "Article Manager Dashboard", className="text-center mb-4"
                        ),
                        html.P(
                            "Comprehensive article management and analysis for cancer genomics research",
                        ...
                        html.Div(
                            [
                                html.Div(
                                    [
                                        html.H4(
                                            "Article Details", className="modal-title"
                                        ),
                                        html.Button(
                                            "×",
                                            className="btn-close",
                    ...
                    [
                        html.Div(
                            [
                                ...
                                html.Div(
                                    [html.Div(id="tag-content")], className="modal-body"
                                ),
                                html.Div(
                                    [
                                        html.Button(
    ...
    def create_analytics_tab(self) -> html.Div:
        ...
        return html.Div(
            [
                html.Div(
                    [html.H4("Article Analytics"), dcc.Graph(id="analytics-chart")],
                    className="card mb-4",
                ),
                html.Div(
                    [
    ...
    def setup_callbacks(self):
        ...
        def search_articles(
            ...
            if not ctx.triggered:
                return "", html.Div(
                    "Use the search form to find articles", className="text-muted"
                )
            ...
            try:
                if trigger_id == "load-mock-data":
                    ...
                    ), self.create_articles_display(articles)
                    ...
                    articles, total = self.db_manager.search_articles(
                        query=query or "", filters=filters, limit=100
                    )
                ...
                try:
                    ...
                    if stats["top_journals"]:
                        ...
                        journals_fig = go.Figure(
                            data=[go.Bar(x=journals, y=counts, name="Articles")]
                        )
                        journals_fig.update_layout(
                            title="Top Journals", xaxis_tickangle=-45
                        )
                    else:
                        journals_fig = go.Figure()
                    ...
                    if stats["articles_by_source"]:
                        ...
                        source_fig = px.pie(
                            values=counts, names=sources, title="Articles by Source"
                        )
                    else:
        ...
        def find_similar_articles(n_clicks, article_id):
            ...
            if n_clicks and article_id:
                try:
                    similar_articles = self.db_manager.get_article_similarity(
                        int(article_id), limit=10
                    )
                    ...
                    if similar_articles:
                        ...
                        for similar_id, similarity_score in similar_articles:
                            ...
                            if article:
                                results.append(
                                    {"article": article, "similarity": similarity_score}
                                )

                        return self.create_similarity_display(results)
                    else:
                        return html.Div(
                            "No similar articles found", className="text-muted"
                        )
            ...
            return html.Div(
                "Select an article and click 'Find Similar Articles'",
        ...
        def run_topic_modeling(n_clicks, n_topics):
            ...
            if n_clicks:
                try:
                    ...
                    if results:
                        ...
                        return html.Div(
                            "No articles available for topic modeling",
                            className="text-muted",
                        )
            ...
            return html.Div(
                "Click 'Run Topic Modeling' to analyze article topics",
                className="text-muted",
            )
                    ...
                    [
                        html.Div(
                            [
                                ...
                                html.P(
                                    f"Source: {article.source or 'N/A'}",
                                    className="card-text",
                                ),
                                html.P(
                                ...
                                html.Div(
                                    [
                                        ...
                                        html.Span(
                                            f"Status: {article.read_status}",
                                            className="badge bg-info me-2",
                                        ),
                                        html.Span(
                                            f"Citations: {article.citations or 'N/A'}",
                ...
                [
                    html.Div(
                        [
                            ...
                            html.P(
                                f"Similarity: {similarity:.3f}", className="card-text"
                            ),
                            html.P(
                                f"Journal: {article.journal or 'N/A'}",
                                className="card-text",
    ...
    def create_topic_modeling_display(self, results: Dict[str, Any]) -> html.Div:
        ...
        for topic in topics:
            ...
            for word, weight in zip(words, weights):
                ...
                word_elements.append(
                    html.Span(
                        word, style={"font-size": f"{size}px", "margin-right": "5px"}
                    )
                )
            ...
            card = html.Div(
                [
                    html.Div(
                        [
                            html.H6(
                                f"Topic {topic['topic_id'] + 1}", className="card-title"
                            ),
        ...
        return html.Div([html.H5("Identified Topics"), html.Div(topic_cards)])


def create_manager_dashboard(
    app: dash.Dash, db_path: str = "article_manager.db"
) -> ManagerDashboard:
    """
