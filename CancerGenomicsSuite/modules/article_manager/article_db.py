...
class ArticleDatabaseManager:
        ...
        with self.get_db_connection() as conn:
            try:
                ...
                for key, value in updates.items():
                    if key in ["authors", "keywords", "tags"] and isinstance(
                        value, list
                    ):
    ...
    def delete_article(self, article_id: int) -> bool:
        ...
        with self.get_db_connection() as conn:
            try:
                ...
                conn.execute(
                    "DELETE FROM citations WHERE citing_article_id = ? OR cited_article_id = ?",
    ...
    def search_articles(
        ...
        limit: int = 100,
        offset: int = 0,
    ) -> Tuple[List[ArticleMetadata], int]:
        """
    ...
    def create_tag(
        self, name: str, description: str = "", color: str = "#007bff"
    ) -> int:
        """
        Create a new tag.
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        with self.get_db_connection() as conn:
            ...
            cursor = conn.execute(
                'SELECT COUNT(*) FROM articles WHERE read_status = "read"'
            )
            ...
            # Articles by journal
            cursor = conn.execute(
                "SELECT journal, COUNT(*) FROM articles WHERE journal IS NOT NULL GROUP BY journal ORDER BY COUNT(*) DESC LIMIT 10"
            )
    ...
    def get_article_similarity(
        self, article_id: int, limit: int = 10
        ...
        try:
            ...
            with self.get_db_connection() as conn:
                cursor = conn.execute(
                    'SELECT id, title, abstract FROM articles WHERE abstract IS NOT NULL AND abstract != ""'
                )
    ...
    def export_articles(
        self, output_path: str, format: str = "csv", filters: Dict[str, Any] = None
    ) -> None:
        """
