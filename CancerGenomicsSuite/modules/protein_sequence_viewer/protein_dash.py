...
class ProteinDashboard:
        ...
        return html.Div(
            [
                ...
                html.Div(
                    [
                        html.H1(
                            "Protein Sequence Viewer", className="text-center mb-4"
                        ),
                        html.P(
                            "Comprehensive protein sequence analysis tools for cancer genomics research",
                        ...
                        html.Div(
                            [
                                ...
                                html.Div(
                                    [
                                        html.Label(
                                            "Sequence Name:", className="form-label"
                                        ),
                                        dcc.Input(
                                        ...
                                        html.Label(
                                            "Protein Sequence:", className="form-label"
                                        ),
                                        dcc.Textarea(
                                        ...
                                        html.Div(
                                            [
                                                html.Button(
                                                    "Analyze Protein",
                                                    id="analyze-protein-btn",
                                                    className="btn btn-primary me-2",
                                                ),
                                                html.Button(
                                                    "Clear",
                                                    id="clear-protein-btn",
                                            ...
                                            [
                                                dcc.Checklist(
                                                    ...
                                                    options=[
                                                        ...
                                                        {
                                                            "label": "Predict Secondary Structure",
                                                            "value": "secondary_structure",
                                                        },
                                                        {
                                                            "label": "Calculate Hydrophobicity",
                                                            "value": "hydrophobicity",
                                                        },
                                                        {
                                                            "label": "Find Disulfide Bonds",
                                                            "value": "disulfide_bonds",
                                                        },
                                                        {
                                                            "label": "Analyze Amino Acid Composition",
                                                            "value": "composition",
                                                        },
                                                    ],
                                                    value=[
    ...
    def _register_callbacks(self):
        ...
        def analyze_protein(n_clicks, sequence, name, options, hydrophobicity_window):
            ...
            if n_clicks == 0 or not sequence:
                return (
                    html.P("Enter a protein sequence and click 'Analyze Protein'"),
                    html.Div(),
                )
    ...
    def _create_results_display(self, results: Dict[str, Any]) -> html.Div:
        ...
        components.append(
            html.P(f"Length: {results.get('sequence_length', 0)} amino acids")
        )
            ...
            stats_data = [
                {
                    ...
                    "Value": f"{stats.get('molecular_weight', 0):.2f} Da",
                },
                {
                    "Metric": "Isoelectric Point",
                    "Value": f"{stats.get('isoelectric_point', 0):.2f}",
                },
                {
                    "Metric": "Aromaticity",
                    "Value": f"{stats.get('aromaticity', 0):.3f}",
                },
                {
                    "Metric": "Instability Index",
                    "Value": f"{stats.get('instability_index', 0):.2f}",
                },
                {"Metric": "GRAVY", "Value": f"{stats.get('gravy', 0):.3f}"},
                {
                    "Metric": "Net Charge (pH 7)",
                    "Value": f"{stats.get('charge_at_ph7', 0):.2f}",
                },
                {
                    "Metric": "Extinction Coefficient",
                    "Value": f"{stats.get('extinction_coefficient', 0):.0f} M⁻¹cm⁻¹",
                },
                {
                    "Metric": "Estimated Half-life",
                    "Value": stats.get("half_life", "Unknown"),
                },
            ]
        ...
        if "secondary_structure" in results:
            ...
            components.append(
                html.P(
                    f"Predicted Structure: {ss.get('predicted_structure', 'Unknown')}"
                )
            )
    ...
    def _create_visualizations(self, results: Dict[str, Any]) -> html.Div:
        ...
        if (
            "amino_acid_composition" in results
            and "amino_acid_percentages" in results["amino_acid_composition"]
        ):
            ...
            fig.update_layout(title="Amino Acid Composition", showlegend=True)
        ...
        if (
            "amino_acid_composition" in results
            and "property_percentages" in results["amino_acid_composition"]
        ):
