...
class GeneExpressionPlotter:
    ...
    def load_expression_data(
        ...
        try:
            ...
            if metadata is not None:
                if isinstance(metadata, str):
                    if metadata.endswith(".csv"):
                        ...
                        raise ValueError(
                            f"Unsupported metadata file format: {metadata}"
                        )
                else:
            ...
            self.logger.info(
                f"Loaded expression data: {self.expression_data.shape[0]} genes, {self.expression_data.shape[1]} samples"
            )
    ...
    def _validate_expression_data(self) -> Dict[str, Any]:
        ...
        # Check data types
        non_numeric_cols = self.expression_data.select_dtypes(
            exclude=[np.number]
        ).columns
        ...
        low_expression_genes = (
            self.expression_data.max(axis=1) < self.config.min_expression_threshold
        ).sum()
    ...
    def preprocess_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.config.filter_low_expression:
                ...
                self.logger.info(
                    f"Filtered {original_shape[0] - processed_data.shape[0]} low expression genes"
                )
    ...
    def analyze_expression(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if (
                self.config.perform_differential_expression
                and self.metadata is not None
            ):
                results[
                    "differential_expression"
                ] = self._perform_differential_expression()
    ...
    def _calculate_basic_statistics(self) -> Dict[str, Any]:
        ...
        stats_dict = {
            ...
            "zero_expression_percentage": float(
                (self.expression_data == 0).sum().sum()
                / self.expression_data.size
                * 100
            ),
        }
        ...
        gene_stats = {
            ...
            "cv": (
                self.expression_data.std(axis=1)
                / (self.expression_data.mean(axis=1) + 1e-10)
            ).to_dict(),
        }
        ...
        return {
            "gene_correlations": {
                ...
                "mean_correlation": float(
                    gene_corr.values[np.triu_indices_from(gene_corr.values, k=1)].mean()
                ),
            },
            "sample_correlations": {
                ...
                "mean_correlation": float(
                    sample_corr.values[
                        np.triu_indices_from(sample_corr.values, k=1)
                    ].mean()
                ),
            },
        }
        ...
        return {
            ...
            "sample_clusters": {
                ...
                for sample, cluster in zip(
                    self.expression_data.columns, sample_clusters
                )
            },
            "gene_clusters": {
                gene: int(cluster)
                for gene, cluster in zip(self.expression_data.index, gene_clusters)
            },
    ...
    def _perform_differential_expression(self) -> Dict[str, Any]:
        ...
        if self.metadata is None:
            return {
                "error": "No metadata available for differential expression analysis"
            }
        ...
        group1_samples = [
            s for s in group1_samples if s in self.expression_data.columns
        ]
        group2_samples = [
            s for s in group2_samples if s in self.expression_data.columns
        ]

        if len(group1_samples) == 0 or len(group2_samples) == 0:
            return {
                "error": "No samples found in expression data for differential expression"
            }
    ...
    def create_visualizations(self, output_dir: str = "outputs/plots") -> List[str]:
        ...
        try:
            ...
            if hasattr(self, "_pca_result"):
                pca_df = pd.DataFrame(
                    self._pca_result, index=self.expression_data.columns
                )
    ...
    def get_analysis_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_data_shape": self.expression_data.shape
