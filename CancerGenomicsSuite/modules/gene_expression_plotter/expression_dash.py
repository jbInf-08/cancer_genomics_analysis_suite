...
from .plotter import ExpressionAnalysisConfig, GeneExpressionPlotter
...
class ExpressionDashboard:
        ...
        return html.Div(
            [
                ...
                html.Div(
                    [
                        html.H1(
                            "Gene Expression Plotter", className="text-center mb-4"
                        ),
                        html.P(
                            "Comprehensive gene expression analysis tools for cancer genomics research",
                            ...
                            [
                                ...
                                html.Div(
                                    [
                                        ...
                                        html.Br(),
                                        html.Label(
                                            "Sample Metadata (Optional, CSV/TSV):",
                                            className="form-label",
                                        ),
                                        dcc.Upload(
                                            id="upload-metadata",
    ...
    def _register_callbacks(self):
        ...
        def handle_upload(
            ...
            if trigger_id == "upload-expression-data" and expression_contents:
                return self._process_expression_upload(
                    expression_contents, expression_filename
                )
            elif trigger_id == "upload-metadata" and metadata_contents:
                return self._process_metadata_upload(
        ...
        try:
            ...
            if filename.endswith(".csv"):
                ...
                return (
                    html.Div(
                        [
                            html.P(
                                "Unsupported file format. Please upload CSV or TSV files.",
            ...
            if load_results["success"]:
                ...
                return (
                    html.Div(
                        [
                            html.P(
                                f"Successfully loaded {load_results['genes']} genes and {load_results['samples']} samples",
                ...
                return (
                    html.Div(
                        [
                            html.P(
                                f"Error loading data: {load_results['error']}",
                                className="text-danger",
                            )
                        ]
                    ),
            ...
            return (
                html.Div(
                    [
                        html.P(
                            f"Error processing file: {str(e)}", className="text-danger"
                        )
                    ]
                ),
                html.Div(),
        ...
        try:
            ...
            return (
                html.Div(
                    [
                        html.P(
                            f"Successfully loaded metadata for {len(df)} samples",
                            className="text-success",
                        )
                    ]
                ),
            ...
            return (
                html.Div(
                    [
                        html.P(
                            f"Error processing metadata: {str(e)}",
                            className="text-danger",
                        )
                    ]
                ),
    ...
    def _create_data_preview(self, df: pd.DataFrame, title: str) -> html.Div:
        ...
        return html.Div(
            [
                ...
                html.P(
                    f"Showing first 10 rows and columns of {df.shape[0]} x {df.shape[1]} data"
                ),
            ]
        )
    ...
    def _create_results_display(self, results: Dict[str, Any]) -> html.Div:
            ...
            stats_data = [
                {
                    ...
                    "Value": f"{stats.get('mean_expression', 0):.3f}",
                },
                {
                    "Metric": "Median Expression",
                    "Value": f"{stats.get('median_expression', 0):.3f}",
                },
                {
                    "Metric": "Std Expression",
                    "Value": f"{stats.get('std_expression', 0):.3f}",
                },
                {
                    "Metric": "Min Expression",
                    "Value": f"{stats.get('min_expression', 0):.3f}",
                },
                {
                    "Metric": "Max Expression",
                    "Value": f"{stats.get('max_expression', 0):.3f}",
                },
                {
                    "Metric": "Zero Expression %",
                    "Value": f"{stats.get('zero_expression_percentage', 0):.1f}%",
                },
            ]
        ...
        if "pca" in results:
            ...
            components.append(
                html.P(
                    f"PC1 explains {pca['explained_variance_ratio'][0]*100:.1f}% of variance"
                )
            )
            components.append(
                html.P(
                    f"PC2 explains {pca['explained_variance_ratio'][1]*100:.1f}% of variance"
                )
            )
        ...
        if "clustering" in results:
            ...
            if clustering["method"] == "kmeans":
                components.append(
                    html.P(f"Number of clusters: {clustering['n_clusters']}")
                )
        ...
        if "differential_expression" in results:
            ...
            if "error" not in de:
                ...
                components.append(
                    html.P(
                        f"Significant genes: {de['significant_genes']} / {de['total_genes']}"
                    )
                )
    ...
    def _create_visualizations(self, results: Dict[str, Any]) -> html.Div:
        ...
        if "basic_statistics" in results and self.plotter.expression_data is not None:
            ...
            fig = go.Figure(
                data=[
                    go.Histogram(
                        x=sample_means, nbinsx=30, name="Sample Mean Expression"
                    )
                ]
            )
            fig.update_layout(
