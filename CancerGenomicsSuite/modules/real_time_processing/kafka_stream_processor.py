...
MUTATIONS_PROCESSED = Counter(
    "mutations_processed_total",
    "Total mutations processed",
    ["gene", "mutation_type", "severity"],
)
MUTATION_PROCESSING_DURATION = Histogram(
    "mutation_processing_duration_seconds", "Time spent processing mutations", ["gene"]
)
MUTATION_ALERTS = Counter(
    "mutation_alerts_total",
    "Total mutation alerts generated",
    ["severity", "gene", "mutation"],
)
MUTATIONS_BY_GENE = Gauge(
    "mutations_by_gene_total", "Total mutations by gene", ["gene"]
)
...
KAFKA_CONSUMER_LAG = Gauge(
    "kafka_consumer_lag_sum", "Kafka consumer lag", ["topic", "partition"]
)
STREAM_PROCESSING_RATE = Gauge(
    "stream_processing_rate", "Stream processing rate", ["processor_type"]
)
NEO4J_OPERATIONS = Counter(
    "neo4j_operations_total", "Total Neo4j operations", ["operation_type", "status"]
)
ML_PREDICTIONS = Counter(
    "ml_predictions_total", "Total ML predictions made", ["model_type", "prediction"]
)
...
class Neo4jGraphProcessor:
    ...
    async def create_mutation_node(self, mutation: MutationData) -> bool:
        ...
        try:
            ...
            NEO4J_OPERATIONS.labels(
                operation_type="create_mutation", status="error"
            ).inc()
            return False
    ...
    async def create_patient_node(self, patient_id: str, sample_id: str) -> bool:
        ...
        try:
            ...
            NEO4J_OPERATIONS.labels(
                operation_type="create_patient", status="error"
            ).inc()
            return False

    async def create_mutation_patient_relationship(
        self, mutation_id: str, patient_id: str
    ) -> bool:
        ...
        try:
            ...
            NEO4J_OPERATIONS.labels(
                operation_type="create_relationship", status="error"
            ).inc()
            return False

    async def find_similar_mutations(
        self, mutation: MutationData, threshold: float = 0.8
    ) -> List[Dict]:
        ...
        try:
            ...
            NEO4J_OPERATIONS.labels(
                operation_type="find_similar", status="success"
            ).inc()
    ...
    async def get_gene_network(self, gene: str) -> Dict:
        ...
        try:
            ...
            network = {"gene": gene, "interactions": []}
            ...
            NEO4J_OPERATIONS.labels(
                operation_type="get_network", status="success"
            ).inc()
            return network
...
class KafkaStreamProcessor:
    ...
    async def process_mutation(
        ...
        try:
            ...
            mutation = MutationData(
                ...
                clinical_significance=mutation_data.get(
            ...
            with self.neo4j_processor as neo4j:
                ...
                # Create relationship
                await neo4j.create_mutation_patient_relationship(
                    mutation.id, mutation.patient_id
                )
            ...
            if self._should_generate_alert(mutation, anomaly_result, similar_mutations):
                ...
                MUTATION_ALERTS.labels(
                    severity=alert.severity, gene=alert.gene, mutation=alert.mutation
                ).inc()

            # Record processing duration
            processing_time = time.time() - start_time
            MUTATION_PROCESSING_DURATION.labels(gene=mutation.gene).observe(
                processing_time
            )
    ...
    def _should_generate_alert(
        self, mutation: MutationData, anomaly_result: Dict, similar_mutations: List
    ) -> bool:
        ...
        if similar_mutations:
            high_severity_similar = any(
                sim["severity"] in ["critical", "high"] for sim in similar_mutations
            )
    ...
    def _determine_alert_severity(
        self, mutation: MutationData, anomaly_result: Dict
    ) -> str:
    ...
    def _generate_alert_message(
        ...
        if anomaly_result["is_anomaly"]:
            message += (
                f" (Anomaly detected, confidence: {anomaly_result['confidence']:.2f})"
            )

        if mutation.pathogenicity_score > 0.7:
            message += (
                f" (High pathogenicity score: {mutation.pathogenicity_score:.2f})"
            )
    ...
    async def send_alert(self, alert: AlertData):
        ...
        try:
            ...
            self.producer.send(self.topics["alerts"], value=alert_data)
            ...
            while self.running:
                ...
                for topic_partition, messages in message_batch.items():
                    ...
                    for message in messages:
                        try:
                            ...
                            STREAM_PROCESSING_RATE.labels(
                                processor_type="mutation"
                            ).inc()
                ...
                for topic_partition in self.consumer.assignment():
                    try:
                        ...
                        if committed:
                            high_water = self.consumer.get_watermark_offsets(
                                topic_partition
                            )[1]
