...
try:
    ...
    logging.warning(
        "Confluent Kafka client not available. Install confluent-kafka package."
    )
...
class KafkaManager:
    ...
    def __init__(
        ...
        if not KAFKA_AVAILABLE and not CONFLUENT_KAFKA_AVAILABLE:
            raise ImportError(
                "Kafka clients not available. Install kafka-python or confluent-kafka package."
            )
    ...
    def create_topic(
        ...
        try:
            if self.use_confluent:
                return self._create_topic_confluent(
                    topic_name, num_partitions, replication_factor, config
                )
            else:
                return self._create_topic_kafka_python(
                    topic_name, num_partitions, replication_factor, config
                )
    ...
    def _create_topic_kafka_python(
        self,
        topic_name: str,
        num_partitions: int,
        replication_factor: int,
    ...
    def produce_message(
        ...
        try:
            ...
            if self.use_confluent:
                return self._produce_message_confluent(
                    topic, message_with_metadata, key, partition
                )
            else:
                return self._produce_message_kafka_python(
                    topic, message_with_metadata, key, partition
                )
    ...
    def _produce_message_confluent(
        self, topic: str, message: Dict, key: Optional[str], partition: Optional[int]
    ) -> bool:
    ...
    def _produce_message_kafka_python(
        self, topic: str, message: Dict, key: Optional[str], partition: Optional[int]
    ) -> bool:
    ...
    def consume_messages(
        ...
        try:
            if self.use_confluent:
                return self._consume_messages_confluent(
                    topics, group_id, auto_offset_reset, max_messages, timeout_ms
                )
            else:
                return self._consume_messages_kafka_python(
                    topics, group_id, auto_offset_reset, max_messages, timeout_ms
                )
    ...
    def _consume_messages_confluent(
        self,
        topics: List[str],
    ...
    def _consume_messages_kafka_python(
        self,
        topics: List[str],
        group_id: Optional[str],
        auto_offset_reset: str,
                ...
                if topic_name in metadata.topics:
                    ...
                    return {
                        ...
                        "replication_factor": len(
                            topic_metadata.partitions[0].replicas
                        ),
                        "partition_details": {
                            str(pid): {
