...
class GalaxyClient:
    ...
    def __init__(
        self, base_url: str = "https://usegalaxy.org", api_key: Optional[str] = None
    ):
        """
        ...
        try:
            with open(file_path, "rb") as f:
                ...
                response = self.session.post(
                    f"{self.api_url}/tools/fetch", files=files, data=data
                )
                response.raise_for_status()
    ...
    def run_workflow(self, workflow_id: str, inputs: Dict[str, Any]) -> Optional[str]:
        ...
        try:
            ...
            response = self.session.post(
                f"{self.api_url}/workflows/{workflow_id}/invocations", json=payload
            )
            response.raise_for_status()
