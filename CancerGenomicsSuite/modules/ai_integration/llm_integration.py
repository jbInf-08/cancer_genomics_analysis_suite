...
class ScientificLiteratureProcessor:
    ...
    def build_literature_database(self, papers_dir: str) -> str:
        ...
        logger.info(
            f"Created vector database with {len(texts)} chunks from {len(documents)} papers"
        )
    ...
    def search_literature(self, query: str, k: int = 5) -> List[Dict[str, Any]]:
        ...
        if not self.vectorstore:
            logger.error(
                "Literature database not built. Call build_literature_database first."
            )
            return []
...
class GenomicQueryEngine:
    ...
    def setup_knowledge_base(self, documents: List[str], metadatas: List[Dict] = None):
        ...
        # Create vector store
        self.vectorstore = Chroma.from_texts(
            texts=documents, embedding=self.embeddings, metadatas=metadatas
        )
