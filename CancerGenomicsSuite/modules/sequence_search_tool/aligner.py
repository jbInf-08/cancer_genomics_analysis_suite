...
class SequenceAligner:
    ...
    def add_sequence_to_database(
        self, sequence_id: str, sequence: str, description: str = ""
    ) -> bool:
        """
        ...
        try:
            ...
            if not validation["valid"]:
                self.logger.error(
                    f"Invalid sequence for {sequence_id}: {validation['errors']}"
                )
                return False
            ...
            self.sequence_database[sequence_id] = {
                ...
                "gc_content": gc_fraction(clean_sequence, ambiguous="ignore") * 100
                if self._detect_sequence_type(clean_sequence) == "DNA"
                else None,
            }

            self.logger.info(
                f"Added sequence {sequence_id} to database ({len(clean_sequence)} bp)"
            )
            return True
    ...
    def _validate_sequence(self, sequence: str) -> Dict[str, Any]:
        ...
        if dna_chars and protein_chars:
            errors.append(
                f"Invalid characters found: {', '.join(dna_chars | protein_chars)}"
            )
    ...
    def search_sequence(
        self, query_sequence: str, query_name: str = "Query"
    ) -> Dict[str, Any]:
        """
    ...
    def _align_sequences(
        ...
        try:
            ...
            if (
                len(aligned_query) < self.config.min_alignment_length
                or identity < self.config.identity_threshold
            ):
                return {
                    "target_id": target_id,
                    "target_description": target_desc,
    ...
    def search_pattern(
        self, pattern: str, pattern_name: str = "Pattern"
    ) -> Dict[str, Any]:
        """
        ...
        self.logger.info(
            f"Found {len(results['matches'])} pattern matches for {pattern_name}"
        )
        return results
    ...
    def get_database_statistics(self) -> Dict[str, Any]:
        ...
        total_length = sum(
            seq_data["length"] for seq_data in self.sequence_database.values()
        )
