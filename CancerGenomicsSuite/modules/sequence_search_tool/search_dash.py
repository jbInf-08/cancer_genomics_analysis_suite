...
from .aligner import AlignmentConfig, SequenceAligner
...
class SearchDashboard:
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        html.P(
                            "Comprehensive sequence search and alignment tools for cancer genomics research",
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            [
                                ...
                                html.Div(
                                    [
                                        html.Label(
                                            "Upload Sequence File (FASTA/GenBank):",
                        ...
                        html.Div(
                            [
                                ...
                                html.Div(
                                    [
                                        html.Label(
                                            "Query Sequence:", className="form-label"
                                        ),
                                        dcc.Textarea(
                                            id="query-sequence",
                        ...
                        # Search results
                        html.Div(
                            [html.H3("Search Results"), html.Div(id="search-results")],
                            className="card mb-4",
                        ),
                        # Visualizations
                        html.Div(
    ...
    def _register_callbacks(self):
        ...
        def update_database_stats(upload_contents, add_clicks, clear_clicks):
            ...
            stats_display = [
                ...
                html.P(
                    f"Sequence Types: {', '.join(f'{k}: {v}' for k, v in stats['sequence_types'].items())}"
                ),
            ]
        ...
        def handle_sequence_upload(contents, filename):
            ...
            if contents:
                try:
                    ...
                    if result["success"]:
                        self.logger.info(
                            f"Loaded {result['loaded_sequences']} sequences"
                        )
                    else:
        ...
        def perform_search(
            ...
            if not ctx.triggered:
                return (
                    html.P("Configure search parameters and click search"),
                    html.Div(),
                )
            ...
            try:
                if trigger_id == "search-sequence-btn" and query_seq:
                    ...
                    return (
                        html.P("Enter query sequence or pattern and click search"),
                        html.Div(),
                    )
        ...
        @self.app.callback(
            [Output("query-sequence", "value"), Output("search-pattern", "value")],
                ...
                matches_data.append(
                    {
                        ...
                        "Description": match["target_description"][:50] + "..."
                        if len(match["target_description"]) > 50
                        else match["target_description"],
                ...
                matches_data.append(
                    {
                        ...
                        "Description": match["sequence_description"][:50] + "..."
                        if len(match["sequence_description"]) > 50
                        else match["sequence_description"],
    ...
    def _create_sequence_visualizations(self, results: Dict[str, Any]) -> html.Div:
        ...
        match_names = [
            match["target_id"][:20] + "..."
            if len(match["target_id"]) > 20
            else match["target_id"]
            for match in top_matches
        ]
        match_scores = [match["score"] for match in top_matches]

        fig2 = go.Figure(
            data=[go.Bar(x=match_names, y=match_scores, marker_color="lightblue")]
        )
        fig2.update_layout(
    ...
    def _create_pattern_visualizations(self, results: Dict[str, Any]) -> html.Div:
        ...
        if sequence_counts:
            fig = go.Figure(
                data=[
                    go.Bar(
                        x=list(sequence_counts.keys()),
                        y=list(sequence_counts.values()),
