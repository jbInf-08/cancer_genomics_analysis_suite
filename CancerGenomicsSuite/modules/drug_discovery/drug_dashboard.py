...
from .drug_analyzer import DrugAnalyzer, DrugDiscoveryConfig, DrugResult
...
class DrugDiscoveryDashboard:
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        html.P(
                            "Discover and analyze drugs for cancer treatment",
                ...
                html.Div(
                    [
                        html.Div(
                            [
                                html.H3(
                                    "Analysis Configuration", style={"color": "#2c3e50"}
                                ),
                                # File Upload
                                dcc.Upload(
                                    id="upload-data",
                        ...
                        html.Div(
                            [
                                html.H3(
                                    "Drug Discovery Results", style={"color": "#2c3e50"}
                                ),
                                # Summary Statistics
                                html.Div(
                                    id="summary-stats", style={"marginBottom": "20px"}
                                ),
                                # Results Table
                ...
                html.Div(
                    [
                        html.H3(
                            "Drug Analysis Visualizations",
                            style={"color": "#2c3e50", "textAlign": "center"},
                        ),
                        # Visualization Tabs
                        dcc.Tabs(
                            id="viz-tabs",
                            value="efficacy-safety",
                            children=[
                                dcc.Tab(
                                    label="Efficacy vs Safety", value="efficacy-safety"
                                ),
                                dcc.Tab(label="Drug Scores", value="drug-scores"),
                                dcc.Tab(
                                    label="Target Analysis", value="target-analysis"
                                ),
                                dcc.Tab(
                                    label="Repurposing Potential", value="repurposing"
                                ),
    ...
    def _setup_callbacks(self):
        ...
        @self.app.callback(
            [Output("summary-stats", "children"), Output("results-table", "children")],
        ...
        def run_analysis(
            ...
            try:
                ...
                if genomic_data is None or drug_data is None:
                    return (
                        html.Div(
                            "Error: Could not parse uploaded data",
                            style={"color": "red"},
                        ),
                        "",
                    )
                ...
                # Run analysis
                results = self.analyzer.analyze_drugs(
                    genomic_data, drug_data, analysis_type
                )
        ...
        @self.app.callback(
            Output("viz-content", "children"), [Input("viz-tabs", "value")]
        )
        ...
        @self.app.callback(
            Output("export-csv", "n_clicks"), [Input("export-csv", "n_clicks")]
        )
        ...
        @self.app.callback(
            Output("export-excel", "n_clicks"), [Input("export-excel", "n_clicks")]
        )
            ...
            drug_data = pd.DataFrame(
                {
                    ...
                    "type": np.random.choice(
                        ["targeted", "chemotherapy", "immunotherapy"], n_drugs
                    ),
    ...
    def _generate_summary_stats(self, results: List[DrugResult]) -> html.Div:
        ...
        repurposing_candidates = len(
            [r for r in results if r.repurposing_potential > 0.8]
        )
        ...
        stats_cards = [
            html.Div(
                [
                    html.H4(
                        str(total_drugs), style={"color": "#3498db", "margin": "0"}
                    ),
            ...
            html.Div(
                [
                    html.H4(
                        str(high_efficacy), style={"color": "#e74c3c", "margin": "0"}
                    ),
            ...
            html.Div(
                [
                    html.H4(
                        str(high_safety), style={"color": "#f39c12", "margin": "0"}
                    ),
        ...
        return html.Div(
            stats_cards, style={"display": "flex", "justifyContent": "space-around"}
        )
            ...
            table_data.append(
                {
                    ...
                    "Targets": "; ".join(
                        result.target_genes[:3]
                    ),  # Show first 3 targets
                    "Mechanism": result.mechanism_of_action[:50] + "..."
                    if len(result.mechanism_of_action) > 50
                    else result.mechanism_of_action,
                }
            )
    ...
    def _create_drug_scores_plot(self) -> dcc.Graph:
        ...
        fig = go.Figure(
            data=[
                go.Bar(
                    name="Efficacy", x=drug_names, y=efficacy_scores, marker_color="red"
                ),
                go.Bar(
                    name="Safety", x=drug_names, y=safety_scores, marker_color="green"
                ),
                go.Bar(
                    name="Repurposing",
                    x=drug_names,
    ...
    def _create_target_analysis_plot(self) -> dcc.Graph:
        ...
        fig = go.Figure(
            data=go.Bar(x=target_names, y=target_counts_list, marker_color="lightblue")
        )
    ...
    def _create_repurposing_plot(self) -> dcc.Graph:
        ...
        fig = go.Figure(
            data=go.Bar(x=drug_names, y=repurposing_scores, marker_color="green")
        )
        ...
        for node in nodes:
            fig.add_trace(
                go.Scatter(
                    ...
                    y=[0.5],
                    mode="markers+text",
                    marker=dict(size=node["size"], color=node["color"], opacity=0.7),
                    text=node["label"],
...
class DrugVisualizationEngine:
    ...
    def create_drug_scores_plot(
        self, results: List[DrugResult], top_n: int = 10
    ) -> go.Figure:
        """Create drug scores bar plot."""
        if not results:
            return go.Figure()
