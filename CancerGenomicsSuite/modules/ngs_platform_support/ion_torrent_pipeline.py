...
class IonTorrentPipeline:
    ...
    def run_full_pipeline(self, input_files: Dict[str, str], 
        ...
        try:
            ...
            if self.config.get("homopolymer_correction", True):
                ...
                corrected_files = self._run_homopolymer_correction(preprocessed_files, output_dir)
            ...
            logger.info(f"Ion Torrent pipeline completed successfully for sample {sample_id}")
            return results
    ...
    def _run_homopolymer_correction(self, input_files: Dict[str, str], 
                                   output_dir: Path) -> Dict[str, str]:
        ...
        for file_type, file_path in input_files.items():
            if file_path and os.path.exists(file_path):
                output_file = output_dir / f"{Path(file_path).stem}_homopolymer_corrected.fastq.gz"
                ...
                try:
                    ...
                    logger.error(f"Error correcting homopolymers in {file_path}: {str(e)}")
                    corrected_files[file_type] = file_path  # Use original file if correction fails
...
class IonTorrentQualityControl:
    ...
    def run_quality_control(self, input_files: Dict[str, str], 
        ...
        recommendations = self._generate_recommendations(quality_metrics, homopolymer_analysis)
        ...
        for file_type, file_path in input_files.items():
            if file_path and os.path.exists(file_path):
                try:
                    ...
                    logger.error(f"Failed to analyze homopolymers in {file_path}: {str(e)}")
    ...
    def _analyze_homopolymer_patterns(self, file_path: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "average_homopolymer_length": np.mean(homopolymer_lengths) if homopolymer_lengths else 0,
                "max_homopolymer_length": np.max(homopolymer_lengths) if homopolymer_lengths else 0,
    ...
    def _generate_recommendations(self, quality_metrics: Dict[str, Any], 
                                 homopolymer_analysis: Dict[str, Any]) -> List[str]:
        ...
        for file_type, analysis in homopolymer_analysis.items():
            ...
            if analysis.get("long_homopolymers", 0) > 100:
                recommendations.append(f"Many long homopolymers detected in {file_type}. Consider homopolymer correction.")
            
            if analysis.get("average_homopolymer_length", 0) > 5:
                recommendations.append(f"Long average homopolymer length ({analysis['average_homopolymer_length']:.1f}) in {file_type}. Homopolymer correction recommended.")
        ...
        for file_type, file_path in input_files.items():
            if file_path and os.path.exists(file_path):
                ...
                try:
                    ...
                    cmd = [
                        self.tools["cutadapt"],
                        "-a", "ATCTCGTATGCCGTCTTCTGCTTG",  # Ion Torrent adapter
                        "-o", str(output_file),
...
class IonTorrentAlignment:
    ...
    def _run_tmap_alignment(self, input_files: Dict[str, str], 
                           output_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [
                ...
                "mapall",
                "-f", reference_genome,
                "-r", input_files.get("fastq", input_files.get("r1", "")),
                "-n", str(self.config.get("threads", 4)),
                "-o", "2",  # Output format: SAM
                "-s"  # Sort output
            ]
    ...
    def _run_bowtie2_alignment(self, input_files: Dict[str, str], 
        ...
        try:
            ...
            cmd = [
                self.tools["bowtie2"],
                "-x", reference_genome,
                "-U", input_files.get("fastq", input_files.get("r1", "")),
...
class IonTorrentVariantCalling:
    ...
    def _run_tvc_variant_calling(self, bam_file: str, output_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [
                ...
                "--output-vcf", str(vcf_file),
                "--num-threads", str(self.config.get("threads", 4))
            ]
