"""
Ion Torrent Sequencing Pipeline

This module provides comprehensive support for Ion Torrent sequencing platforms,
...
try:
    ...
    logging.warning(
        "Biopython not available. Some sequence operations will be limited."
    )
...
class IonTorrentPipeline:
    """
    Main Ion Torrent sequencing pipeline orchestrator.
    """

    def __init__(self, config: Dict[str, Any] = None):
        """
        Initialize Ion Torrent pipeline.
        ...
        self.quality_control = IonTorrentQualityControl(self.config)
        self.alignment = IonTorrentAlignment(self.config)
        self.variant_calling = IonTorrentVariantCalling(self.config)
        self.expression_analysis = IonTorrentExpressionAnalysis(self.config)
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "read_type": "single_end",  # single_end, paired_end
            "read_length": 200,
            "quality_threshold": 15,  # Lower threshold for Ion Torrent
            "adapter_trimming": True,
            "quality_trimming": True,
            "duplicate_removal": True,
            "homopolymer_correction": True,  # Specific to Ion Torrent
            "alignment_tool": "tmap",  # tmap, bwa, bowtie2
            "variant_caller": "tvc",  # tvc, gatk, freebayes
            "expression_tool": "salmon",  # salmon, kallisto
            "reference_genome": None,
            "annotation_file": None,
            "output_dir": "./ion_torrent_output",
            "threads": 4,
            "memory": "8G",
            "temp_dir": "./temp",
        }
    ...
    def _get_platform_info(self) -> Dict[str, Any]:
        """Get Ion Torrent platform-specific information."""
        return {
            "supported_platforms": ["pgm", "proton", "s5", "genexus"],
            "read_lengths": {
                "pgm": [100, 200, 400],
                "proton": [100, 200, 400],
                "s5": [100, 200, 400, 600],
                "genexus": [100, 200, 400, 600],
            },
            "throughput": {
                "pgm": "low",
                "proton": "medium",
            ...
            "error_rates": {
                "pgm": 1.0,  # Higher error rate due to homopolymer issues
                "proton": 1.0,
                "s5": 0.8,
                "genexus": 0.5,
            },
            "homopolymer_error_rates": {
                "pgm": 2.0,
                "proton": 2.0,
                "s5": 1.5,
                "genexus": 1.0,
    ...
    def run_full_pipeline(
        self, input_files: Dict[str, str], sample_id: str
    ) -> Dict[str, Any]:
        """
        Run the complete Ion Torrent sequencing pipeline.

        Args:
            input_files: Dictionary with 'fastq' file path
        ...
        """
        try:
            logger.info(f"Starting Ion Torrent pipeline for sample {sample_id}")
            ...
            qc_results = self.quality_control.run_quality_control(
            ...
            # Step 3: Homopolymer Correction (Ion Torrent specific)
            if self.config.get("homopolymer_correction", True):
                self.current_step = "homopolymer_correction"
                logger.info("Running homopolymer correction...")
                corrected_files = self._run_homopolymer_correction(
                    preprocessed_files, output_dir
                )
                results["steps"]["homopolymer_correction"] = corrected_files
                preprocessed_files = corrected_files

            # Step 4: Alignment
            self.current_step = "alignment"
            logger.info("Running alignment...")
            alignment_results = self.alignment.run_alignment(
                preprocessed_files, output_dir
            )
            ...
            # Step 6: Expression Analysis (if RNA-seq)
            ...
            logger.info(
                f"Ion Torrent pipeline completed successfully for sample {sample_id}"
            )
            return results
    ...
    def _run_preprocessing(
        self, input_files: Dict[str, str], output_dir: Path
    ) -> Dict[str, str]:
    ...
    def _run_homopolymer_correction(
        self, input_files: Dict[str, str], output_dir: Path
    ) -> Dict[str, str]:
        """Run homopolymer correction (Ion Torrent specific)."""
        corrected_files = {}
        ...
        for file_type, file_path in input_files.items():
            if file_path and os.path.exists(file_path):
                output_file = (
                    output_dir
                    / f"{Path(file_path).stem}_homopolymer_corrected.fastq.gz"
                )

                try:
                    # Simple homopolymer correction algorithm
                    corrected_file = self._correct_homopolymers(file_path, output_file)
                    corrected_files[file_type] = str(corrected_file)

                except Exception as e:
                    logger.error(
                        f"Error correcting homopolymers in {file_path}: {str(e)}"
                    )
                    corrected_files[
                        file_type
                    ] = file_path  # Use original file if correction fails

        return corrected_files

    def _correct_homopolymers(self, input_file: str, output_file: Path) -> Path:
        """Correct homopolymer errors in Ion Torrent reads."""
        if not BIOPYTHON_AVAILABLE:
            logger.warning("Biopython not available. Skipping homopolymer correction.")
        ...
        try:
            ...
            with open(input_file, "r") as handle:
                for record in SeqIO.parse(handle, "fastq"):
                    # Simple homopolymer correction: limit homopolymer runs to 4 bases
                    corrected_seq = self._limit_homopolymer_runs(str(record.seq))

                    # Create new record with corrected sequence
            ...
            with open(output_file, "w") as handle:
                SeqIO.write(corrected_records, handle, "fastq")

            return output_file

        except Exception as e:
            logger.error(f"Homopolymer correction failed: {str(e)}")
            return Path(input_file)

    def _limit_homopolymer_runs(self, sequence: str, max_run: int = 4) -> str:
        """Limit homopolymer runs to maximum length."""
        if not sequence:
            return sequence
        ...
        return sequence[0] + "".join(corrected)


class IonTorrentQualityControl:
    """
    Quality control utilities for Ion Torrent sequencing data.
    """
    ...
    def run_quality_control(
        ...
        """
        Run comprehensive quality control analysis for Ion Torrent data.
        ...
        # Analyze homopolymer patterns (Ion Torrent specific)
        homopolymer_analysis = self._analyze_homopolymers(input_files)
        qc_results["homopolymer_analysis"] = homopolymer_analysis

        # Generate recommendations
        recommendations = self._generate_recommendations(
            quality_metrics, homopolymer_analysis
        )
    ...
    def _run_fastqc(self, input_file: str, output_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {"input_file": input_file, "status": "failed", "error": str(e)}
    ...
    def _analyze_homopolymers(self, input_files: Dict[str, str]) -> Dict[str, Any]:
        """Analyze homopolymer patterns in Ion Torrent data."""
        homopolymer_analysis = {}

        for file_type, file_path in input_files.items():
            if file_path and os.path.exists(file_path):
                try:
                    analysis = self._analyze_homopolymer_patterns(file_path)
                    homopolymer_analysis[file_type] = analysis
                except Exception as e:
                    logger.error(
                        f"Failed to analyze homopolymers in {file_path}: {str(e)}"
                    )
                    homopolymer_analysis[file_type] = {"error": str(e)}

        return homopolymer_analysis

    def _analyze_homopolymer_patterns(self, file_path: str) -> Dict[str, Any]:
        """Analyze homopolymer patterns in a single file."""
        if not BIOPYTHON_AVAILABLE:
            return {"error": "Biopython not available"}

        try:
            homopolymer_lengths = []
            homopolymer_counts = {"A": 0, "T": 0, "G": 0, "C": 0}
            ...
            with open(file_path, "r") as handle:
                for record in SeqIO.parse(handle, "fastq"):
                    ...
                    current_base = seq[0] if seq else None
                    run_length = 1
                    ...
                    for i in range(1, len(seq)):
                        if seq[i] == current_base:
                            ...
                            if run_length >= 2:  # Homopolymer run
                                homopolymer_lengths.append(run_length)
                                if current_base in homopolymer_counts:
                                    homopolymer_counts[current_base] += 1
                                total_homopolymers += 1
                            current_base = seq[i]
                            run_length = 1
            ...
            return {
                "total_homopolymers": total_homopolymers,
                "average_homopolymer_length": np.mean(homopolymer_lengths)
                if homopolymer_lengths
                else 0,
                "max_homopolymer_length": np.max(homopolymer_lengths)
                if homopolymer_lengths
                else 0,
                "homopolymer_counts": homopolymer_counts,
                "long_homopolymers": len([x for x in homopolymer_lengths if x > 4]),
            }
    ...
    def _generate_recommendations(
        self, quality_metrics: Dict[str, Any], homopolymer_analysis: Dict[str, Any]
    ) -> List[str]:
        """Generate preprocessing recommendations based on quality metrics and homopolymer analysis."""
        recommendations = []
        ...
        for file_type, metrics in quality_metrics.items():
            ...
            if metrics.get("average_quality", 0) < 15:
                recommendations.append(
                    f"Low average quality ({metrics['average_quality']:.1f}) in {file_type}. Consider quality trimming."
                )

            if metrics.get("min_quality", 0) < 5:
                recommendations.append(
                    f"Very low minimum quality ({metrics['min_quality']}) in {file_type}. Aggressive quality trimming recommended."
                )

        # Homopolymer-specific recommendations
        for file_type, analysis in homopolymer_analysis.items():
            if "error" in analysis:
                continue

            if analysis.get("long_homopolymers", 0) > 100:
                recommendations.append(
                    f"Many long homopolymers detected in {file_type}. Consider homopolymer correction."
                )

            if analysis.get("average_homopolymer_length", 0) > 5:
                recommendations.append(
                    f"Long average homopolymer length ({analysis['average_homopolymer_length']:.1f}) in {file_type}. Homopolymer correction recommended."
                )

        return list(set(recommendations))  # Remove duplicates

    def trim_quality(
        self, input_files: Dict[str, str], output_dir: Path, quality_threshold: int
    ) -> Dict[str, str]:
    ...
    def trim_adapters(
        ...
        for file_type, file_path in input_files.items():
            if file_path and os.path.exists(file_path):
                output_file = (
                    output_dir / f"{Path(file_path).stem}_adapter_trimmed.fastq.gz"
                )
                ...
                try:
                    ...
                    cmd = [
                        self.tools["cutadapt"],
                        "-a",
                        "ATCTCGTATGCCGTCTTCTGCTTG",  # Ion Torrent adapter
                        "-o",
                        str(output_file),
                        "--threads",
                        str(self.config.get("threads", 4)),
...
class IonTorrentAlignment:
    """
    Alignment utilities for Ion Torrent sequencing data.
    """
    ...
    def run_alignment(
        ...
        Returns:
            Alignment results
        """
        alignment_tool = self.config.get("alignment_tool", "tmap")

        if alignment_tool == "tmap":
            return self._run_tmap_alignment(input_files, output_dir)
        elif alignment_tool == "bwa":
    ...
    def _run_tmap_alignment(
        self, input_files: Dict[str, str], output_dir: Path
    ) -> Dict[str, Any]:
        """Run TMAP alignment (Ion Torrent specific)."""
        try:
            reference_genome = self.config.get("reference_genome")
            if not reference_genome:
                raise ValueError("Reference genome path not specified")

            # TMAP index if needed
            self._ensure_tmap_index(reference_genome)
            ...
            cmd = [
                ...
                reference_genome,
                "-r",
                input_files.get("fastq", input_files.get("r1", "")),
                "-n",
                str(self.config.get("threads", 4)),
                "-o",
                "2",  # Output format: SAM
            ...
            if result.returncode != 0:
                raise RuntimeError(f"TMAP alignment failed: {result.stderr}")
            ...
            logger.error(f"TMAP alignment failed: {str(e)}")
            return {"status": "failed", "error": str(e)}

    def _run_bwa_alignment(
        self, input_files: Dict[str, str], output_dir: Path
    ) -> Dict[str, Any]:
        """Run BWA alignment."""
        try:
    ...
    def _ensure_tmap_index(self, reference_genome: str):
        """Ensure TMAP index exists."""
        index_files = [f"{reference_genome}.tmap.index"]

        if not all(os.path.exists(f) for f in index_files):
            logger.info(f"Building TMAP index for {reference_genome}")
            cmd = [self.tools["tmap"], "index", "-f", reference_genome]
            result = subprocess.run(cmd, capture_output=True, text=True)
            if result.returncode != 0:
                raise RuntimeError(f"TMAP indexing failed: {result.stderr}")
    ...
    def _ensure_bowtie2_index(self, reference_genome: str):
        ...
        index_files = [
            f"{reference_genome}.1.bt2",
            f"{reference_genome}.2.bt2",
            f"{reference_genome}.3.bt2",
            f"{reference_genome}.4.bt2",
            f"{reference_genome}.rev.1.bt2",
            f"{reference_genome}.rev.2.bt2",
        ]
...
class IonTorrentVariantCalling:
    """
    Variant calling utilities for Ion Torrent sequencing data.
    """
    ...
    def run_variant_calling(self, bam_file: str, output_dir: Path) -> Dict[str, Any]:
        ...
        """
        variant_caller = self.config.get("variant_caller", "tvc")

        if variant_caller == "tvc":
            return self._run_tvc_variant_calling(bam_file, output_dir)
    ...
    def _run_tvc_variant_calling(
        self, bam_file: str, output_dir: Path
    ) -> Dict[str, Any]:
        """Run TVC variant calling (Ion Torrent specific)."""
        try:
            reference_genome = self.config.get("reference_genome")
            if not reference_genome:
                raise ValueError("Reference genome path not specified")
            ...
            if result.returncode != 0:
                raise RuntimeError(f"TVC variant calling failed: {result.stderr}")
            ...
            logger.error(f"TVC variant calling failed: {str(e)}")
            return {"status": "failed", "error": str(e)}

    def _run_gatk_variant_calling(
        self, bam_file: str, output_dir: Path
    ) -> Dict[str, Any]:
    ...
    def _run_freebayes_variant_calling(
        self, bam_file: str, output_dir: Path
    ) -> Dict[str, Any]:
    ...
    def _run_bcftools_variant_calling(
        self, bam_file: str, output_dir: Path
    ) -> Dict[str, Any]:
...
class IonTorrentExpressionAnalysis:
    """
    Expression analysis utilities for Ion Torrent RNA-seq data.
    """

    def __init__(self, config: Dict[str, Any]):
        self.config = config
        self.tools = {"salmon": "salmon", "kallisto": "kallisto"}
    ...
    def _parse_salmon_results(self, output_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "top_transcripts": df.nlargest(10, "TPM")[["Name", "TPM"]].to_dict(
                    "records"
                ),
            }
    ...
    def _parse_kallisto_results(self, output_dir: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "mean_tpm": df["tpm"].mean(),
                "median_tpm": df["tpm"].median(),
                "top_transcripts": df.nlargest(10, "tpm")[["target_id", "tpm"]].to_dict(
                    "records"
                ),
            }
