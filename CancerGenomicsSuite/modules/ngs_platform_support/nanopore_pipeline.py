    ...
    def run_full_pipeline(
        ...
        try:
            ...
            if "fast5" in input_files and self.config.get("basecalling", True):
                ...
                basecalling_results = self.basecalling.run_basecalling(
                    input_files["fast5"], output_dir
                )
            ...
            logger.info(
                f"Nanopore pipeline completed successfully for sample {sample_id}"
            )
            return results
...
class NanoporeBasecalling:
    ...
    def _run_guppy_basecalling(
        self, fast5_dir: str, output_dir: Path
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [
                ...
                fast5_dir,
                "-s",
                str(basecalling_output),
                "--config",
                "dna_r9.4.1_450bps_hac.cfg",  # Default config
                "--num_callers",
        ...
        for file_type, metrics in quality_metrics.items():
            ...
            if metrics.get("average_quality", 0) < 7:
                recommendations.append(
                    f"Low average quality ({metrics['average_quality']:.1f}) in {file_type}. Consider quality filtering."
                )
...
class NanoporeVariantCalling:
    ...
    def _run_clair3_variant_calling(
    ...
    def _run_medaka_variant_calling(
        self, bam_file: str, output_dir: Path
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = [
                ...
                "variant",
                "--model",
                "r941_min_high_g360",  # Default model
                "--threads",
                str(self.config.get("threads", 8)),
                reference_genome,
                bam_file,
