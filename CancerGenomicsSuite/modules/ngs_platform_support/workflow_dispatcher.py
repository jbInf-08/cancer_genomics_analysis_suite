...
class WorkflowDispatcher:
    ...
    def submit_job(
        ...
        try:
            ...
            # Validate job
            validation_result = self.validator.validate_job(
                sample_id, input_files, platform, config
            )
            if not validation_result["valid"]:
                raise ValueError(
                    f"Job validation failed: {validation_result['errors']}"
                )
    ...
    def _get_job_progress(self, job: Job) -> Dict[str, Any]:
        ...
        return {
            "started_at": job.started_at.isoformat() if job.started_at else None,
            "elapsed_time": str(datetime.now() - job.started_at)
            if job.started_at
            else None,
    ...
    def get_resource_recommendations(self) -> List[str]:
        ...
        if usage.get("memory_percent", 0) > 90:
            recommendations.append(
                "High memory usage detected. Consider reducing concurrent jobs."
            )
...
class WorkflowScheduler:
    ...
    def schedule_job(
        self, job_id: str, schedule_time: datetime, job_data: Dict[str, Any]
    ) -> bool:
    ...
    def get_due_jobs(self) -> List[Dict[str, Any]]:
        ...
        for job_id, job_info in self.scheduled_jobs.items():
            if (
                job_info["schedule_time"] <= current_time
                and job_info["status"] == "scheduled"
            ):
                due_jobs.append({"job_id": job_id, "job_data": job_info["job_data"]})
                job_info["status"] = "due"
...
class PlatformDetector:
    ...
    def __init__(self):
        self.platform_indicators = {
            ...
            PlatformType.PACBIO: ["pacbio", "ccs", "subreads", "hifi", "sequel", "rs"],
...
class WorkflowValidator:
    ...
    def _is_valid_file_extension(self, file_path: str) -> bool:
        ...
        return any(
            file_path.lower().endswith(ext) for ext in self.required_file_extensions
        )

    def _validate_platform_requirements(
        self, platform: PlatformType, input_files: Dict[str, str]
    ) -> List[str]:
        """Validate platform-specific requirements."""
        errors = []
        ...
        if platform == PlatformType.ILLUMINA:
            ...
            if not any(
                "r1" in key.lower() or "r2" in key.lower() for key in input_files.keys()
            ):
            ...
            if not any(
                "subreads" in key.lower() or "ccs" in key.lower()
                for key in input_files.keys()
            ):
                errors.append(
                    "PacBio platform typically requires subreads or CCS files"
                )

        elif platform == PlatformType.NANOPORE:
            # Nanopore can have FAST5 or FASTQ files
            if not any(
                "fast5" in key.lower() or "fastq" in key.lower()
                for key in input_files.keys()
            ):
                errors.append(
                    "Nanopore platform typically requires FAST5 or FASTQ files"
                )
    ...
    def _validate_configuration(self, config: Dict[str, Any]) -> List[str]:
        ...
        if "memory" in config:
            if not isinstance(config["memory"], str) or not config["memory"].endswith(
                "G"
            ):
                errors.append(
                    "Memory must be specified as a string ending with 'G' (e.g., '8G')"
                )
    ...
    def validate_workflow_config(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "max_memory_gb" in config:
            if (
                not isinstance(config["max_memory_gb"], (int, float))
                or config["max_memory_gb"] <= 0
            ):
                errors.append("max_memory_gb must be a positive number")

        if "max_cpu_percent" in config:
            if not isinstance(config["max_cpu_percent"], (int, float)) or not (
                0 < config["max_cpu_percent"] <= 100
            ):
...
class DockerManager:
    ...
    def create_container(
        self, job: Job, pipeline_config: Dict[str, Any]
    ) -> Optional[str]:
    ...
    def _register_tasks(self, celery_app: Celery):
        ...
        def run_pipeline_task(self, job_data: Dict[str, Any]):
            ...
            try:
                ...
                logger.error(
                    f"Celery task failed for job {job_data.get('job_id', 'unknown')}: {e}"
                )
                raise
    ...
    def get_queue_info(self) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "active_tasks": len(active_tasks.get("celery@worker", []))
                if active_tasks
                else 0,
                "scheduled_tasks": len(scheduled_tasks.get("celery@worker", []))
                if scheduled_tasks
                else 0,
            }
