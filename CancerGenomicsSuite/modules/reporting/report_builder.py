...
*Report generated by Cancer Genomics Analysis Suite*
    ...
    def add_text_section(
        self, report_id: str, title: str, content: str, order: int = 0
    ):
    ...
    def add_table_section(
        self, report_id: str, title: str, data: pd.DataFrame, order: int = 0
    ):
    ...
    def add_chart_section(
        self, report_id: str, title: str, chart_html: str, order: int = 0
    ):
    ...
    def add_image_section(
        self, report_id: str, title: str, image_path: str, order: int = 0
    ):
    ...
    def add_html_section(
        self, report_id: str, title: str, html_content: str, order: int = 0
    ):
    ...
    def generate_analysis_summary_report(
        self, report_id: str, analysis_data: Dict[str, Any]
    ) -> str:
        """
        ...
        if "summary_stats" in analysis_data:
            stats_df = pd.DataFrame(
                list(analysis_data["summary_stats"].items()),
        ...
        if "parameters" in analysis_data:
            params_text = "\n".join(
                [f"- **{k}:** {v}" for k, v in analysis_data["parameters"].items()]
            )
    ...
    def generate_quality_control_report(
        self, report_id: str, qc_data: Dict[str, Any]
    ) -> str:
        """
    ...
    def generate_differential_expression_report(
        self, report_id: str, de_data: Dict[str, Any]
    ) -> str:
        """
        ...
        if "top_genes" in de_data:
            ...
            self.add_table_section(
                report_id, "Top Differentially Expressed Genes", top_genes_df, 2
            )

        # Add volcano plot if available
        if "volcano_plot" in de_data:
            self.add_chart_section(
                report_id, "Volcano Plot", de_data["volcano_plot"], 3
            )
    ...
    def export_report(
        self, report_id: str, format: ReportFormat, output_path: str = None
    ) -> str:
        """
    ...
    def _export_pdf(self, report: Dict[str, Any], output_path: Path) -> str:
        ...
        story.append(
            Paragraph(
                f"Generated on: {report['updated_at'].strftime('%Y-%m-%d %H:%M:%S')}",
                styles["Normal"],
            )
        )
        ...
        story.append(
            Paragraph(
                "Report generated by Cancer Genomics Analysis Suite", styles["Normal"]
            )
        )
        ...
        with pd.ExcelWriter(str(output_path), engine="openpyxl") as writer:
            ...
            if report["metadata"]:
                metadata_df = pd.DataFrame(
                    list(report["metadata"].items()), columns=["Key", "Value"]
                )
            ...
            for i, section_data in enumerate(report["sections"]):
                ...
                section = ReportSection.from_dict(section_data)
                sheet_name = (
                    f"Section_{i+1}_{section.title[:30]}"  # Limit sheet name length
                )
            ...
            if include_data:
                for section_data in report["sections"]:
                    ...
                    if section.content_type == "table":
                        ...
                        zipf.write(
                            data_path, f"data/{section.title.replace(' ', '_')}.csv"
                        )
