...
class BatchJob:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "BatchJob":
        ...
        if data.get("estimated_duration"):
            data["estimated_duration"] = timedelta(
                seconds=int(data["estimated_duration"].split(":")[-1])
            )
        if data.get("actual_duration"):
            data["actual_duration"] = timedelta(
                seconds=int(data["actual_duration"].split(":")[-1])
            )
        return cls(**data)
...
class BatchQueue:
    ...
    def get_queue_status(self) -> Dict[str, Any]:
        ...
        active_workers = sum(
            1 for worker in self.workers.values() if worker["status"] == "running"
        )
...
class BatchDashboard:
        ...
        self.app.layout = html.Div(
            [
                ...
                html.Div(
                    [
                        html.H1(
                            "Batch Processing Dashboard", className="dashboard-title"
                        ),
                        html.Div(
                            [
                                html.Button(
                                ...
                                dcc.Interval(
                                    id="refresh-interval", interval=2000, n_intervals=0
                                ),
                            ],
                ...
                dcc.Tabs(
                    [
                        dcc.Tab(
                            ...
                            children=[
                                html.Div(
                                    id="queue-status-content", className="tab-content"
                                )
                            ],
                        ),
                        dcc.Tab(
                            ...
                            children=[
                                html.Div(
                                    id="job-management-content", className="tab-content"
                                )
                            ],
                        ),
                        dcc.Tab(
                            ...
                            children=[
                                html.Div(
                                    id="performance-content", className="tab-content"
                                )
                            ],
                        ),
                        dcc.Tab(
                            ...
                            children=[
                                html.Div(
                                    id="job-creation-content", className="tab-content"
                                )
                            ],
                        ),
                    ],
        ...
        return html.Div(
            [
                ...
                html.Div(
                    [
                        html.P(
                            f"Total Jobs: {queue_status['total_jobs']}",
                        ...
                        html.P(
                            f"Active Workers: {queue_status['active_workers']}/{queue_status['max_workers']}",
                            className="status-info",
                        ),
                        html.P(
                            f"Processing: {'Running' if queue_status['is_running'] else 'Stopped'}",
                            className="status-info",
                        ),
                    ],
                ...
                html.Div(
                    [
                        ...
                        html.P(
                            ...
                            className=f"job-status text-{status_color}",
                        ),
                        html.P(
                            f"Progress: {job.progress:.1%}", className="job-progress"
                        ),
                        html.Div(
                            [
                                dcc.Progress(
                                    value=job.progress * 100, className="progress-bar"
                                )
                            ],
    ...
    def _create_performance_content(self) -> html.Div:
        ...
        if completed_jobs:
            durations = [
                job.actual_duration.total_seconds()
                ...
                html.Div(
                    [
                        html.Div(
                            [
                                html.H4(
                                    f"{success_rate:.1f}%", className="metric-value"
                                ),
    ...
    def run_dashboard(
        self, host: str = "127.0.0.1", port: int = 8052, debug: bool = False
    ):
        """
