...
class ProteinStructureDashboard:
        ...
        self.app.layout = html.Div(
            [
                ...
                html.Div(
                    [
                        html.H1(
                            "Protein Structure Visualizer", className="header-title"
                        ),
                        html.P(
                            "Interactive 3D protein structure visualization and analysis",
                    ...
                    [
                        ...
                        html.Div(
                            [
                                ...
                                html.Div(
                                    [
                                        html.Span(
                                            [
                                                html.Label(
                                                    "UniProt for AlphaFold (optional)"
                                                ),
                                                html.Span(
                                                    " ⓘ",
                                ...
                                dcc.Store(
                                    id="structure-md-celery-task-store", data=None
                                ),
                                dcc.Interval(
    ...
    def setup_callbacks(self):
        ...
        def load_structure(load_clicks, sample_clicks, structure_content, pdb_id):
            ...
            if self.current_structure:
                ...
                chain_options = [
                    {"label": chain_id, "value": chain_id}
                    for chain_id in self.current_structure.chains.keys()
                ]
        ...
        def run_structure_md(
            n_clicks, pdb_id, uniprot, gene_flags, celery_flags, poll_flags
        ):
            if not n_clicks:
                return "", None, True
                ...
                if want_poll:
                    return (
                        html.Div(
                            [
                                html.P(
                                    [html.Strong("Celery task submitted — polling")]
                                ),
                                html.P(
                                    [
                                        "Task id: ",
                                        html.Code(
                                            tid, style={"wordBreak": "break-all"}
                                        ),
                                    ]
                                ),
                                html.P(
                                    f"Workflow: {wname}", style={"fontSize": "0.95em"}
                                ),
                            ]
                        ),
                        {
        ...
        @self.app.callback(
            ...
            Output(
                "structure-md-celery-poll-interval", "disabled", allow_duplicate=True
            ),
        ...
        def update_visualization(
            ...
            try:
                figure = self._create_3d_structure_plot(
                    chain_id, representation, color_scheme, features
                )
        ...
        def export_structure(export_clicks, format_type):
            ...
            try:
                exported_data = self.visualizer.export_structure(
                    self.current_structure.pdb_id, format_type
                )
        ...
        def update_statistics(load_clicks, sample_clicks):
            ...
            return html.Div(
                [
                    html.P(
                        [
                            html.Strong("Loaded Structures: "),
                            str(stats["loaded_structures"]),
                            html.Br(),
                            html.Strong("Structure IDs: "),
                            ", ".join(stats["structure_ids"]),
                            html.Br(),
                            html.Strong("Current Structure: "),
                            stats["current_structure"] or "None",
                            html.Br(),
                            html.Strong("Supported Formats: "),
                            ", ".join(stats["supported_formats"]),
                            html.Br(),
                            html.Strong("Total Features: "),
                            str(stats["total_features"]),
                        ]
                    )
                ]
            )
            ...
            [
                html.P(
                    [
                        ...
                        f"{structure.resolution:.2f} Å"
                        if structure.resolution
                        else "N/A",
                        html.Br(),
                        html.Strong("R-value: "),
                        f"{structure.r_value:.3f}" if structure.r_value else "N/A",
                        html.Br(),
                        html.Strong("R-free: "),
                        f"{structure.r_free:.3f}" if structure.r_free else "N/A",
                        html.Br(),
                        html.Strong("Chains: "),
                        ", ".join(structure.chains.keys()),
                        html.Br(),
                        html.Strong("Total Atoms: "),
                        str(len(structure.get_all_atoms())),
                        html.Br(),
                        html.Strong("Total Residues: "),
                        str(sum(len(chain) for chain in structure.chains.values())),
                    ]
                )
            ]
        )
    ...
    def _create_3d_structure_plot(
        self, chain_id: str, representation: str, color_scheme: str, features: List[str]
    ) -> go.Figure:
        ...
        if representation == "cartoon":
            ...
            if ca_atoms:
                ...
                trace = go.Scatter3d(
                    ...
                    text=[
                        f"{atom.residue_name}{atom.residue_number}" for atom in ca_atoms
                    ],
                )
                traces.append(trace)
            ...
            if all_atoms:
                ...
                colors = [
                    element_colors.get(atom.element, "#888888") for atom in all_atoms
                ]
                ...
                trace = go.Scatter3d(
                    ...
                    text=[
                        f"{atom.element} {atom.residue_name}{atom.residue_number}"
        ...
        if (
            "domains" in features
            and structure.pdb_id in self.visualizer.structural_features
        ):
    ...
    def _create_statistics_content(self, chain_id: str) -> html.Div:
        ...
        try:
            stats = self.visualizer.get_chain_statistics(
                self.current_structure.pdb_id, chain_id
            )
            ...
            return html.Div(
                [
                    ...
                    html.P(
                        [
                            html.Strong("Total Residues: "),
                            str(stats["total_residues"]),
                            html.Br(),
                            html.Strong("Total Atoms: "),
                            str(stats["total_atoms"]),
                            html.Br(),
                            html.Strong("Chain Length: "),
                            f"{stats['chain_length']:.2f} Å",
                            html.Br(),
                            html.Strong("Residue Range: "),
                            f"{stats['first_residue']} - {stats['last_residue']}",
                        ]
                    ),
                    ...
                    [
                        html.Div(
                            [
                                ...
                                dcc.Input(
                                    id="residue1-input", type="number", value=1, min=1
                                ),
                            ],
                        ...
                        html.Div(
                            [
                                ...
                                dcc.Input(
                                    id="residue2-input", type="number", value=5, min=1
                                ),
                            ],
                            className="distance-input",
                        ),
    ...
    def _create_secondary_structure_content(self, chain_id: str) -> html.Div:
        ...
        try:
            secondary_structure = self.visualizer.get_secondary_structure(
                self.current_structure.pdb_id, chain_id
            )
                    ...
                    [
                        ...
                        html.P(
                            [
                                ...
                                feature.chain_id,
                                html.Br(),
                                html.Strong("Residues: "),
                                f"{feature.start_residue}-{feature.end_residue}",
                                html.Br(),
