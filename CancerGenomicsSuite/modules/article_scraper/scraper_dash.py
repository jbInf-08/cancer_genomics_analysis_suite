...
class ScraperDashboard:
        ...
        return html.Div(
            [
                ...
                html.Div(
                    [
                        html.H1(
                            "Article Scraper Dashboard", className="text-center mb-4"
                        ),
                        html.P(
                            "Comprehensive article scraping and management for cancer genomics research",
                ...
                html.Div(
                    [
                        html.Div(
                            [
                                html.H4(
                                    "Scraping Configuration", className="card-title"
                                ),
                                # Source selection
                                html.Div(
                                    [
                                ...
                                html.Div(
                                    [
                                        ...
                                        dcc.Slider(
                                            ...
                                            value=100,
                                            marks={
                                                i: str(i) for i in range(10, 501, 50)
                                            },
                                            tooltip={
                                                "placement": "bottom",
                                                "always_visible": True,
    ...
    def setup_callbacks(self):
        ...
        @self.app.callback(
            Output("date-range-container", "style"), [Input("scraping-source", "value")]
        )
        ...
        @self.app.callback(
            Output("rss-feeds-container", "style"), [Input("scraping-source", "value")]
        )
        ...
        def start_scraping(
            n_clicks, source, query, max_results, start_date, end_date, rss_feeds
        ):
            ...
            if n_clicks and query:
                try:
                    ...
                    if source == "pubmed":
                        ...
                        feed_urls = [
                            url.strip() for url in rss_feeds.split("\n") if url.strip()
                        ]
                        articles = self.scraper.scrape_rss_feeds(
                            feed_urls=feed_urls,
                            max_articles_per_feed=max_results // len(feed_urls)
                            if feed_urls
                            else max_results,
                        )
                    ...
                    return articles_json, html.Div(
                        progress_msg, className="alert alert-success"
                    )

                except Exception as e:
                    logger.error(f"Error in scraping: {e}")
                    return "", html.Div(
                        f"Error: {str(e)}", className="alert alert-danger"
                    )

            return "", html.Div(
                "Click 'Start Scraping' to begin", className="text-muted"
            )
        ...
        def load_mock_data(n_clicks):
            ...
            if n_clicks:
                try:
                    ...
                    return articles_json, html.Div(
                        progress_msg, className="alert alert-info"
                    )
        ...
        @self.app.callback(
            ...
            [Input("scraped-articles", "children"), Input("filter-source", "value")],
        )
        def update_articles_list(articles_json, filter_source):
            ...
            return html.Div(
                "No articles loaded. Start scraping to see articles here.",
            ...
            if active_tab == "statistics":
                try:
                    ...
                    stats_fig = go.Figure(
                        data=[
                            go.Bar(
                                x=list(stats["scraping_stats"]["sources"].keys()),
            ...
            [
                html.Div(
                    [
                        ...
                        html.P(
                            f"Authors: {', '.join(article_data.get('authors', []))}",
                            className="card-text",
                        ),
                        html.P(
                            f"Journal: {article_data.get('journal', 'N/A')}",
                            className="card-text",
                        ),
                        html.P(
                        ...
                        html.P(
                            f"DOI: {article_data.get('doi', 'N/A')}",
                            className="card-text",
                        ),
                        html.P(
                            article_data.get("abstract", "No abstract available")[:200]
                            + "...",
                            className="card-text",
                        ),
                        html.Div(
                            [
                                html.Span(
                                    f"Citations: {article_data.get('citations', 'N/A')}",
                                    className="badge bg-primary me-2",
                                ),
                                html.Span(
                                    f"Date: {article_data.get('publication_date', 'N/A')}",
