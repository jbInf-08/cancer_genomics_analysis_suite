...
class ArticleScraper:
    ...
    def scrape_pubmed(
        self, query: str, max_results: int = 100, date_range: Tuple[str, str] = None
    ) -> List[Article]:
        """
        ...
        try:
            ...
            self._log_scraping(
                "pubmed", "scrape", "success", f"Scraped {len(articles)} articles"
            )
    ...
    def scrape_arxiv(
        self, query: str, max_results: int = 100, category: str = "cs.AI"
    ) -> List[Article]:
        """
        ...
        try:
            ...
            self._log_scraping(
                "arxiv", "scrape", "success", f"Scraped {len(articles)} articles"
            )
            self.scraping_stats["successful"] += len(articles)
    ...
    def _parse_arxiv_article(self, entry_xml) -> Optional[Article]:
        ...
        try:
            ...
            # Extract categories
            categories = []
            for category in entry_xml.findall(
    ...
    def scrape_google_scholar(
        ...
        try:
            ...
            response = self.session.get(
                base_url, params=params, headers=self.headers, timeout=30
            )
            response.raise_for_status()
            ...
            if authors_text:
                ...
                for part in author_parts[:3]:  # Limit to first 3 authors
                    ...
                    if author and not any(
                        word in author.lower()
                        for word in ["journal", "conference", "proceedings"]
                    ):
    ...
    def scrape_rss_feeds(
        self, feed_urls: List[str], max_articles_per_feed: int = 50
    ) -> List[Article]:
        """
            ...
            for feed_url in feed_urls:
                try:
                    ...
                    self._log_scraping(
                        ...
                        "scrape",
                        "success",
                        f"Scraped {len(feed.entries)} articles from {feed_url}",
                    )
    ...
    def load_articles_from_db(
        self, source: str = None, limit: int = None
    ) -> List[Article]:
        """
        ...
        with self.get_db_connection() as conn:
            ...
            for row in cursor.fetchall():
                try:
                    ...
                    article_dict["authors"] = json.loads(
                        article_dict["authors"] or "[]"
                    )
                    article_dict["keywords"] = json.loads(
                        article_dict["keywords"] or "[]"
                    )
    ...
    def export_articles(
        self, output_path: str, format: str = "csv", source: str = None
    ) -> None:
        """
