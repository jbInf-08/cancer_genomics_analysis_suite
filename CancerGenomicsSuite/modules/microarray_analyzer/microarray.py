...
class MicroarrayAnalyzer:
        ...
        try:
            ...
            if sample_metadata_file.endswith(".csv"):
                ...
                sample_metadata = pd.read_csv(
                    sample_metadata_file, sep="\t", index_col=0
                )
            else:
                raise ValueError("Unsupported file format. Use CSV or TSV.")
            ...
            if expression_matrix.shape[1] != sample_metadata.shape[0]:
                raise ValueError(
                    "Number of samples in expression matrix and metadata don't match"
                )

            if expression_matrix.shape[0] != gene_metadata.shape[0]:
                raise ValueError(
                    "Number of genes in expression matrix and metadata don't match"
                )
            ...
            logger.info(
                f"Loaded microarray data: {expression_matrix.shape[0]} genes, {expression_matrix.shape[1]} samples"
            )
            return self.data
    ...
    def _calculate_quality_metrics(
        ...
        metrics = {
            ...
            "cv_mean": float(
                (expression_matrix.std() / expression_matrix.mean()).mean()
            ),
        }
        return metrics
    ...
    def perform_differential_expression(
        self, group_column: str, group1: str, group2: str, use_normalized: bool = True
        ...
        data_to_use = (
            ...
            if use_normalized and self.normalized_data is not None
            else self.data
        )
        ...
        logger.info(
            f"Performing differential expression analysis: {group1} vs {group2}"
        )
        logger.info(
            f"Group 1 samples: {len(group1_samples)}, Group 2 samples: {len(group2_samples)}"
        )
        ...
        for gene_id in expression_matrix.index:
            gene_symbol = (
                data_to_use.gene_metadata.loc[gene_id, "gene_symbol"]
                if "gene_symbol" in data_to_use.gene_metadata.columns
                else gene_id
            )
            ...
            significant = (
                abs(log2_fold_change) >= np.log2(self.fold_change_threshold)
        ...
        for i, result in enumerate(results):
            ...
            result.significant = (
                abs(result.log2_fold_change) >= np.log2(self.fold_change_threshold)
                and result.adjusted_p_value < self.adjusted_p_value_threshold
            )
        ...
        logger.info(
            f"Differential expression analysis completed. {significant_count} significant genes found."
        )
    ...
    def get_top_differentially_expressed(
        self, n: int = 100, sort_by: str = "p_value"
    ) -> List[DifferentialExpressionResult]:
        """
        ...
        if not self.differential_results:
            raise ValueError(
                "No differential expression results available. Run analysis first."
            )
        ...
        if sort_by == "p_value":
            ...
            sorted_results = sorted(
                ...
                key=lambda x: abs(x.log2_fold_change),
                reverse=True,
            )
    ...
    def export_results(self, format: str = "json") -> str:
        ...
        results = {
            ...
            "clustering": self.clustering_results.to_dict()
    ...
    gene_metadata = pd.DataFrame(
        {
            ...
            "chromosome": np.random.choice(
                [f"chr{i}" for i in range(1, 23)] + ["chrX", "chrY"], n_genes
            ),
            "gene_type": np.random.choice(
                ["protein_coding", "lncRNA", "miRNA"], n_genes
            ),
...
if __name__ == "__main__":
    ...
    for gene in top_genes:
        print(
            f"  {gene.gene_symbol}: FC={gene.log2_fold_change:.3f}, p={gene.p_value:.3e}, significant={gene.significant}"
        )
    ...
    if analyzer.clustering_results:
        ...
        print(
            f"  Number of clusters: {len(set(analyzer.clustering_results.cluster_labels))}"
        )

    print("\nPCA Results:")
    if analyzer.pca_results:
        print(
            f"  Explained variance: {analyzer.pca_results['explained_variance_ratio']}"
        )
