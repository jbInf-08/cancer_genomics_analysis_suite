...
class MicroarrayDashboard:
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        html.P(
                            "Comprehensive microarray data analysis with normalization, differential expression, and clustering",
                    ...
                    [
                        ...
                        html.Div(
                            [
                                ...
                                html.Button(
                                    "Clustering",
                                    id="clustering-button",
                                    className="analyze-button",
                                ),
                                html.Button(
                                    "PCA", id="pca-button", className="analyze-button"
                                ),
                            ],
    ...
    def setup_callbacks(self):
        ...
        def load_data(
            ...
            group_options = [
                {"label": col, "value": col}
                for col in self.current_data.sample_metadata.columns
            ]
            ...
            try:
                if button_id == "normalize-button" and normalize_clicks:
                    ...
                    if not all([group_column, group1, group2]):
                        return (
                            "Please select groups for differential expression analysis",
                            "",
                        )

                    self.analyzer.fold_change_threshold = fc_threshold
                    self.analyzer.p_value_threshold = pval_threshold
                    self.analyzer.perform_differential_expression(
                ...
                [
                    html.P(
                        [
                            ...
                            "Loaded" if stats["data_loaded"] else "Not loaded",
                            html.Br(),
                            html.Strong("Normalized Data: "),
                            "Available"
                            if stats["normalized_data_available"]
                            else "Not available",
                            html.Br(),
                            html.Strong("Differential Results: "),
                            str(stats["differential_results_count"]),
                            html.Br(),
                            html.Strong("Clustering: "),
                            "Completed"
                            if stats["clustering_performed"]
                            else "Not performed",
                            html.Br(),
                            html.Strong("PCA: "),
                            "Completed" if stats["pca_performed"] else "Not performed",
                        ]
                    )
                ]
            )
            ...
            [
                html.P(
                    [
                        ...
                        self.current_data.platform,
                        html.Br(),
                        html.Strong("Dimensions: "),
                        f"{shape[0]} genes × {shape[1]} samples",
                        html.Br(),
                        html.Strong("Mean Expression: "),
                        f"{quality['mean_expression']:.2f}",
                        html.Br(),
                        html.Strong("Missing Values: "),
                        str(quality["missing_values"]),
                        html.Br(),
                        html.Strong("Normalization: "),
                        self.current_data.normalization_method,
                    ]
                )
            ]
        )
    ...
    def _create_heatmap(self) -> html.Div:
        ...
        sampled_genes = np.random.choice(
            expression_matrix.index, n_genes_to_show, replace=False
        )
    ...
    def _create_pca_plot(self) -> html.Div:
        ...
        fig = px.scatter(
            ...
            title="PCA Plot",
            labels={
                "x": f'PC1 ({pca_data["explained_variance_ratio"][0]:.1%})',
    ...
    def _create_quality_control_plots(self) -> html.Div:
        ...
        fig.update_layout(
            xaxis_title="Samples", yaxis_title="Expression Value", height=400
        )
    ...
    def _create_top_genes_table(self) -> html.Div:
        ...
        top_genes = self.analyzer.get_top_differentially_expressed(
            n=50, sort_by="p_value"
        )
