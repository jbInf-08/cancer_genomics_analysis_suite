...
class TreeConstructionConfig:
    ...
    method: str = (
        "neighbor_joining"  # neighbor_joining, upgma, parsimony, maximum_likelihood
    )
...
class PhylogeneticTreeBuilder:
    ...
    def load_sequences(
        self, sequences: Union[str, List[SeqRecord], MultipleSeqAlignment]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            self.logger.info(
                f"Loaded {len(alignment)} sequences, length {alignment.get_alignment_length()}"
            )
    ...
    def build_tree(self, tree_name: str = "phylogenetic_tree") -> Dict[str, Any]:
        ...
        if self.sequence_alignment is None:
            return {"success": False, "error": "No sequences loaded"}

        self.logger.info(
            f"Building phylogenetic tree using {self.config.method} method..."
        )
        ...
        try:
            ...
            self.logger.info(
                f"Tree construction completed in {construction_time:.2f} seconds"
            )
    ...
    def _perform_bootstrap_analysis(self, tree_name: str) -> Dict[str, Any]:
        ...
        self.logger.info(
            f"Performing bootstrap analysis with {self.config.bootstrap_replicates} replicates..."
        )
        ...
        try:
            ...
            return {
                ...
                "high_support_nodes": len(
                    [
                        s
                        for s in support_values.values()
                        if s >= self.config.bootstrap_threshold
                    ]
                ),
            }
    ...
    def compare_trees(self, tree1_name: str, tree2_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                "tree1": {"name": tree1_name, "statistics": stats1},
                "tree2": {"name": tree2_name, "statistics": stats2},
    ...
    def export_tree(
        self, tree_name: str, output_path: str, format: str = "newick"
    ) -> str:
        """
        Export tree to file.
    ...
    def create_tree_visualization(self, tree_name: str, output_path: str = None) -> str:
        ...
        try:
            ...
            # Customize plot
            ax.set_title(
                f"Phylogenetic Tree: {tree_name}", fontsize=16, fontweight="bold"
            )
    ...
    def get_tree_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "current_alignment": {
                "sequences": len(self.sequence_alignment)
                if self.sequence_alignment
                else 0,
                "alignment_length": self.sequence_alignment.get_alignment_length()
                if self.sequence_alignment
                else 0,
            },
            "config": {
