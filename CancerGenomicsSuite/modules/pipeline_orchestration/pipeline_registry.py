...
class PipelineRegistry:
    ...
    def __init__(self, registry_file: Optional[str] = None):
        ...
        """
        self.registry_file = (
            Path(registry_file) if registry_file else Path("pipeline_registry.json")
        )
    ...
    def search_pipelines(self, query: str) -> List[Dict]:
        ...
        for pipeline in self.pipelines.values():
            ...
            if (
                ...
                or any(query_lower in tag.lower() for tag in pipeline["tags"])
            ):
    ...
    def export_pipeline(self, pipeline_name: str, export_path: str) -> bool:
        ...
        if (
            export_path.suffix.lower() == ".yaml"
            or export_path.suffix.lower() == ".yml"
        ):
    ...
    def import_pipeline(self, import_path: str) -> bool:
        ...
        try:
            if (
                import_path.suffix.lower() == ".yaml"
                or import_path.suffix.lower() == ".yml"
            ):
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        recent_pipelines = sorted(
            self.pipelines.values(), key=lambda x: x["registered_at"], reverse=True
        )[:10]
