...
class HDOCKManager:
    def __init__(self, work_dir: Optional[str] = None):
        self.work_dir = Path(work_dir) if work_dir else Path.cwd() / "hdock_work"
    ...
    def execute_job(
        ...
        receptor: str,
        ligand: str,
        hdock_args: Optional[List[str]] = None,
        env: Optional[Dict[str, str]] = None,
        job_name: Optional[str] = None,
    ) -> Dict[str, Any]:
        job_name = job_name or f"hdock_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        exec_dir = self.work_dir / job_name
        exec_dir.mkdir(parents=True, exist_ok=True)

        cmd = ["hdock", receptor, ligand]
        if hdock_args:
            cmd.extend(hdock_args)

        merged_env = os.environ.copy()
        if env:
            merged_env.update(env)

        logger.info("Executing HDOCK job %s", job_name)
        ...
        try:
            ...
            logger.error("Failed to execute HDOCK job %s: %s", job_name, exc)
    ...
    def stop_job(self, job_name: str) -> bool:
        ...
        if process and process.poll() is None:
            ...
            del self.active_jobs[job_name]
            logger.info("HDOCK job %s stopped", job_name)
            return True
        return False
