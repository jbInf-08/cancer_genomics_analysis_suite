...
class WorkflowExecutor:
    ...
    def __init__(
        ...
        self.perl_manager = PerlManager(work_dir=str(self.work_dir / "perl"))
        self.seurat_manager = SeuratManager(work_dir=str(self.work_dir / "seurat"))
        self.hdock_manager = HDOCKManager(work_dir=str(self.work_dir / "hdock"))
        self.haddock_manager = HADDOCKManager(work_dir=str(self.work_dir / "haddock"))
        self.saturn_manager = SATurnManager(work_dir=str(self.work_dir / "saturn"))
        self.seqant_manager = SeqAntManager(work_dir=str(self.work_dir / "seqant"))
    ...
    def execute_workflow(
        ...
        workflow_name = (
            workflow_name
            or f"{pipeline_name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
        )
        ...
        if dependencies:
            for dep in dependencies:
                if dep not in [w["name"] for w in self.workflow_history]:
                    raise ValueError(
                        f"Dependency {dep} not found in completed workflows"
                    )
    ...
    def get_workflow_status(self, workflow_name: str) -> Optional[Dict[str, Any]]:
        ...
        if workflow_name in self.active_workflows:
            ...
            if workflow_info["started_at"]:
                workflow_info["duration"] = (
                    datetime.now() - workflow_info["started_at"]
                ).total_seconds()
    ...
    def stop_workflow(self, workflow_name: str) -> bool:
        ...
        if success:
            workflow_info.update({"status": "stopped", "completed_at": datetime.now()})
    ...
    def _start_workflow(self, workflow_info: Dict):
        ...
        try:
            ...
            if orchestration_system == "nextflow":
                ...
                raise ValueError(
                    f"Unsupported orchestration system: {orchestration_system}"
                )
    ...
    def _detect_orchestration_system(self, script_path: Path) -> str:
        ...
        if script_path.suffix == ".nf":
            ...
            try:
                ...
                if (
                    "#!/usr/bin/env nextflow" in content
                    or "nextflow" in content.lower()
                ):
                    return "nextflow"
                elif "rule" in content and "input:" in content and "output:" in content:
                    return "snakemake"
                elif "#!/usr/bin/env perl" in content or content.strip().startswith(
                    "#!/usr/bin/perl"
                ):
                    return "perl"
    ...
    def _execute_hdock_workflow(self, workflow_info: Dict) -> str:
        ...
        receptor = config.get("receptor") or pipeline_info.get("metadata", {}).get(
            "receptor"
        )
    ...
    def _execute_haddock_workflow(self, workflow_info: Dict) -> str:
        ...
        project_dir = (
            ...
            or pipeline_info.get("metadata", {}).get("project_dir")
            or str(Path(pipeline_info["script_path"]).parent)
        )
    ...
    def _monitor_workflow(self, workflow_name: str):
        ...
        try:
            ...
            if status_info and status_info["status"] in [
                ...
                logger.info(
                    f"Workflow {workflow_name} completed with status: {status_info['status']}"
                )
    ...
    def create_cancer_genomics_workflow(
        ...
        if not self.registry.get_pipeline(pipeline_name):
            ...
            self.registry.register_pipeline(
                ...
                config_template={
                    "defaults": {"output_dir": output_dir, "data_paths": data_paths},
        ...
        # Execute workflow
        return self.execute_workflow(pipeline_name=pipeline_name, config=config)
    ...
    def run_molecular_dynamics_workflow(
        ...
        logger.info(
            "Molecular dynamics workflow %s finished: %s", name, entry["status"]
        )
        return result
    ...
    def get_workflow_statistics(self) -> Dict[str, Any]:
        ...
        stats = {
            ...
            "failed_workflows": len(
                [w for w in self.workflow_history if w["status"] in ["failed", "error"]]
            ),
        ...
        for workflow in self.workflow_history + list(self.active_workflows.values()):
            ...
            stats["orchestration_systems"][system] = (
                stats["orchestration_systems"].get(system, 0) + 1
            )

        # Count by pipeline type
        for workflow in self.workflow_history + list(self.active_workflows.values()):
            pipeline_type = workflow.get("pipeline_info", {}).get(
                "pipeline_type", "unknown"
            )
            stats["pipeline_types"][pipeline_type] = (
                stats["pipeline_types"].get(pipeline_type, 0) + 1
            )
        ...
        completed_workflows = [
            ...
            and w.get("started_at")
            and w.get("completed_at")
        ]
