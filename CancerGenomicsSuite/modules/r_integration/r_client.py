...
try:
    ...
    logger.warning(
        f"rpy2 not available: {e}. R integration will use subprocess fallback."
    )
except Exception as e:
    logger.warning(
        f"Error initializing rpy2: {e}. R integration will use subprocess fallback."
    )
...
class RClient:
    ...
    def __init__(self):
        ...
        if self.rpy2_available:
            try:
                ...
                logger.warning(
                    f"Failed to initialize rpy2: {e}. Using subprocess fallback."
                )
                self.rpy2_available = False
                self.r = None
        else:
            logger.info(
                "R client initialized in subprocess-only mode (rpy2 not available)"
            )
    ...
    def execute_r_code(self, r_code: str) -> Any:
        ...
        if not self.rpy2_available or self.r is None:
            ...
            if result["success"]:
                ...
                raise RuntimeError(
                    f"R execution failed: {result.get('stderr', result.get('error', 'Unknown error'))}"
                )
    ...
    def install_package(self, package_name: str, source: str = "CRAN") -> bool:
        ...
        try:
            ...
            if result["success"]:
                ...
                self.available_packages = self._get_installed_packages()
                logger.info(
                    f"Successfully installed package {package_name} from {source}"
                )
                return True
            else:
                logger.error(
    ...
    def load_data(
        self, data: Union[pd.DataFrame, np.ndarray, List], name: str = "data"
    ) -> bool:
        """
    ...
    def create_heatmap(
        self, data: pd.DataFrame, title: str = "Heatmap", clustering: bool = True
    ) -> str:
        """
