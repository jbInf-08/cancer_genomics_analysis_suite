...
class KEGGPathwayOverlay:
    ...
    def __init__(self, base_url: str = "https://rest.kegg.jp"):
        ...
        self.session.headers.update({"User-Agent": "CancerGenomicsSuite/1.0"})
    ...
    def search_pathways(
        self, query: str, organism: str = "hsa"
    ) -> List[Dict[str, str]]:
        """
        ...
        for pathway in all_pathways:
            if (
                query_lower in pathway["name"].lower()
                or query_lower in pathway["description"].lower()
            ):
    ...
    def overlay_expression_data(
        self, pathway_id: str, expression_data: pd.DataFrame
    ) -> Dict[str, Any]:
        """
    ...
    def _parse_gene_entry(self, line: str) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if len(parts) >= 2:
                ...
                return {"id": gene_id, "name": gene_name, "description": description}
        except Exception as e:
    ...
    def _parse_reaction_entry(self, line: str) -> Optional[Dict[str, str]]:
        ...
        try:
            ...
            if len(parts) >= 1:
                ...
                return {"id": reaction_id, "description": description}
        except Exception as e:
            logger.warning(f"Error parsing reaction entry: {e}")
        return None

    def get_common_pathways(
        self, gene_list: List[str], organism: str = "hsa"
    ) -> Dict[str, List[str]]:
        """
        ...
        for gene in gene_list:
            ...
            for kegg_gene, pathways in gene_pathway_mapping.items():
                if (
                    gene.lower() in kegg_gene.lower()
                    or kegg_gene.lower() in gene.lower()
                ):
