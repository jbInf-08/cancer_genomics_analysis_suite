...
class NGSPipelineManager:
    ...
    async def load_pipeline_definition(
        self, pipeline_path: Union[str, Path]
    ) -> PipelineDefinition:
        ...
        try:
            ...
            if not pipeline_path.exists():
                raise FileNotFoundError(
                    f"Pipeline definition not found: {pipeline_path}"
                )
            ...
            logger.info(
                f"Loaded pipeline definition: {pipeline.name} v{pipeline.version}"
            )
            return pipeline

        except Exception as e:
            logger.error(
                f"Failed to load pipeline definition from {pipeline_path}: {e}"
            )
            raise
    ...
    async def save_pipeline_definition(
        ...
        try:
            if output_path is None:
                output_path = (
                    self.pipeline_dir / f"{pipeline.name}_v{pipeline.version}.yaml"
                )
    ...
    async def execute_pipeline(
        ...
        try:
            ...
            execution = PipelineExecution(
                execution_id=execution_id, pipeline_definition=pipeline
            )
    ...
    async def list_executions(
        self, status_filter: Optional[PipelineStatus] = None
    ...
    async def cleanup_old_executions(self, days_old: int = 30) -> int:
        ...
        for execution_id, execution in list(self.executions.items()):
            if (
                ...
                and execution.end_time < cutoff_date
            ):
...
class PipelineStepExecutor:
    ...
    async def execute_step(
        self, step: PipelineStep, inputs: Dict[str, Any], parameters: Dict[str, Any]
    ) -> Dict[str, Any]:
    ...
    async def _execute_in_docker(
        self, step: PipelineStep, command: str
    ) -> Dict[str, Any]:
        ...
        return {"exit_code": exit_code, "logs": logs, "container_id": container_id}
...
class PipelineValidator:
    ...
    def validate_execution_inputs(
        pipeline: PipelineDefinition, inputs: Dict[str, Any]
    ) -> List[str]:
            ...
            if input_name not in inputs:
                ...
                if "type" in input_spec:
                    ...
                    if expected_type == "file" and not Path(actual_value).exists():
                        ...
                        expected_type == "directory" and not Path(actual_value).is_dir()
                    ):
...
class EnhancedWorkflowDispatcher(WorkflowDispatcher):
    ...
    async def execute_pipeline(
        ...
        return await self.pipeline_manager.execute_pipeline(
            pipeline_name, inputs, parameters
        )
