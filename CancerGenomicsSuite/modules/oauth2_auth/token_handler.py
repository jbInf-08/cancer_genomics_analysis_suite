...
class TokenHandler:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        if not JWT_AVAILABLE:
            raise ImportError(
                "JWT libraries not available. Install python-jose package."
            )
    ...
    def generate_refresh_token(
        self, user_id: str, username: str, access_token_id: str
    ) -> str:
        """
    ...
    def refresh_access_token(self, refresh_token: str) -> Dict[str, str]:
        ...
        try:
            ...
            new_access_claims = jwt.decode(
                new_access_token, self.secret_key, algorithms=[self.algorithm]
            )
            new_access_token_id = new_access_claims.get("jti")

            # Generate new refresh token
            new_refresh_token = self.generate_refresh_token(
                user_id=user_id, username=username, access_token_id=new_access_token_id
            )
    ...
    def revoke_all_user_tokens(self, user_id: str) -> int:
        ...
        for token_id, refresh_metadata in self.refresh_tokens.items():
            if (
                refresh_metadata["user_id"] == user_id
                and not refresh_metadata["revoked"]
            ):
        ...
        try:
            ...
            if token_type == "access" and token_id in self.token_storage:
                ...
                return {
                    ...
                    "last_used": metadata.get(
                        "last_used", metadata["issued_at"]
                    ).isoformat(),
    ...
    def cleanup_expired_tokens(self):
        ...
        if expired_access_tokens or expired_refresh_tokens:
            logger.info(
                f"Cleaned up {len(expired_access_tokens)} expired access tokens and {len(expired_refresh_tokens)} expired refresh tokens"
            )
    ...
    def get_token_statistics(self) -> Dict[str, Any]:
        ...
        active_access_tokens = sum(
            1
            for metadata in self.token_storage.values()
