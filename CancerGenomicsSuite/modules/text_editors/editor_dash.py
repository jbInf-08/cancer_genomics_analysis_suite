    ...
    [
        html.Div(
            [
                ...
                html.P(
                    "File editing, text processing, and editor management",
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            [
                                html.Span(
                                    "Available Editors: ", className="status-label"
                                ),
                                html.Span(
                                    id="available-editors", className="status-value"
                                ),
                            ],
                            className="status-item",
                        ),
                ...
                html.Div(
                    [
                        html.Div(
                            [
                                html.Label(
                                    "Current Directory:", className="input-label"
                                ),
                                dcc.Input(
...
def register_callbacks(app):
                    ...
                    [
                        ...
                        html.Div(
                            [
                                ...
                                html.Ul(
                                    [html.Li(feature) for feature in info["features"]],
    ...
    def create_file(n_clicks, file_path, content, editor):
        ...
        try:
            ...
            result = editor_client.edit_file_content(
                file_path, content or "", backup=False
            )
            ...
            if result["success"]:
                return html.Div(
                    [
                        ...
                        html.P(f"File: {result['file_path']}"),
                        html.P(
                            f"Backup created: {'Yes' if result['backup_created'] else 'No'}"
                        ),
    ...
    @app.callback(
        ...
        [Input("browse-directory", "n_clicks"), Input("refresh-directory", "n_clicks")],
    ...
    def update_file_browser(browse_clicks, refresh_clicks, directory):
        ...
        try:
            ...
            all_items = sorted(directories, key=lambda x: x["name"].lower()) + sorted(
                    ...
                    [
                        ...
                        html.Div(
                            [
                                ...
                                html.P(
                                    f"Size: {result['size_human']} ({result['size']} bytes)"
                                ),
                                html.P(f"Lines: {result['lines']}"),
                                html.P(f"Encoding: {result['encoding']}"),
                                html.P(
                                    f"Text File: {'Yes' if result['is_text'] else 'No'}"
                                ),
    ...
    def search_in_file(n_clicks, file_path, search_term, options):
        ...
        try:
            case_sensitive = "case_sensitive" in options
            result = editor_client.search_in_file(
                file_path, search_term, case_sensitive
            )
            ...
            if result["success"]:
                ...
                if matches:
                    ...
                    for match in matches[:20]:  # Show first 20 matches
                        match_item = html.Div(
                            [
                                ...
                                html.Span(
                                    match["line_content"], className="match-content"
                                ),
                            ],
                    ...
                    return html.Div(
                        [
                            html.H5(
                                f"🔍 Search Results ({result['total_matches']} matches)"
                            ),
