...
class DNAAnalyzer:
        ...
        if "individual_analyses" in results:
            for seq_name, seq_results in results["individual_analyses"].items():
                if seq_results.get("valid") and "statistics" in seq_results:
                    ...
                    self._create_sequence_plots(
                        seq_results, seq_name, output_dir, plot_files
                    )
    ...
    def _create_comparison_plots(
        self, results: Dict[str, Any], output_dir: str, plot_files: List[str]
    ):
