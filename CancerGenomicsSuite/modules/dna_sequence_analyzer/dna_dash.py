...
from .analyzer import AnalysisConfig, DNAAnalyzer
            ...
            [
                ...
                html.Div(
                    [
                        ...
                        html.P(
                            "Comprehensive DNA sequence analysis tools for cancer genomics research",
                            ...
                            [
                                ...
                                html.Div(
                                    [
                                        ...
                                        html.Label(
                                            "DNA Sequence:", className="form-label"
                                        ),
                                        dcc.Textarea(
    ...
    def _register_callbacks(self):
        ...
        def analyze_sequence(n_clicks, sequence, name, options, min_orf_length):
            ...
            if n_clicks == 0 or not sequence:
                return (
                    html.P("Enter a sequence and click 'Analyze Sequence'"),
                    html.Div(),
                )
        ...
        @self.app.callback(
            [Output("dna-sequence-input", "value"), Output("sequence-name", "value")],
    ...
    def _create_visualizations(self, results: Dict[str, Any]) -> html.Div:
            ...
            fig = go.Figure(
                data=[
                    go.Bar(
                        ...
                        text=[f"{gc_content:.1f}%"],
                        textposition="auto",
                    )
                ]
            )
            fig.add_hline(
                y=50, line_dash="dash", line_color="red", annotation_text="50% GC"
            )
            fig.update_layout(
                title="GC Content", yaxis_title="GC Content (%)", showlegend=False
            )
        ...
        if "statistics" in results and "nucleotide_counts" in results["statistics"]:
            ...
            fig = go.Figure(
                data=[
                    go.Bar(
                        x=list(nuc_counts.keys()),
                        y=list(nuc_counts.values()),
        ...
        if "orfs" in results and results["orfs"]:
            ...
            fig = go.Figure(
                data=[go.Histogram(x=orf_lengths, nbinsx=20, marker_color="lightgreen")]
            )
            fig.update_layout(
