...
class DNAUtils:
    ...
    def _find_palindromes(
        self, sequence: str, min_length: int = 4
    ) -> List[Dict[str, Any]]:
        ...
        for i in range(len(sequence) - min_length + 1):
            for j in range(i + min_length, len(sequence) + 1):
                ...
                if subseq == self._reverse_complement(subseq):
                    palindromes.append(
                        {"start": i, "end": j, "length": j - i, "sequence": subseq}
                    )
    ...
    def translate_sequence(
        self, sequence: str, reading_frame: int = 0
    ) -> Dict[str, Any]:
        """
    ...
    def find_restriction_sites(
        self, sequence: str, enzyme_list: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
        """
        ...
        if enzyme_list:
            restriction_enzymes = {
                k: v for k, v in restriction_enzymes.items() if k in enzyme_list
            }
    ...
    def create_sequence_plots(
        self, sequence: str, output_dir: str = "outputs/plots"
    ) -> List[str]:
        """
    ...
    def export_analysis_results(
        self, sequence: str, analysis_results: Dict[str, Any], output_file: str
    ) -> str:
        """
        ...
        export_data = {
            "sequence_info": {"length": len(sequence), "sequence": sequence},
            ...
            for key, value in analysis_results.items():
                if isinstance(value, dict):
                    for subkey, subvalue in value.items():
                        flat_data.append(
                            {"category": key, "metric": subkey, "value": subvalue}
                        )
                else:
                    flat_data.append(
                        {"category": "general", "metric": key, "value": value}
                    )
