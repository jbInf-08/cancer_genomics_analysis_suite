...
class DashboardConfig:
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "DashboardConfig":
        ...
        data["widgets"] = [
            DashboardWidget.from_dict(widget) for widget in data["widgets"]
        ]
        return cls(**data)
...
class DashboardLoader:
    ...
    def __init__(self, config_dir: str = None, templates_dir: str = None):
        ...
        self.templates_dir = (
            Path(templates_dir) if templates_dir else Path("templates/dashboards")
        )
    ...
    def create_dashboard_from_template(
        self, template_name: str, dashboard_id: str, dashboard_name: str, **kwargs
    ) -> str:
        """
    ...
    def _create_dashboard_layout(self, config: DashboardConfig) -> html.Div:
        ...
        header = html.Div(
            [
                ...
                html.P(config.description, className="dashboard-description")
                if config.description
                else None,
                html.Div(
                    [
                        html.Button(
                            "Refresh", id="refresh-btn", className="btn btn-primary"
                        ),
                        html.Button(
                            "Settings", id="settings-btn", className="btn btn-secondary"
                        ),
                        dcc.Interval(
                            id="refresh-interval",
    ...
    def _create_filter_widget(self, widget: DashboardWidget) -> html.Div:
        ...
        if filter_type == "dropdown":
            ...
            component = dcc.RangeSlider(
                ...
                value=[
                    min(options) if options else 0,
                    max(options) if options else 100,
                ],
        ...
        return html.Div(
            [html.H3(widget.title, className="widget-title"), component],
    ...
    def _create_image_widget(self, widget: DashboardWidget) -> html.Div:
        ...
        return html.Div(
            [
                html.H3(widget.title, className="widget-title"),
                html.Img(src=image_path, alt=widget.title, className="widget-image")
                if image_path
                else html.P("No image"),
            ],
