...
class LiveControls:
    ...
    def _start_stream_handler(self, stream: LiveDataStream):
        ...
        thread = threading.Thread(
            target=handler, args=(stream,), name=f"stream-{stream.id}", daemon=True
        )
        thread.start()
    ...
    def _handle_file_watch_stream(self, stream: LiveDataStream):
        ...
        try:
                    ...
                    if not event.is_directory:
                        try:
                            with open(event.src_path, "r") as f:
                                data = json.load(f)
                                self.live_controls._process_stream_data(
                                    self.stream_id, data
                                )
                        except Exception as e:
            ...
            observer = Observer()
            observer.schedule(
                FileHandler(self, stream.id), stream.source, recursive=False
            )
            observer.start()
    ...
    def _process_stream_data(self, stream_id: str, data: Any):
        ...
        try:
            ...
            buffer.append({"timestamp": datetime.now(), "data": data})
    ...
    def get_stream_data(
        self, stream_id: str, limit: int = None
    ) -> List[Dict[str, Any]]:
                ...
                control_components.append(
                    html.Div(
                        [
                            ...
                            component,
                            html.Div(
                                id=f"control-output-{control.id}",
    ...
    def _create_slider_control(self, control: ControlWidget) -> dcc.Slider:
        ...
        return dcc.Slider(
            ...
            marks={
                i: str(i)
                for i in range(
                    int(control.min_value or 0), int(control.max_value or 100) + 1, 10
                )
            },
        )
    ...
    def _create_range_slider_control(self, control: ControlWidget) -> dcc.RangeSlider:
        ...
        return dcc.RangeSlider(
            ...
            step=control.step or 1,
            value=control.value
            if isinstance(control.value, list)
            else [control.min_value or 0, control.max_value or 100],
            disabled=not control.enabled,
            marks={
                i: str(i)
                for i in range(
