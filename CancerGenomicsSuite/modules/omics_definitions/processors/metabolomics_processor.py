...
class MetabolomicsProcessor(OmicsDataProcessor):
        ...
        try:
            ...
            if missing_strategy == "remove":
                ...
                processing_log.append(
                    f"Removed metabolites with missing rate > {max_missing_rate}"
                )
            ...
            if "min_abundance" in kwargs:
                ...
                processing_log.append(
                    f"Filtered metabolites with abundance < {min_abundance}"
                )
            ...
            if "min_variance" in kwargs:
                ...
                processing_log.append(
                    f"Filtered metabolites with variance < {min_variance}"
                )
            ...
            if "min_detection_rate" in kwargs:
                ...
                processing_log.append(
                    f"Filtered metabolites with detection rate < {min_detection_rate}"
                )
            ...
            if "max_sample_missing_rate" in kwargs:
                ...
                processing_log.append(
                    f"Filtered samples with missing rate > {max_sample_missing_rate}"
                )
    ...
    def normalize_data(
        ...
        try:
            processing_log = [
                f"Starting metabolomics normalization with method: {method}"
            ]

            if method == "internal_standard":
                # Internal standard normalization
                data_normalized = self._apply_internal_standard_normalization(
                    data, **kwargs
                )
    ...
    def _apply_internal_standard_normalization(
    ...
    def _apply_total_ion_current_normalization(
    ...
    def _apply_relative_abundance_normalization(
        self, data: pd.DataFrame
    ) -> pd.DataFrame:
    ...
    def perform_differential_metabolite_analysis(
        self,
        data: pd.DataFrame,
    ...
    def _perform_ttest_dma(
        self, group1_data: pd.DataFrame, group2_data: pd.DataFrame
    ) -> Dict[str, Any]:
    ...
    def perform_metabolic_pathway_analysis(
        self, dma_results: Dict[str, Any], pathway_database: str = "kegg", **kwargs
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if "log2_fold_change" in dma_results and "adjusted_p_value" in dma_results:
                ...
                for pathway in mock_pathways:
                    ...
                    pathway_metabolites = significant_metabolites[
                    ...
                    pathway_results["metabolite_pathway_mapping"][
                        pathway
                    ] = pathway_metabolites
...
"""
        ...
        report += (
            f"- Abundance range: [{data.min().min():.2f}, {data.max().max():.2f}]\n"
        )

        # Metabolite categories
        mean_abundance_per_metabolite = data.mean(axis=1)
        high_abundance = sum(
            mean_abundance_per_metabolite > mean_abundance_per_metabolite.quantile(0.8)
        )
        low_abundance = sum(
            mean_abundance_per_metabolite < mean_abundance_per_metabolite.quantile(0.2)
        )
