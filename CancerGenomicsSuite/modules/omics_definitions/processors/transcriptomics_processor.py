...
class TranscriptomicsProcessor(OmicsDataProcessor):
    ...
    def _categorize_genes(self, data: pd.DataFrame) -> Dict[str, int]:
        ...
        categories = {
            ...
            "moderately_expressed": sum(
                (mean_expression >= mean_expression.quantile(0.2))
        ...
        try:
            ...
            if "min_expression" in kwargs:
                ...
                processing_log.append(
                    f"Filtered genes with expression < {min_expression}"
                )
            ...
            if "min_detection_rate" in kwargs:
                ...
                processing_log.append(
                    f"Filtered genes with detection rate < {min_detection_rate}"
                )
            ...
            if "min_library_size" in kwargs:
                ...
                processing_log.append(
                    f"Filtered samples with library size < {min_library_size}"
                )
            ...
            if "min_genes_detected" in kwargs:
                ...
                processing_log.append(
                    f"Filtered samples with < {min_genes_detected} genes detected"
                )
    ...
    def normalize_data(
        ...
        try:
            processing_log = [
                f"Starting transcriptomics normalization with method: {method}"
            ]
            ...
            if method == "tmm":
                ...
                if gene_lengths is not None:
                    ...
                    processing_log.append(
                        "RPKM normalization skipped (no gene lengths provided)"
                    )
                ...
                if gene_lengths is not None:
                    ...
                    data_normalized = data
                    processing_log.append(
                        "FPKM normalization skipped (no gene lengths provided)"
                    )
    ...
    def _apply_deseq2_normalization(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        for sample in data.columns:
            ...
            if valid_genes.sum() > 0:
                size_factor = np.exp(
                    np.median(
                        log_data.loc[valid_genes, sample] - geometric_means[valid_genes]
                    )
                )
            else:
                size_factor = 1.0
    ...
    def _apply_rpkm_normalization(
        self, data: pd.DataFrame, gene_lengths: Dict[str, float]
    ) -> pd.DataFrame:
        """Apply RPKM normalization."""
        # RPKM = (reads mapped to gene / total reads) * 10^9 / gene length

        # Calculate reads per kilobase
        reads_per_kb = data.div(
            [gene_lengths.get(gene, 1000) / 1000 for gene in data.index], axis=0
        )
    ...
    def perform_pathway_analysis(
        self, de_results: Dict[str, Any], pathway_database: str = "kegg", **kwargs
    ) -> Dict[str, Any]:
    ...
    def generate_transcriptomics_report(
...
"""
        ...
        report += (
            f"- Expression range: [{data.min().min():.2f}, {data.max().max():.2f}]\n"
        )
