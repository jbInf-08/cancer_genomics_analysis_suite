...
class EpigenomicsProcessor(OmicsDataProcessor):
    ...
    def _categorize_features(self, data: pd.DataFrame) -> Dict[str, int]:
        ...
        if data.max().max() <= 1.0:
            categories = {
                ...
                "moderately_methylated": sum(
                    (mean_modification >= 0.3) & (mean_modification <= 0.7)
                ),
            ...
            categories = {
                ...
                "moderate_enrichment": sum(
                    (mean_modification >= mean_modification.quantile(0.2))
                    & (mean_modification <= mean_modification.quantile(0.8))
                ),
                "low_enrichment": sum(
                    mean_modification < mean_modification.quantile(0.2)
        ...
        try:
            ...
            if "min_detection_pvalue" in kwargs:
                ...
                processing_log.append(
                    f"Filtered features with detection p-value > {min_pvalue}"
                )
            ...
            if "max_missing_rate" in kwargs:
                ...
                processing_log.append(
                    f"Filtered features with missing rate > {max_missing_rate}"
                )
            ...
            if "min_variance" in kwargs:
                ...
                processing_log.append(
                    f"Filtered features with variance < {min_variance}"
                )
            ...
            if "missing_value_strategy" in kwargs:
                ...
                if strategy == "remove":
                    ...
                    processing_log.append(
                        f"Imputed missing values using {impute_method}"
                    )
    ...
    def normalize_data(
        ...
        try:
            processing_log = [
                f"Starting epigenomics normalization with method: {method}"
            ]
    ...
    def perform_differential_methylation_analysis(
        self,
        data: pd.DataFrame,
    ...
    def _perform_beta_binomial_dma(
        self, group1_data: pd.DataFrame, group2_data: pd.DataFrame
    ) -> Dict[str, Any]:
    ...
    def perform_chromatin_analysis(
    ...
    def generate_epigenomics_report(
...
"""
        ...
        report += (
            f"- Modification range: [{data.min().min():.3f}, {data.max().max():.3f}]\n"
        )
        ...
        if data.max().max() <= 1.0:  # Methylation data
            ...
            highly_modified = sum(
                mean_modification_per_feature
                > mean_modification_per_feature.quantile(0.8)
            )
            lowly_modified = sum(
                mean_modification_per_feature
                < mean_modification_per_feature.quantile(0.2)
            )
