...
class OmicsMetadataManager:
    ...
    def add_feature_metadata(
        self, omics_type: str, feature_metadata: OmicsFeatureMetadata
    ):
    ...
    def get_feature_metadata(
        self, omics_type: str, feature_id: str
    ) -> Optional[OmicsFeatureMetadata]:
    ...
    ) -> Optional[OmicsExperimentMetadata]:
    ...
    def create_sample_metadata_from_dataframe(
        self, df: pd.DataFrame, sample_id_col: str = "sample_id"
    ) -> Dict[str, OmicsSampleMetadata]:
    ...
    def create_feature_metadata_from_dataframe(
        self, df: pd.DataFrame, omics_type: str, feature_id_col: str = "feature_id"
    ) -> Dict[str, OmicsFeatureMetadata]:
    ...
    def create_experiment_metadata_from_dataframe(
        self, df: pd.DataFrame, experiment_id_col: str = "experiment_id"
    ) -> Dict[str, OmicsExperimentMetadata]:
    ...
    def export_feature_metadata(
        self, omics_type: str, file_path: str, format: str = "csv"
    ):
    ...
    def import_sample_metadata(self, file_path: str, format: str = "csv"):
        ...
        logger.info(
            f"Imported {len(sample_metadata)} sample metadata records from {file_path}"
        )

    def import_feature_metadata(
        self, omics_type: str, file_path: str, format: str = "csv"
    ):
        ...
        logger.info(
            f"Imported {len(feature_metadata)} feature metadata records for {omics_type} from {file_path}"
        )
    ...
    def import_experiment_metadata(self, file_path: str, format: str = "csv"):
        ...
        logger.info(
            f"Imported {len(experiment_metadata)} experiment metadata records from {file_path}"
        )
    ...
    def get_metadata_summary(self) -> Dict[str, Any]:
        ...
        summary = {
            ...
            "feature_metadata_count": sum(
                len(metadata) for metadata in self.feature_metadata.values()
            ),
    ...
    def validate_metadata_consistency(self) -> Dict[str, List[str]]:
        ...
        for sample_id, metadata in self.sample_metadata.items():
            ...
            if (
                ...
                and metadata.experiment_id not in self.experiment_metadata
            ):
        ...
        for omics_type, features in self.feature_metadata.items():
            for feature_id, metadata in features.items():
                if not metadata.feature_id:
                    issues["feature_issues"].append(
                        f"Feature {feature_id} in {omics_type} has no feature_id"
                    )

        # Validate experiment metadata
        for experiment_id, metadata in self.experiment_metadata.items():
            if not metadata.experiment_id:
                issues["experiment_issues"].append(
                    f"Experiment {experiment_id} has no experiment_id"
                )
