...
class OmicsAnalysisRunner:
    ...
    def process_data_file(
        self, file_path: str, omics_type: str, output_path: str = None
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if not preprocess_result.success:
                logger.error(
                    f"Failed to preprocess data: {preprocess_result.error_message}"
                )
            ...
            # Print results
            self._print_processing_results(
                load_result, preprocess_result, processed_data
            )
    ...
    def _print_demonstration_summary(self, results: Dict[str, Any]):
        ...
        if "processing" in results:
            successful = sum(
                1
                for r in results["processing"].values()
        ...
        if "metadata" in results and "summary" in results["metadata"]:
            ...
            print(
                f"Metadata: {summary['sample_metadata_count']} samples, {summary['feature_metadata_count']} features"
            )
    ...
    def _print_integration_results(self, result: Any):
        ...
        if hasattr(result, "integrated_data") and result.integrated_data is not None:
            ...
            if (
                hasattr(result, "sample_clusters")
                and result.sample_clusters is not None
            ):
                print(f"Clusters: {len(result.sample_clusters.unique())}")
                print(
                    f"Cluster Distribution: {result.sample_clusters.value_counts().to_dict()}"
                )
        else:
            print("Integration failed or no integrated data available")

    def _print_processing_results(
        self, load_result: Any, preprocess_result: Any, processed_data: pd.DataFrame
    ):
        ...
        print(
            f"Data Range: [{processed_data.min().min():.3f}, {processed_data.max().max():.3f}]"
        )
...
Examples:
  ...
  python run_omics_analysis.py --process data.csv --type metabolomics --output processed.csv
    ...
    parser.add_argument(
        "--dashboard", action="store_true", help="Launch interactive dashboard"
    )
    parser.add_argument(
        "--list-fields", action="store_true", help="List all available omics fields"
    )
    parser.add_argument(
        "--field-info",
        type=str,
    ...
    # Data processing
    parser.add_argument(
        "--validate", type=str, metavar="FILE", help="Validate a data file"
    )
    parser.add_argument(
        "--type",
        type=str,
    ...
    parser.add_argument(
        "--process", type=str, metavar="FILE", help="Process a data file"
    )
    parser.add_argument(
        "--output", type=str, metavar="FILE", help="Output file for processed data"
    )

    # Integration
    parser.add_argument(
        "--integrate", nargs="+", metavar="FILE", help="Integrate multiple data files"
    )
    parser.add_argument(
        "--types",
        nargs="+",
    ...
    # Dashboard options
    parser.add_argument(
        "--port", type=int, default=8050, help="Port for dashboard (default: 8050)"
    )
    parser.add_argument(
        "--debug", action="store_true", help="Enable debug mode for dashboard"
    )
