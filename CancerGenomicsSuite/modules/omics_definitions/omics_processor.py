...
class OmicsDataValidator:
    ...
    def validate_metadata(
        ...
        if "data_type" in metadata and metadata["data_type"] != omics_type:
            errors.append(
                f"Metadata data_type mismatch: expected {omics_type}, got {metadata['data_type']}"
            )
...
class OmicsQualityControl:
    ...
    def calculate_reproducibility(
        self, data: pd.DataFrame, replicates: Optional[List[str]] = None
    ) -> float:
    ...
    def calculate_accuracy(
        self, data: pd.DataFrame, reference: Optional[pd.DataFrame] = None
    ) -> float:
    ...
    def calculate_quality_metrics(
        ...
        for metric in field_def.quality_control_metrics:
            ...
            elif metric == "mapping_rate" and omics_type in [
                "genomics",
                "transcriptomics",
            ]:
...
class OmicsDataProcessor(ABC):
    ...
    def quality_control(
        self, data: pd.DataFrame, omics_type: str, **kwargs
    ) -> QualityControlMetrics:
    ...
    def preprocess_data(self, data: pd.DataFrame, **kwargs) -> ProcessingResult:
        ...
        try:
            ...
            if "min_detection_pvalue" in kwargs:
                ...
                processing_log.append(
                    f"Filtered probes with detection p-value > {min_pvalue}"
                )
