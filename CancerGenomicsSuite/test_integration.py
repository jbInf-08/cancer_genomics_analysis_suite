...
def test_unified_interface():
    ...
    try:
        ...
        options = BiomarkerAnalysisOptions(
            p_value_threshold=0.05, effect_size_threshold=0.2
        )
        print(
            f"✅ Analysis options created: p-value threshold = {options.p_value_threshold}"
        )
...
def create_sample_biomarker_data():
    ...
    try:
        ...
        print(
            f"✅ Sample data created: {data.shape[0]} samples, {data.shape[1]} features"
        )
        print(
            f"   Labels: {np.sum(labels)} positive, {n_samples - np.sum(labels)} negative"
        )
...
def run_biomarker_discovery(data, labels):
    ...
    try:
        ...
        if biomarkers:
            ...
            print(
                f"   Best biomarker: {best.get('name', 'Unknown')} "
