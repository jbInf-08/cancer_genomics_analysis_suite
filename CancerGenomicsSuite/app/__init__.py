...
def register_blueprints(app):
    ...
    def index():
        ...
        return render_template(
            "index.html", app_name=settings.app_name, app_version=settings.app_version
        )
        ...
        return jsonify(
            {
                "status": "operational",
                "features": {
                    "gene_expression_analysis": settings.get_feature_status(
                        "enable_gene_expression_analysis"
                    ),
                    "mutation_analysis": settings.get_feature_status(
                        "enable_mutation_analysis"
                    ),
                    "machine_learning": settings.get_feature_status(
                        "enable_machine_learning"
                    ),
                    "pathway_analysis": settings.get_feature_status(
                        "enable_pathway_analysis"
                    ),
                    "multi_omics_integration": settings.get_feature_status(
                        "enable_multi_omics_integration"
                    ),
                },
                "external_services": {
    ...
    def celery_status():
        ...
        try:
            from .celery_config import get_queue_lengths, get_worker_stats, health_check
