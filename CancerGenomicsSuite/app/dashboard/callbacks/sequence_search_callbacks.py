...
def register_sequence_search_callbacks(app):
    ...
    def perform_sequence_search(
        n_clicks, sequence_input, search_type, database, parameters
    ):
        """
    ...
    def download_sequence_results(n_clicks, search_results, analysis_results):
        ...
        try:
            ...
            return (
                dict(content=results_text, filename="sequence_analysis_results.txt"),
                no_update,
            )
...
def generate_search_results_html(results: List[Dict]) -> html.Div:
    ...
    return html.Div([html.H4("Search Results"), html.Div(result_items)])
...
def generate_sequence_stats(sequence: str, results: List[Dict]) -> html.Div:
    ...
    return html.Div([html.H4("Search Statistics"), html.Div(stats_items)])


def generate_analysis_results_html(
        ...
        if isinstance(value, dict):
            result_items.append(
                html.Div(
                    [
                        html.Strong(f"{key}: "),
                        html.Div([html.P(f"{k}: {v}") for k, v in value.items()]),
                    ]
                )
            )
        else:
    ...
    return html.Div(
        [html.H4(f"{analysis_type} Analysis Results"), html.Div(result_items)]
    )
...
def create_alignment_plot(alignment_result: Dict[str, Any]) -> go.Figure:
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            tickmode="array", tickvals=[0, 1], ticktext=["Sequence 2", "Sequence 1"]
        ),
    )
...
def generate_motif_results_html(motif_results: List[Dict]) -> html.Div:
    ...
    return html.Div([html.H4("Motif Search Results"), html.Div(result_items)])
