...
def register_reporting_callbacks(app):
    ...
    @app.callback(
        ...
        [State("report-job-select", "value"), State("report-format-select", "value")],
    )
...
def generate_analysis_summary_report(parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for job in jobs:
        ...
        summary_stats["analysis_types"][job_type] = (
            summary_stats["analysis_types"].get(job_type, 0) + 1
        )
...
def generate_data_export_report(parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if "gene_expression" in data_types:
        ...
        export_data["gene_expression"] = [
            record.to_dict() for record in expression_data
        ]
...
def generate_custom_report(template: str, parameters: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {"type": "custom", "template": template, "data": parameters}


def create_report_job(
    report_type: str, template: str, parameters: Dict[str, Any], format_type: str
) -> AnalysisJob:
    """
...
def create_batch_export_job(selection: List[str], format_type: str) -> AnalysisJob:
    ...
    job = AnalysisJob(
        ...
        status="pending",
        input_data=json.dumps({"selection": selection, "format": format_type}),
    )
...
def create_scheduled_report_job(
    report_type: str, frequency: str, email: str
) -> AnalysisJob:
    """
    ...
    job = AnalysisJob(
        ...
        status="scheduled",
        input_data=json.dumps(
            {"report_type": report_type, "frequency": frequency, "email": email}
        ),
