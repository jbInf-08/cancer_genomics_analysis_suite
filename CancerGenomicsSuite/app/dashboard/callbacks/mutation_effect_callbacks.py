...
def register_mutation_effect_callbacks(app):
    ...
    def update_mutation_effect_plot(
        selected_genes,
        selected_pathogenicity,
        plot_type,
        refresh_clicks,
        sample_filter,
        cancer_type_filter,
        ...
        try:
            ...
            if selected_pathogenicity:
                query = query.filter(
                    MutationRecord.pathogenicity.in_(selected_pathogenicity)
                )

            if sample_filter:
                query = query.filter(
                    MutationRecord.sample_id.like(f"%{sample_filter}%")
                )

            if cancer_type_filter:
                query = query.filter(
                    MutationRecord.cancer_type.like(f"%{cancer_type_filter}%")
                )
    ...
    def update_pathogenicity_options(refresh_clicks):
        ...
        try:
            pathogenicity_levels = (
                db.session.query(MutationRecord.pathogenicity)
                .distinct()
                .filter(MutationRecord.pathogenicity.isnot(None))
                .all()
            )
            return [
                {"label": path[0], "value": path[0]} for path in pathogenicity_levels
            ]
        except Exception as e:
                ...
                if mutation:
                    return html.Div(
                        [
                            ...
                            html.P(
                                f"Clinical Significance: {mutation.clinical_significance or 'N/A'}"
                            ),
                            html.P(f"Pathogenicity: {mutation.pathogenicity or 'N/A'}"),
                            html.P(
                                f"Allele Frequency: {mutation.allele_frequency or 'N/A'}"
                            ),
        ...
        try:
            ...
            return html.Div(
                [
                    ...
                    html.P(
                        f"Predicted Effect: {prediction_result['predicted_effect']}"
                    ),
                    html.P(
                        f"Pathogenicity Score: {prediction_result['pathogenicity_score']}"
                    ),
                    html.P(f"Confidence: {prediction_result['confidence']}"),
                    html.P(
                        f"Clinical Significance: {prediction_result['clinical_significance']}"
                    ),
                ]
            )
...
def create_mutation_scatter_plot(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        xaxis_title="Allele Frequency", yaxis_title="Quality Score", hovermode="closest"
    )
...
def generate_mutation_stats(df: pd.DataFrame) -> html.Div:
    ...
    return html.Div([html.H4("Mutation Statistics"), html.Div(stats_items)])
