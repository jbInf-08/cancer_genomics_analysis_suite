...
def register_gene_expression_callbacks(app):
    ...
    def update_gene_expression_plot(
        selected_genes, selected_conditions, plot_type, refresh_clicks, sample_filter
    ):
        """
        ...
        try:
            ...
            if sample_filter:
                query = query.filter(
                    GeneExpression.sample_id.like(f"%{sample_filter}%")
                )
    ...
    def update_condition_options(refresh_clicks):
        ...
        try:
            conditions = (
                db.session.query(GeneExpression.condition)
                .distinct()
                .filter(GeneExpression.condition.isnot(None))
                .all()
            )
    ...
    def download_gene_expression_data(
        n_clicks, selected_genes, selected_conditions, sample_filter
    ):
        """
        ...
        try:
            ...
            return (
                dict(content=csv_string, filename="gene_expression_data.csv"),
                no_update,
            )
...
def create_scatter_plot(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(
        xaxis_title="Sample ID", yaxis_title="Expression Value", hovermode="closest"
    )
...
def create_box_plot(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(xaxis_title="Condition", yaxis_title="Expression Value")
...
def create_heatmap(df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(xaxis_title="Sample ID", yaxis_title="Gene Symbol")
...
def generate_expression_stats(df: pd.DataFrame) -> html.Div:
    ...
    return html.Div([html.H4("Statistics"), html.Div(stats_items)])
