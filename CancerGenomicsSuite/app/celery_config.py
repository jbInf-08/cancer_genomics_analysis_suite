...
celery_app.conf.update(
    ...
    task_queues=(
        ...
        Queue(
            "expression_analysis",
            Exchange("expression_analysis"),
            routing_key="expression_analysis",
        ),
        Queue(
            "mutation_analysis",
            Exchange("mutation_analysis"),
            routing_key="mutation_analysis",
        ),
...
if (
    "amqp" in settings.celery.broker_url.lower()
    or "rabbitmq" in settings.celery.broker_url.lower()
):
    celery_app.conf.update(
        broker_transport_options={
            "visibility_timeout": 3600,
