...
class GeneExpression(db.Model):
    ...
    expression_unit = db.Column(
        db.String(20), default="FPKM"
    )  # FPKM, TPM, counts, etc.
    ...
    data_file = db.relationship(
        "DataFile", backref=db.backref("gene_expression_data", lazy=True)
    )
    ...
    def get_expression_summary(cls, gene_symbol: str):
        ...
        result = (
            ...
            .filter_by(gene_symbol=gene_symbol)
            .first()
        )
...
class MutationRecord(db.Model):
    ...
    data_file_id = db.Column(db.Integer, db.ForeignKey("data_files.id"), nullable=True)
    data_file = db.relationship(
        "DataFile", backref=db.backref("mutation_data", lazy=True)
    )
    ...
    def get_mutation_summary(cls, gene: str):
        ...
        pathogenic_count = cls.query.filter_by(
            gene=gene, pathogenicity="pathogenic"
        ).count()
        ...
        return {
            ...
            "avg_allele_frequency": float(result.avg_allele_frequency)
            if result.avg_allele_frequency
            else 0,
        }
...
class AnalysisJob(db.Model):
    ...
    )  # gene_expression, mutation, pathway, ml
    ...
    # Relationships
    results_rel = db.relationship(
        "AnalysisResult", backref="job", lazy=True, cascade="all, delete-orphan"
    )
    ...
    def set_progress(
        self, progress: float, estimated_completion: Optional[datetime] = None
    ):
        """Update job progress."""
    ...
    def get_running_jobs(cls):
        ...
        return (
            cls.query.filter_by(status="running")
    ...
    def get_job_statistics(cls):
        ...
        return {
            "by_status": {
                stat.status: {
                    "count": stat.count,
                    "avg_progress": float(stat.avg_progress or 0),
                }
                for stat in stats
            },
...
class DataFile(db.Model):
    ...
    # Relationships
    gene_expression_data = db.relationship(
        "GeneExpression", backref="data_file", lazy=True
    )
    ...
    def to_dict(self):
        ...
        return {
            ...
            "validation_errors": json.loads(self.validation_errors)
    ...
    def get_pending_processing(cls):
        ...
        return (
            cls.query.filter_by(processing_status="pending")
            .order_by(cls.upload_date.asc())
            .all()
        )
...
class AnalysisResult(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    job_id = db.Column(
        db.Integer, db.ForeignKey("analysis_jobs.id"), nullable=False, index=True
    )
    result_type = db.Column(
        db.String(50), nullable=False, index=True
    )  # plot, table, summary, report, etc.
    ...
    )  # Whether result is publicly accessible
    created_at = db.Column(db.DateTime, default=datetime.utcnow, index=True)
    updated_at = db.Column(
        db.DateTime, default=datetime.utcnow, onupdate=datetime.utcnow
    )
...
class Dataset(db.Model):
    ...
    project_id = db.Column(
        db.String(100), db.ForeignKey("projects.project_id"), nullable=True, index=True
    )
    owner_id = db.Column(
        db.Integer, db.ForeignKey("users.id"), nullable=False, index=True
    )
    ...
    # Dataset metadata
    dataset_metadata = db.Column(
        db.Text, nullable=True
    )  # JSON string of dataset metadata
    tags = db.Column(db.Text, nullable=True)  # JSON array of tags
...
def create_sample_data():
    ...
    try:
        ...
        try:
            sample_job = AnalysisJob(
                ...
                input_data=json.dumps(
                    {"genes": ["BRCA1", "TP53"], "samples": ["SAMPLE_001"]}
                ),
        ...
        return {"created": {}, "errors": [f"Sample data creation failed: {str(e)}"]}
