...
ROLES = {
    ...
    "analyst": {
        "level": 60,
        "permissions": ["read_data", "run_analysis", "export_results", "view_reports"],
...
def validate_password(password: str) -> Dict[str, Any]:
    ...
    if len(password) < PASSWORD_REQUIREMENTS["min_length"]:
        errors.append(
            f"Password must be at least {PASSWORD_REQUIREMENTS['min_length']} characters long"
        )

    # Check for uppercase letters
    if PASSWORD_REQUIREMENTS["require_uppercase"] and not any(
        c.isupper() for c in password
    ):
        errors.append("Password must contain at least one uppercase letter")

    # Check for lowercase letters
    if PASSWORD_REQUIREMENTS["require_lowercase"] and not any(
        c.islower() for c in password
    ):
        errors.append("Password must contain at least one lowercase letter")

    # Check for numbers
    if PASSWORD_REQUIREMENTS["require_numbers"] and not any(
...
def update_user_password(user: User, old_password: str, new_password: str) -> bool:
    ...
    try:
        ...
        if not verify_password(old_password, user.password_hash):
            logger.warning(
                f"Password update failed: invalid old password for user '{user.username}'"
            )
            return False
        ...
        if not password_validation["valid"]:
            logger.warning(
                f"Password update failed: validation failed for user '{user.username}'"
            )
            return False
    ...
    try:
        ...
        if created_at_str:
            ...
            if datetime.utcnow() - created_at > timedelta(
                seconds=DEFAULT_SESSION_TIMEOUT
            ):
