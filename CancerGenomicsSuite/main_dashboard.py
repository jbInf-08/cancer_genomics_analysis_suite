...
server = Flask(__name__, static_folder=str(STATIC_DIR), static_url_path="/static")
...
try:
    app_layout_content = [
        ...
        html.Link(
            rel="stylesheet",
            href="/static/css/dashboard_light.css",
            id="light-theme",
            media="all",
        ),
        html.Link(
            rel="stylesheet",
            href="/static/css/dashboard_dark.css",
            id="dark-theme",
            media="none",
        ),
            ...
            [
                html.Div(
                    [
                        ...
                        html.H1(
                            "Cancer Genomics Analysis Suite", className="app-title"
                        ),
                    ],
                ...
                html.Div(
                    [
                        ...
                        html.Div(
                            [
                                html.I(
                                    className="fa-solid fa-sun dark-light-switch clickable purple-on-hover",
                                    id="light-mode-icon",
                                    title="Light Mode",
                                    n_clicks=0,
                                ),
                                ...
                                html.I(
                                    className="fa-solid fa-moon dark-light-switch clickable purple-on-hover",
                                    id="dark-mode-icon",
                                    title="Dark Mode",
                                    n_clicks=0,
                                ),
                            ],
                            id="dark-light-switch-container",
                            className="dark-light-switch-container",
                        ),
                    ],
            ...
            [
                html.Div(
                    [
                        ...
                        else html.Ul([], className="sidebar-menu"),
                ...
                html.Div(
                    [
                        dcc.Tabs(
                            ...
                            children=[
                                dcc.Tab(label=label, value=label)
                                for label in plugins.keys()
                            ]
                            if plugins and len(plugins) > 0
                            else [dcc.Tab(label="Welcome", value="welcome")],
                            className="main-tabs",
                        ),
                        html.Div(
                            [
        ...
        html.Div(
            [
                ...
                html.P(
                    "No modules are currently available. Please check the plugin registry configuration."
                ),
                html.P(
                    f"Environment: {settings.flask_env} | Debug: {settings.dash_debug_mode}"
                ),
            ],
            id="welcome-message",
            className="welcome-message",
            style={"display": "none" if plugins else "block"},
        ),
    ...
    app.layout = html.Div(
        [
            ...
            html.Pre(
                error_trace,
                style={"fontSize": "10px", "overflow": "auto", "maxHeight": "400px"},
            ),
        ]
    )
...
app.clientside_callback(
    ...
    """,
    Output(
        "dark-light-switch-container", "id"
    ),  # Dummy output, we just need to trigger
    [Input("light-mode-icon", "n_clicks"), Input("dark-mode-icon", "n_clicks")],
)
...
def render_tab(tab_name):
    ...
    try:
        ...
        print(
            f"DEBUG: plugins available: {list(plugins.keys()) if plugins else 'None'}"
        )
        ...
        if plugins is None:
            ...
            return html.Div(
                [
                    ...
                    html.P(
                        "Plugins were not loaded correctly. Please check the server logs."
                    ),
                ]
            )
        ...
        if tab_name not in plugins:
            print(
                f"DEBUG: tab_name '{tab_name}' not in plugins, available: {list(plugins.keys())}"
            )
            ...
            if plugins and len(plugins) > 0:
                ...
                return html.Div(
                    [
                        ...
                        html.P(
                            f"Available modules: {', '.join(plugins.keys()) if plugins else 'None'}"
                        ),
                    ]
                )
        ...
        try:
            ...
            error_trace = traceback.format_exc()
            print(
                f"ERROR: Error rendering layout for '{tab_name}': {str(layout_error)}"
            )
            print(error_trace)
            return html.Div(
                [
                    ...
                    html.P(
                        f"Error rendering layout for '{tab_name}': {str(layout_error)}"
                    ),
                    html.Pre(
                        error_trace,
                        style={
