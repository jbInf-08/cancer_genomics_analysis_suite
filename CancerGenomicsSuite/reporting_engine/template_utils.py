...
class TemplateUtils:
    ...
    def format_date(
        self, date_value: Union[str, datetime], format_string: str = "%Y-%m-%d"
    ) -> str:
        """
    ...
    def format_dataframe(
        self, df: pd.DataFrame, max_rows: int = 1000, max_cols: int = 20
    ) -> pd.DataFrame:
        """
    ...
    def create_table_html(
        ...
        """
        return df.to_html(
            table_id=table_id, classes=css_classes, index=include_index, escape=False
        )

    def create_summary_stats(
        self, data: Union[List, pd.Series, pd.DataFrame]
    ) -> Dict[str, Any]:
        """
    ...
    def validate_template(self, template: str) -> Dict[str, Any]:
        ...
        if open_braces != close_braces:
            errors.append(
                f"Unmatched variable braces: {open_braces} open, {close_braces} close"
            )
        ...
        if if_blocks != endif_blocks:
            errors.append(
                f"Unmatched if/endif blocks: {if_blocks} if, {endif_blocks} endif"
            )
        ...
        if for_blocks != endfor_blocks:
            errors.append(
                f"Unmatched for/endfor blocks: {for_blocks} for, {endfor_blocks} endfor"
            )
