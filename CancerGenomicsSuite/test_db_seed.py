...
class DatabaseSeeder:
    ...
    def generate_gene_expression_data(
        self, num_samples: int, num_genes: int
    ) -> List[Dict[str, Any]]:
        ...
        selected_genes = random.sample(
            self.cancer_genes, min(num_genes, len(self.cancer_genes))
        )
    ...
    def generate_mutation_data(
        self, num_samples: int, mutations_per_gene: int
    ) -> List[Dict[str, Any]]:
            ...
            for gene in mutated_genes:
                ...
                for _ in range(num_mutations):
                    ...
                    mut_type = random.choices(
                        ...
                        weights=[
                            mt["frequency"] for mt in self.mutation_types.values()
                        ],
                    )[0]
                    ...
                    mutation_data.append(
                        {
                            ...
                            "source": random.choice(
                                ["WGS", "WES", "Targeted Panel", "RNA-seq"]
                            ),
                            "created_at": datetime.now(),
                        }
                    )
    ...
    def generate_pathway_data(self) -> List[Dict[str, Any]]:
        ...
        pathways = [
            ...
            {
                "name": "DNA Repair",
                "genes": ["BRCA1", "BRCA2", "ATM", "CHEK2", "MLH1", "MSH2"],
            },
            ...
            pathway_data.append(
                {
                    ...
                    "category": random.choice(
                        ["Oncogenic", "Tumor Suppressor", "DNA Repair", "Immune"]
                    ),
    ...
    def _determine_pathogenicity(self, gene: str, mut_type: str) -> str:
        ...
        if gene in ["TP53", "BRCA1", "BRCA2", "RB1", "PTEN"] and mut_type in [
            "nonsense",
            "frameshift",
        ]:
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Show seeding plan without execution"
    )
