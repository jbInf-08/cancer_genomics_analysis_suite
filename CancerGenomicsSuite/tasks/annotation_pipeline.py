...
class AnnotationPipeline:
    ...
    def annotate_variants(
        self, variants: Union[pd.DataFrame, List[Dict]]
        ...
        missing_columns = [
            col for col in required_columns if col not in variants_df.columns
        ]
        if missing_columns:
        ...
        for i in range(0, total_variants, self.config.batch_size):
            ...
            self.logger.info(
                f"Processing batch {i//self.config.batch_size + 1}/{(total_variants-1)//self.config.batch_size + 1}"
            )
    ...
    def _get_gene_from_ensembl_db(
        self, annotation: VariantAnnotation
    ) -> Optional[Dict]:
    ...
    def _get_gene_from_ensembl_api(
        self, annotation: VariantAnnotation
    ) -> Optional[Dict]:
        ...
        try:
            ...
            if data and len(data) > 0:
                ...
                gene_info = {
                    ...
                    "consequence": result.get("consequence_terms", [None])[0]
                    if result.get("consequence_terms")
                    else None,
    ...
    def _add_functional_prediction(
        self, annotation: VariantAnnotation
    ) -> VariantAnnotation:
    ...
    def _add_clinical_significance(
        self, annotation: VariantAnnotation
    ) -> VariantAnnotation:
    ...
    def _get_clinvar_info(self, annotation: VariantAnnotation) -> Optional[Dict]:
        ...
        if not matches.empty:
            match = matches.iloc[0]
            return {"significance": match.get("clinical_significance")}
    ...
    def _get_cosmic_info(self, annotation: VariantAnnotation) -> Optional[Dict]:
        ...
        if not matches.empty:
            match = matches.iloc[0]
            return {"cosmic_id": match.get("cosmic_id")}

        return None

    def annotate_genes(
        self, genes: Union[List[str], pd.DataFrame]
    ) -> List[GeneAnnotation]:
        """
        ...
        if isinstance(genes, pd.DataFrame):
            if "gene_symbol" in genes.columns:
                ...
                raise ValueError(
                    "DataFrame must contain 'gene_symbol' or 'gene_id' column"
                )
        else:
            gene_symbols = genes
        ...
        for i in range(0, total_genes, self.config.batch_size):
            ...
            self.logger.info(
                f"Processing batch {i//self.config.batch_size + 1}/{(total_genes-1)//self.config.batch_size + 1}"
            )
    ...
    def _annotate_gene_batch(self, gene_symbols: List[str]) -> List[GeneAnnotation]:
        ...
        for gene_symbol in gene_symbols:
            annotation = GeneAnnotation(gene_id=gene_symbol, gene_symbol=gene_symbol)
    ...
    def _get_basic_gene_info_from_db(
        self, annotation: GeneAnnotation
    ) -> Optional[Dict]:
    ...
    def _get_basic_gene_info_from_api(
        self, annotation: GeneAnnotation
    ) -> Optional[Dict]:
    ...
    def save_annotations(
        self,
        annotations: List[Union[VariantAnnotation, GeneAnnotation]],
        output_path: str,
    ):
        """
                ...
                data.append(
                    {
                        ...
                        "disease_associations": ";".join(
                            annotation.disease_associations
                        ),
                    }
                )
    ...
    def generate_annotation_report(
        ...
        with open(output_path, "w") as f:
            ...
            variant_annotations = [
                a for a in annotations if isinstance(a, VariantAnnotation)
            ]
            ...
            if variant_annotations:
                ...
                for ann in variant_annotations:
                    if ann.consequence:
                        consequences[ann.consequence] = (
                            consequences.get(ann.consequence, 0) + 1
                        )

                if consequences:
                    f.write("\nConsequence distribution:\n")
                    for consequence, count in sorted(
                        consequences.items(), key=lambda x: x[1], reverse=True
                    ):
            ...
            if gene_annotations:
                ...
                if gene_types:
                    ...
                    for gene_type, count in sorted(
                        gene_types.items(), key=lambda x: x[1], reverse=True
                    ):
