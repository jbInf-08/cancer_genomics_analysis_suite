...
class BlastPipeline:
    ...
    def _validate_blast_installation(self):
        ...
        try:
            ...
            raise RuntimeError(
                f"BLAST {self.config.program} is not installed or not in PATH"
            )
    ...
    def filter_results(
        ...
        for result in self.results:
            if (
                ...
                and result.evalue <= max_evalue
                and result.alignment_length >= min_alignment_length
            ):
                filtered_results.append(result)

        self.logger.info(
            f"Filtered {len(filtered_results)} results from {len(self.results)} total"
        )
        return filtered_results

    def results_to_dataframe(
        self, results: Optional[List[BlastResult]] = None
    ) -> pd.DataFrame:
        """
    ...
    def get_top_hits(
        self, n: int = 10, sort_by: str = "bit_score"
    ) -> List[BlastResult]:
        """
        ...
        if sort_by == "bit_score":
            sorted_results = sorted(
                self.results, key=lambda x: x.bit_score, reverse=True
            )
        elif sort_by == "evalue":
            sorted_results = sorted(self.results, key=lambda x: x.evalue)
        elif sort_by == "identity":
            sorted_results = sorted(
                self.results, key=lambda x: x.identity, reverse=True
            )
        else:
    ...
    def create_blast_database(
        self, sequences_file: str, database_name: str, db_type: str = "nucl"
    ) -> str:
        """
    ...
    def batch_blast_analysis(
        self, query_files: List[str], output_dir: str
    ) -> Dict[str, str]:
        """
    ...
    def generate_summary_report(self, output_file: str):
        ...
        with open(output_file, "w") as f:
            ...
            for i, hit in enumerate(top_hits, 1):
                f.write(
                    f"  {i}. {hit.subject_id} (Score: {hit.bit_score:.2f}, "
