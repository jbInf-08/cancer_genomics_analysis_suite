...
from ..reporting_engine import HTMLReporter, PDFBuilder, TemplateUtils
...
class ReportingPipeline:
    ...
    def _load_mutation_data(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "tables": [{"title": "Mutation Summary", "data": mutations_df.head(10)}],
            "charts": [
                {
                    "title": "Mutation Types Distribution",
                    "type": "bar",
                    "data": {
                        "categories": mutations_df["mutation_type"]
                        .value_counts()
                        .index.tolist(),
                        "values": mutations_df["mutation_type"]
                        .value_counts()
                        .values.tolist(),
                    },
                }
            ...
            "tables": [
                {
                    ...
                    "data": expression_df.groupby("gene")["expression_value"]
                    .agg(["mean", "std"])
                    .reset_index(),
                }
            ],
    ...
    def _load_clinical_data(self, filters: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "tables": [{"title": "Clinical Summary", "data": clinical_df.describe()}],
            "charts": [
                {
    ...
    def _generate_pdf_report(
        self, data: Dict[str, Any], config: ReportConfig, job_id: str
    ) -> str:
        """Generate PDF report."""
    ...
    def _generate_html_report(
        ...
        # Set metadata
        html_reporter.set_metadata(title=data["title"], description=data["summary"])
                ...
                if chart_info["type"] in config.chart_types:
                    if chart_info["type"] == "bar":
                        fig = html_reporter.create_bar_chart(
                            chart_info["data"], chart_info.get("title", "Chart")
                        )
                    ...
                    html_reporter.add_chart_section(
                        chart_info.get("title", "Chart"), fig
                    )
    ...
    def _generate_filename(
        self, config: ReportConfig, job_id: str, extension: str
    ) -> str:
    ...
    def _send_report_email(self, job: ReportJob):
        ...
        try:
            ...
            text = msg.as_string()
            server.sendmail(
                self.email_config["from_email"], self.config.email_recipients, text
            )
            server.quit()
    ...
    def schedule_report(self, custom_config: Optional[ReportConfig] = None):
        ...
        self.logger.info(
            f"Report scheduled: {config.schedule_frequency} at {config.schedule_time}"
        )
