...
class MLConfig:
    ...
    model_type: str = (
        "random_forest"  # random_forest, svm, logistic_regression, gradient_boosting
    )
    test_size: float = 0.2
    random_state: int = 42
    cv_folds: int = 5
...
class MLPredictorPipeline:
    ...
    def preprocess_data(
        ...
        if len(categorical_columns) > 0:
            ...
            X_processed = pd.get_dummies(
                X_processed, columns=categorical_columns, drop_first=True
            )
    ...
    def select_features(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        if self.config.feature_selection_method == "mutual_info":
            ...
            raise ValueError(
                f"Unsupported feature selection method: {self.config.feature_selection_method}"
            )
    ...
    def save_model(self, filepath: Optional[str] = None):
        ...
        if filepath is None:
            filepath = (
                self.config.model_save_path or f"model_{self.config.model_type}.pkl"
            )
    ...
    def plot_feature_importance(self, top_n: int = 20, save_path: Optional[str] = None):
        ...
        """
        if (
            not self.performance_metrics
            or not self.performance_metrics.feature_importance
        ):
    ...
    def generate_report(self, output_path: str):
        ...
        with open(output_path, "w") as f:
            ...
            if self.config.feature_selection:
                f.write(
                    f"Feature Selection Method: {self.config.feature_selection_method}\n"
                )
            ...
            f.write(
                f"  Mean: {np.mean(self.performance_metrics.cross_val_scores):.4f}\n"
            )
        ...
        for data_file in data_files:
            ...
            try:
                ...
                output_file = os.path.join(
                    output_dir, f"predictions_{os.path.basename(data_file)}"
                )
