...
class ScopusClient:
    ...
    def search_articles(
        ...
        if subject_areas:
            subject_query = " OR ".join(
                [f'SUBJAREA("{area}")' for area in subject_areas]
            )
            params["query"] = f"({query}) AND ({subject_query})"

        # Add document type filter
        if document_types:
            doc_type_query = " OR ".join(
                [f'DOCTYPE("{doc_type}")' for doc_type in document_types]
            )
        ...
        try:
            ...
            for author in entry.get("author", []):
                author_info = {
                    ...
                    "affiliation_ids": [
                        aff.get("@id", "") for aff in author.get("affiliation", [])
                    ],
                }
        ...
        try:
            ...
            for author in entry.get("authors", {}).get("author", []):
                author_info = {
                    ...
                    "given_name": author.get("preferred-name", {}).get(
                        "ce:given-name", ""
                    ),
            ...
            funding = []
            for fund in (
                entry.get("funding", {}).get("funding-list", {}).get("funding", [])
            ):
                fund_info = {
                    "agency": fund.get("funding-agency", {}).get(
                        "funding-agency-name", ""
                    ),
                    "grant_number": fund.get("funding-agency", {}).get(
                        "funding-agency-grant-number", ""
                    ),
                    "country": fund.get("funding-agency", {}).get(
    ...
    def _parse_author(self, entry: Dict[str, Any]) -> Optional[ScopusAuthor]:
        ...
        try:
            ...
            affiliation_ids = [
                aff.get("@id", "")
                for aff in entry.get("affiliation-history", {}).get("affiliation", [])
            ]
    ...
    def get_affiliation_details(
        self, affiliation_id: str
    ) -> Optional[ScopusAffiliation]:
        """
        ...
        try:
            ...
            entry = data.get("affiliation-retrieval-response", {}).get(
                "affiliation", {}
            )
            ...
            logger.error(
                f"Failed to fetch affiliation details for {affiliation_id}: {e}"
            )
            return None
    ...
    def search_authors(self, query: str, max_results: int = 100) -> List[ScopusAuthor]:
        ...
        """
        params = {"query": f"AUTH({query})", "count": min(max_results, 200), "start": 0}
    ...
    def _parse_author_search_result(
        self, entry: Dict[str, Any]
    ) -> Optional[ScopusAuthor]:
        """
        ...
        try:
            ...
            # Extract affiliation IDs
            affiliation_ids = [
                aff.get("@id", "") for aff in entry.get("affiliation", [])
            ]
    ...
    def export_articles_to_dataframe(
        self, articles: List[ScopusArticle]
    ) -> pd.DataFrame:
        """
...
def search_cancer_genomics_literature(
    query: str, client: Optional[ScopusClient] = None, max_results: int = 100
) -> List[ScopusArticle]:
    """
...
def get_author_publications(
    author_name: str, client: Optional[ScopusClient] = None, max_results: int = 100
) -> List[ScopusArticle]:
    """
