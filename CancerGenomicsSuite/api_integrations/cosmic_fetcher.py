...
class COSMICFetcher:
    ...
    def get_tissue_mutation_patterns(self, tissue_type: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to fetch tissue mutation patterns for {tissue_type}: {e}"
            )
            return []
    ...
    def get_drug_sensitivity_data(self, gene_symbol: str) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            logger.error(
                f"Failed to fetch drug sensitivity data for {gene_symbol}: {e}"
            )
            return []
    ...
    def batch_fetch_mutations(
        self, gene_symbols: List[str], cancer_type: Optional[str] = None
...
def fetch_cancer_genes(
    cancer_type: str, fetcher: Optional[COSMICFetcher] = None
) -> pd.DataFrame:
    """
    ...
    if cancer_type and not gene_df.empty:
        ...
        gene_df = gene_df[
            gene_df["cancer_types"].str.contains(cancer_type, case=False, na=False)
        ]

    return gene_df


def fetch_mutation_summary(
    gene_symbol: str, fetcher: Optional[COSMICFetcher] = None
) -> Dict[str, Any]:
    """
