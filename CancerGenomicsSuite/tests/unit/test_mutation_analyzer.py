...
class TestMutationAnalyzer:
    ...
    @pytest.mark.parametrize(
        "invalid_input", [None, "string", 123, {"invalid": "data"}]
    )
    ...
    def test_filter_mutations_by_sample(self, analyzer, sample_mutations):
        ...
        sample_001_mutations = analyzer.filter_mutations_by_sample(
            sample_mutations, "SAMPLE_001"
        )

        assert len(sample_001_mutations) == 2
        assert all(
            mutation["sample_id"] == "SAMPLE_001" for mutation in sample_001_mutations
        )
