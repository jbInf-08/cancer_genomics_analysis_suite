...
class MockDataGenerator:
    ...
    def _generate_base_data(self):
        ...
        for i in range(self.config.num_patients):
            ...
            cancer_config = self.cancer_configs.get(
                cancer_type, self.cancer_configs["BRCA"]
            )
        ...
        self.logger.info(
            f"Generated {len(self.patients)} patients and {len(self.samples)} samples"
        )
    ...
    def generate_clinical_data(self) -> str:
        ...
        for patient in self.patients:
            ...
            patient_samples = [
                s for s in self.samples if s["patient_id"] == patient["patient_id"]
            ]
            if patient_samples:
                ...
                clinical_record[
                    "sample_id"
                ] = f"S{str(random.randint(1, 1000)).zfill(3)}"
    ...
    def generate_expression_data(self) -> str:
        ...
        for sample in self.samples:
            ...
            patient = next(
                (p for p in self.patients if p["patient_id"] == sample["patient_id"]),
                None,
            )
            if not patient:
                continue
            ...
            cancer_type = self._get_cancer_type_from_diagnosis(
            ...
            selected_genes = random.sample(
                self.config.gene_list, min(num_genes, len(self.config.gene_list))
            )
    ...
    def generate_mutation_data(self) -> str:
        ...
        cancer_mutations = {
            "TP53": [
                ("17", 7574003, "G", "A", "p.R175H"),
                ("17", 7577120, "C", "T", "p.R248Q"),
            ],
            "BRCA1": [
                ("17", 43094695, "G", "C", "p.R1699W"),
                ("17", 43094695, "G", "T", "p.R1699L"),
            ],
            "BRCA2": [
                ("13", 32316467, "G", "A", "p.N372H"),
                ("13", 32316467, "G", "T", "p.N372Y"),
            ],
            "EGFR": [
                ("7", 55241707, "G", "T", "p.G719S"),
                ("7", 55241707, "G", "A", "p.G719A"),
            ],
            "KRAS": [
                ("12", 25245350, "G", "A", "p.G12D"),
                ("12", 25245350, "G", "T", "p.G12V"),
            ],
            "MYC": [
                ("8", 128748315, "G", "A", "p.P44L"),
                ("8", 128748315, "G", "T", "p.P44S"),
            ],
            "PIK3CA": [
                ("3", 178916876, "G", "A", "p.H1047R"),
                ("3", 178916876, "G", "T", "p.H1047L"),
            ],
            "PTEN": [
                ("10", 89692905, "G", "A", "p.R130Q"),
                ("10", 89692905, "G", "T", "p.R130L"),
            ],
            "APC": [
                ("5", 112175779, "G", "A", "p.R1450Q"),
                ("5", 112175779, "G", "T", "p.R1450L"),
            ],
            "RB1": [
                ("13", 48877883, "G", "A", "p.R661Q"),
                ("13", 48877883, "G", "T", "p.R661L"),
            ],
        }
        ...
        for sample in self.samples:
            ...
            for _ in range(num_mutations):
                ...
                if gene in cancer_mutations:
                    chrom, pos, ref, alt, protein_change = random.choice(
                        cancer_mutations[gene]
                    )
                else:
        ...
        if mutation_file.exists():
            ...
            for _, mutation in df_mutations.iterrows():
                ...
                polyphen2_prediction = (
                    "probably_damaging"
                    if polyphen2_score > 0.8
                    else "possibly_damaging"
                    if polyphen2_score > 0.5
                    else "benign"
                )
    ...
    def _generate_mock_pdb(self, protein_name: str, pdb_id: str) -> str:
        ...
        pdb_lines = [
            ...
            "KEYWDS    CANCER, ONCOGENE, TUMOR SUPPRESSOR",
            "REMARK   1 MOCK STRUCTURE FOR TESTING PURPOSES",
            "REMARK   2 GENERATED BY MOCK DATA GENERATOR",
            f"SEQRES   1 A   {random.randint(100, 500)}  ALA ARG ASN ASP CYS GLN GLU GLY HIS ILE LEU LYS MET PHE PRO SER THR TRP TYR VAL",
            "ATOM      1  N   ALA A   1      20.123  15.456  10.789  1.00 20.00           N",
            "ATOM      2  CA  ALA A   1      21.234  16.567  11.890  1.00 20.00           C",
            "ATOM      3  C   ALA A   1      22.345  17.678  12.901  1.00 20.00           C",
            "ATOM      4  O   ALA A   1      23.456  18.789  13.012  1.00 20.00           O",
            "END",
        ]
    ...
    def _get_cancer_type_from_diagnosis(self, diagnosis: str) -> str:
        ...
        if (
            "breast" in diagnosis_lower
            or "ductal" in diagnosis_lower
            or "lobular" in diagnosis_lower
        ):
            return "BRCA"
        elif (
            "lung" in diagnosis_lower
            or "adenocarcinoma" in diagnosis_lower
            or "squamous" in diagnosis_lower
        ):
            return "NSCLC"
...
Examples:
  ...
  python generate_mock_data.py --cancer-types BRCA NSCLC COAD
    ...
    parser.add_argument(
        "--cancer-types", nargs="+", help="Specific cancer types to include"
    )

    parser.add_argument(
        "--clinical-only", action="store_true", help="Generate only clinical data"
    )

    parser.add_argument(
        "--expression-only", action="store_true", help="Generate only expression data"
    )

    parser.add_argument(
        "--mutations-only", action="store_true", help="Generate only mutation data"
    )
