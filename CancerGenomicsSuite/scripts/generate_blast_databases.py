...
class BlastDatabaseGenerator:
    ...
    def fetch_gene_sequence(
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.warning(
                    f"Could not find gene {gene_symbol}: {response.status_code}"
                )
                return None
            ...
            self.logger.debug(
                f"Fetched sequence for {gene_symbol} ({len(sequence)} bp)"
            )
            return record
    ...
    def fetch_protein_sequence(
        ...
        try:
            ...
            if response.status_code != 200:
                self.logger.warning(
                    f"Could not fetch protein sequence for {gene_symbol}"
                )
                return None
            ...
            self.logger.debug(
                f"Fetched protein sequence for {gene_symbol} ({len(sequence)} aa)"
            )
            return record
    ...
    def generate_mock_sequences(
        self, gene_symbols: List[str], seq_type: str = "nucl"
    ) -> List[SeqRecord]:
        """
    ...
    def create_database(
        self, db_name: str, sequences: List[SeqRecord], db_type: str, description: str
    ) -> str:
        """
    ...
    def generate_all_databases(
        self, use_api: bool = True, use_mock: bool = True
    ) -> Dict[str, str]:
        """
    ...
    def create_custom_database(
        self, sequences_file: str, db_name: str, db_type: str, description: str = ""
    ) -> str:
        """
...
Examples:
  ...
  python generate_blast_databases.py --custom-db my_sequences.fasta --db-name custom_db --db-type nucl
    ...
    parser.add_argument(
        "--no-api", action="store_true", help="Skip API calls and use only mock data"
    )
    ...
    parser.add_argument("--custom-db", help="Create custom database from FASTA file")

    parser.add_argument("--db-name", help="Name for custom database")
    ...
    parser.add_argument(
        "--description", default="", help="Description for custom database"
    )
    ...
    generator = BlastDatabaseGenerator(
        output_dir=args.output_dir, log_level=args.log_level
    )
    ...
    if args.custom_db:
        ...
        db_path = generator.create_custom_database(
            args.custom_db, args.db_name, args.db_type, args.description
        )
