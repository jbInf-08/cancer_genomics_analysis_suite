...
class IntegratedServicesManager:
    ...
    def _get_default_config(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "monitoring": {"enabled": True, "interval": 30, "timeout": 5},
        }
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        status = {
            ...
            "monitoring": {
                ...
                "active": self.service_discovery.monitoring_active
                if self.service_discovery
                else False,
            },
        }
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Start integrated biomarker analysis services"
    )
    parser.add_argument("--config", help="Path to configuration file")
    parser.add_argument(
        "--cgas-only", action="store_true", help="Start only CGAS service"
    )
    parser.add_argument(
        "--biomarker-only",
        action="store_true",
        help="Start only Biomarker Identifier service",
    )
    parser.add_argument(
        "--no-wait", action="store_true", help="Do not wait for services to be ready"
    )
