...
class IntegrationConfig:
    ...
    biomarker_identifier: BiomarkerIdentifierConfig = field(
        default_factory=BiomarkerIdentifierConfig
    )
        ...
        return cls(
            biomarker_identifier=BiomarkerIdentifierConfig(
                ...
                health_endpoint=os.getenv(
                    "BIOMARKER_IDENTIFIER_HEALTH_ENDPOINT", "/health"
                ),
                api_endpoint=os.getenv(
                    "BIOMARKER_IDENTIFIER_API_ENDPOINT", "/api/analyze"
                ),
                enabled=os.getenv("BIOMARKER_IDENTIFIER_ENABLED", "true").lower()
                == "true",
            ),
            cgas=CGASConfig(
            ...
            enable_caching=os.getenv("INTEGRATION_ENABLE_CACHING", "true").lower()
            == "true",
            cache_ttl=int(os.getenv("INTEGRATION_CACHE_TTL", "3600")),
            enable_validation=os.getenv("INTEGRATION_ENABLE_VALIDATION", "true").lower()
            == "true",
            enable_aggregation=os.getenv(
                "INTEGRATION_ENABLE_AGGREGATION", "true"
            ).lower()
            == "true",
            prefer_fast_service=os.getenv(
                "INTEGRATION_PREFER_FAST_SERVICE", "true"
            ).lower()
            == "true",
            log_level=os.getenv("INTEGRATION_LOG_LEVEL", "INFO"),
            log_file=os.getenv("INTEGRATION_LOG_FILE"),
            max_concurrent_requests=int(
                os.getenv("INTEGRATION_MAX_CONCURRENT_REQUESTS", "10")
            ),
...
def update_config(config_dict: Dict[str, Any]) -> IntegrationConfig:
    ...
    for key, value in config_dict.items():
        if hasattr(DEFAULT_CONFIG, key) and key not in [
            "biomarker_identifier",
            "cgas",
            "service_discovery",
        ]:
