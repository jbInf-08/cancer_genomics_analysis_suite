...
class CGASCompatibilityAdapter:
        ...
        **kwargs,
        ...
        try:
            ...
            results = self.biomarker_analyzer.discover_biomarkers(
                cgas_data, cgas_labels, biomarker_type=biomarker_type, **kwargs
            )
    ...
    def _convert_labels_to_cgas_format(
        self, labels: Union[pd.Series, List, np.ndarray]
    ) -> pd.Series:
...
class BiomarkerIdentifierCompatibilityAdapter:
        ...
        **kwargs,
        ...
        try:
            ...
            response = requests.post(
                f"{self.base_url}/api/analyze", json=request_data, timeout=30
            )
    ...
    def _convert_to_bi_format(
        ...
        if isinstance(data, pd.DataFrame):
            ...
            return {"data": data.tolist(), "shape": data.shape}
        else:
            raise ValueError(f"Unsupported data type: {type(data)}")

    def _convert_labels_to_bi_format(
        self, labels: Union[pd.Series, List, np.ndarray]
    ) -> List:
        ...
        **kwargs,
        ...
        if service:
            if service not in available_services:
                raise ValueError(
                    f"Service '{service}' is not available. Available: {available_services}"
                )
            selected_service = service
        else:
            # Auto-select based on data size and complexity
            selected_service = self._select_best_service(
                data, labels, available_services
            )
...
def discover_biomarkers_compatible(
    data: Union[pd.DataFrame, Dict[str, Any]],
    labels: Union[pd.Series, List, np.ndarray],
    service: Optional[str] = None,
    **kwargs,
