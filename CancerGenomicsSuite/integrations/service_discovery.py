...
class ServiceDiscovery:
    ...
    def _load_default_config(self) -> Dict[str, Any]:
        ...
        return {
            "auto_monitoring": os.getenv(
                "SERVICE_DISCOVERY_AUTO_MONITORING", "true"
            ).lower()
            == "true",
            "monitoring_interval": int(os.getenv("SERVICE_DISCOVERY_INTERVAL", "30")),
            "health_check_timeout": int(os.getenv("HEALTH_CHECK_TIMEOUT", "5")),
            "max_consecutive_failures": int(os.getenv("MAX_CONSECUTIVE_FAILURES", "3")),
            "service_registration_timeout": int(
                os.getenv("SERVICE_REGISTRATION_TIMEOUT", "10")
            ),
        ...
        try:
            with self.lock:
                ...
                if self._check_service_health(endpoint):
                    ...
                    logger.info(
                        f"Registered service: {endpoint.name} at {endpoint.url}"
                    )
                    ...
                    logger.warning(
                        f"Registered unhealthy service: {endpoint.name} at {endpoint.url}"
                    )
                    return False
        ...
        try:
            with self.lock:
                if service_name in self.services:
                    ...
                    logger.warning(
                        f"Service not found for deregistration: {service_name}"
                    )
                    return False
        except Exception as e:
    ...
    def select_service(
        self, tags: Optional[List[str]] = None, strategy: Optional[str] = None
        ...
        if strategy == "round_robin":
            ...
            logger.warning(
                f"Unknown load balancing strategy: {strategy}, using round_robin"
            )
            return self._round_robin_selection(healthy_services)
    ...
    def _check_service_health(
        self, endpoint: ServiceEndpoint, service_health: Optional[ServiceHealth] = None
    ) -> bool:
        """
        ...
        try:
            ...
            response = requests.get(
                f"{endpoint.url}{endpoint.health_endpoint}", timeout=endpoint.timeout
            )
            ...
            if service_health:
                ...
                if is_healthy:
                    ...
                    if service_health.consecutive_failures >= self.config.get(
    ...
    def discover_services(self, discovery_config: Dict[str, Any]) -> List[str]:
        ...
        if discovery_config.get("dns_discovery", False):
            discovered.extend(
                self._discover_via_dns(discovery_config.get("dns_config", {}))
            )

        # Consul discovery
        if discovery_config.get("consul_discovery", False):
            discovered.extend(
                self._discover_via_consul(discovery_config.get("consul_config", {}))
            )

        # Static configuration discovery
        if discovery_config.get("static_discovery", False):
            discovered.extend(
                self._discover_via_static(discovery_config.get("static_config", {}))
            )
