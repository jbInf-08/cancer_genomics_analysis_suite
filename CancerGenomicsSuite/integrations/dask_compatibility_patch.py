...
def _apply_dask_patch():
    ...
    try:
        ...
        try:
            ...
            print(
                f"Warning: Could not apply dask patch: {error_msg.encode('ascii', 'replace').decode('ascii')}"
            )
...
def patch_lightgbm_import():
    ...
    try:
        ...
        try:
            ...
            print(
                f"[OK] LightGBM imported successfully (version: {lightgbm.__version__})"
            )
        return True
    except Exception as e:
        error_msg = str(e)
        try:
            ...
            print(
                f"[ERROR] LightGBM import failed: {error_msg.encode('ascii', 'replace').decode('ascii')}"
            )
        return False
...
def patch_xgboost_import():
    ...
    try:
        ...
        try:
            ...
            print(
                f"[OK] XGBoost imported successfully (version: {xgboost.__version__})"
            )
        return True
    except Exception as e:
        error_msg = str(e)
        try:
            ...
            print(
                f"[ERROR] XGBoost import failed: {error_msg.encode('ascii', 'replace').decode('ascii')}"
            )
        return False
