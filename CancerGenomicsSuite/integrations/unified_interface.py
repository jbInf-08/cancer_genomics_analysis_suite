...
from .biomarker_service import (
    AnalysisRequest,
    AnalysisResult,
    IntegratedBiomarkerService,
)
...
class BiomarkerAnalysisOptions:
    ...
    # Service selection
    prefer_service: Optional[
        str
    ] = None  # 'cgas', 'biomarker_identifier', or None for auto
...
class UnifiedBiomarkerInterface:
    ...
    def validate_biomarkers(
        ...
        # Perform validation
        validation_results = self.service.validate_biomarkers(
            biomarkers, validation_request
        )
    ...
    def _prepare_requirements(
        self, options: BiomarkerAnalysisOptions
    ) -> Dict[str, Any]:
        ...
        return {
            ...
            "summary": {
                ...
                "significant_biomarkers": len(
                    [b for b in result.biomarkers if b.get("p_value", 1.0) < 0.05]
                ),
                "high_effect_biomarkers": len(
                    [
                        b
                        for b in result.biomarkers
                        if abs(b.get("effect_size", 0.0)) > 0.5
                    ]
                ),
                "high_auc_biomarkers": len(
                    [b for b in result.biomarkers if b.get("auc_score", 0.5) > 0.8]
                ),
            },
            "metadata": {
    ...
    def _export_json(self, analysis: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            ...
            "options": analysis["options"].__dict__
            if hasattr(analysis["options"], "__dict__")
            else analysis["options"],
            "result": analysis.get("result", {}).__dict__
            if hasattr(analysis.get("result", {}), "__dict__")
            else analysis.get("result", {}),
        }
...
def validate_biomarkers(
    ...
    **kwargs,
    ...
    interface = UnifiedBiomarkerInterface()
    options = BiomarkerAnalysisOptions(**kwargs)
    return interface.validate_biomarkers(
        biomarkers, validation_data, validation_labels, options
    )
