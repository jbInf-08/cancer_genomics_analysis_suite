...
class IntegratedBiomarkerService:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None):
        ...
        self.service_discovery = ServiceDiscovery(
            self.config.get("discovery_config", {})
        )
    ...
    def analyze_biomarkers(self, request: AnalysisRequest) -> AnalysisResult:
        ...
        try:
            ...
            logger.info(
                f"Biomarker analysis completed in {result.processing_time:.2f}s using {result.service_used}"
            )
    ...
    def validate_biomarkers(
        self, biomarkers: List[Dict[str, Any]], validation_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self._is_service_available(ServiceType.BIOMARKER_IDENTIFIER):
                ...
                result = self.gateway.route_request(
                    "validate_biomarkers", validation_request
                )
                return result
    ...
    def compare_services(
        ...
        try:
            ...
            if self._is_service_available(ServiceType.CGAS):
                try:
                    cgas_result = self._perform_analysis(
                        ServiceType.CGAS, request, self._prepare_data(request)
                    )
            ...
            if self._is_service_available(ServiceType.BIOMARKER_IDENTIFIER):
                try:
                    bi_result = self._perform_analysis(
                        ServiceType.BIOMARKER_IDENTIFIER,
                        request,
                        self._prepare_data(request),
                    )
    ...
    def _select_service(self, request: AnalysisRequest) -> ServiceType:
        ...
        return self.gateway._select_service(
            request.analysis_type, prepared_data, request.requirements or {}
        )
    ...
    def _process_results(
        self, raw_results: Dict[str, Any], request: AnalysisRequest
    ) -> Dict[str, Any]:
        ...
        if "quality_score" not in processed["metadata"]:
            processed["quality_score"] = self._calculate_quality_score(
                processed["biomarkers"]
            )

        # Add validation results if enabled
        if self.config.get("enable_validation", True):
            processed["validation_results"] = self._validate_results(
                processed["biomarkers"], request
            )
    ...
    def _validate_results(
        self, biomarkers: List[Dict[str, Any]], request: AnalysisRequest
    ) -> Dict[str, Any]:
        """Validate analysis results."""
        validation = {
            "total_biomarkers": len(biomarkers),
            "significant_biomarkers": len(
                [b for b in biomarkers if b.get("p_value", 1.0) < 0.05]
            ),
            "high_effect_biomarkers": len(
                [b for b in biomarkers if abs(b.get("effect_size", 0.0)) > 0.5]
            ),
            "high_auc_biomarkers": len(
                [b for b in biomarkers if b.get("auc_score", 0.5) > 0.8]
            ),
        }
    ...
    def _analyze_service_differences(self, results: Dict[str, Any]) -> Dict[str, Any]:
        ...
        comparison = {
            ...
            "biomarker_counts": {
                service: len(result.get("biomarkers", []))
    ...
    def _on_service_state_change(self, event_type: str, service_name: str):
        ...
        if (
            ...
            and service_name in self.performance_metrics
        ):
